...
const collectionSyncWorker = areQueuesEnabled
  ? new Worker(
      ...
      async (job: Job<CollectionSyncJobData>) => {
        ...
        try {
          ...
          switch (data.type) {
            case "syncEvent":
              if (!data.eventId)
                throw new Error("eventId required for syncEvent");
              result = await collectionSyncService.syncEventToCollections(
                data.eventId,
              );
              break;
            ...
            case "removeEvent":
              if (!data.eventId)
                throw new Error("eventId required for removeEvent");
              result = await collectionSyncService.removeEventFromCollections(
                data.eventId,
              );
              break;
            ...
            case "syncCollection":
              if (!data.collectionId)
                throw new Error("collectionId required for syncCollection");
              result = await collectionSyncService.syncCollection(
                data.collectionId,
              );
              break;
            ...
            case "reconcileAll":
              ...
              if (mongoose.connection.readyState !== 1) {
                throw new Error(
                  "Cannot start reconciliation - MongoDB disconnected",
                );
              }
