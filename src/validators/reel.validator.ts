...
export const createReelValidation = [
  ...
  body("videoSourceType")
    ...
    .withMessage(
      `Video source type must be one of: ${VIDEO_SOURCE_TYPES.join(", ")}`,
    ),
 ...
 */
export const getReelsValidation = [...validatePagination];
...
export const updateDisplayOrdersValidation = [
  ...
  body("reels.*.id").isMongoId().withMessage("Each reel must have a valid ID"),
