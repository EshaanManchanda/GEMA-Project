...
export const validateBlogImport = [
  ...
  body("data.*.seo.metaKeywords.*").optional().isString().trim(),
...
export const validateBlogExportFilters = [
  ...
  body("filters.categoryName").optional().isString(),

  body("filters.dateRange.field")
    .optional()
    .isIn(["createdAt", "updatedAt", "publishedAt"])
    .withMessage(
      "dateRange.field must be: createdAt, updatedAt, or publishedAt",
    ),
