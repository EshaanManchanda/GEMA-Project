...
export const validateCollectionImport: ValidationChain[] = [
  ...
  body("data.*").custom((value) => {
    const hasEventTitles =
      value.eventTitles &&
      Array.isArray(value.eventTitles) &&
      value.eventTitles.length > 0;
    const hasEventIds =
      value.eventIds &&
  ...
  body("data.*.featuredImage")
    .optional()
    .isString()
    .withMessage("featuredImage must be a string")
    .isURL()
  ...
  body("data.*.seo").optional().isObject().withMessage("seo must be an object"),

  body("data.*.seo.metaTitle")
    .optional()
    .isString()
    .withMessage("seo.metaTitle must be a string")
    .trim()
    .isLength({ max: 70 })
  ...
  body("data.*.seo.metaDescription")
    .optional()
    .isString()
    .withMessage("seo.metaDescription must be a string")
    .trim()
    .isLength({ max: 160 })
  ...
  body("data.*.seo.metaKeywords")
    .optional()
    .isArray({ max: 10 })
    .withMessage("Meta keywords cannot have more than 10 items"),
  ...
  body("data.*.seo.canonicalUrl")
    .optional()
    .isString()
    .withMessage("seo.canonicalUrl must be a string")
    .trim()
    .isURL()
  ...
  body("data.*.count").custom((value) => {
    if (value !== undefined) {
      throw new Error(
        "count is auto-generated from event count and cannot be imported directly",
      );
    }
    return true;
  }),

  body("data.*.eventsData").custom((value) => {
    if (value !== undefined) {
      throw new Error(
        "eventsData is auto-regenerated from events[] and cannot be imported directly",
      );
    }
    return true;
  }),

  body("data.*.dataVersion").custom((value) => {
    if (value !== undefined) {
      throw new Error(
        "dataVersion is managed automatically and cannot be imported",
      );
    }
    return true;
  }),

  body("data.*.lastSyncedAt").custom((value) => {
    if (value !== undefined) {
      throw new Error(
        "lastSyncedAt is managed automatically and cannot be imported",
      );
    }
    return true;
  }),
...
export const validateCollectionExport: ValidationChain[] = [
  ...
  body("filters.isActive")
    .optional()
    .isBoolean()
    .withMessage("filters.isActive must be a boolean"),

  // Filter by category
  body("filters.category")
    .optional()
    .isString()
    .withMessage("filters.category must be a string")
    .trim(),
  ...
  body("filters.minEventCount")
    .optional()
    .isInt({ min: 0 })
    .withMessage("filters.minEventCount must be a non-negative integer"),

  body("filters.maxEventCount")
    .optional()
    .isInt({ min: 0 })
    .withMessage("filters.maxEventCount must be a non-negative integer"),

  // Date range filters
  body("filters.createdAfter")
    .optional()
    .isISO8601()
    .withMessage("filters.createdAfter must be a valid ISO 8601 date")
    .toDate(),

  body("filters.createdBefore")
    .optional()
    .isISO8601()
    .withMessage("filters.createdBefore must be a valid ISO 8601 date")
    .toDate(),

  body("filters.updatedAfter")
    .optional()
    .isISO8601()
    .withMessage("filters.updatedAfter must be a valid ISO 8601 date")
    .toDate(),

  body("filters.updatedBefore")
    .optional()
    .isISO8601()
    .withMessage("filters.updatedBefore must be a valid ISO 8601 date")
    .toDate(),
  ...
  body("filters.search")
    .optional()
    .isString()
    .withMessage("filters.search must be a string")
    .trim()
    .isLength({ min: 2 })
  ...
  body("includeEventsData")
    .optional()
    .isBoolean()
    .withMessage("includeEventsData must be a boolean"),
  ...
  body("sortBy")
    ...
    .withMessage(
      "sortBy must be one of: title, createdAt, updatedAt, sortOrder, slug",
    ),
