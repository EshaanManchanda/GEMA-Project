...
export const validateParticipants = [
  ...
  body("items.*.participants.*.emergencyContact.name")
    .if((value, { req, path }) => {
      ...
      if (match) {
        ...
        const age =
          req.body.items?.[itemIndex]?.participants?.[participantIndex]?.age;
  ...
  body("items.*.participants.*.emergencyContact.relationship")
    ...
    .notEmpty()
    .withMessage(
      "Emergency contact relationship is required for participants under 18",
    )
    .trim()
  ...
  body("items.*.participants.*.emergencyContact.phone")
    ...
    .notEmpty()
    .withMessage(
      "Emergency contact phone is required for participants under 18",
    )
    .trim()
    .matches(/^\+[1-9]\d{1,14}$/)
