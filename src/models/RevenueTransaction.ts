...
RevenueTransactionSchema.virtual("adminRevenue").get(function (
  this: IRevenueTransaction,
) {
...
RevenueTransactionSchema.pre("save", function (next) {
  ...
  if (Math.abs(sum - this.totalAmount) > tolerance) {
    return next(
      new Error("Admin commission + vendor payout must equal total amount"),
    );
  }
  next();
});

const RevenueTransaction = mongoose.model<IRevenueTransaction>(
  "RevenueTransaction",
