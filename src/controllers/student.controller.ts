...
export const bulkImportStudents = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // userCache: contactEmail → User doc (avoids repeated DB hits)
    const userCache: Record<string, any> = {};
    // track generated student emails within this import to avoid self-collisions
    ...
    for (let i = 0; i < dataLines.length; i++) {
      ...
      // Split "FirstName Lastname" — everything before last space is first name
      const parts = studentName.split(/\s+/);
      const firstName = parts[0];
      const lastName = parts.length > 1 ? parts.slice(1).join(" ") : "Student";

      // ── Find or create the parent/contact User ──
      if (!userCache[contactEmail]) {
        ...
        if (!user) {
          ...
          // Send welcome email (non-blocking — don't fail import if email fails)
          emailService.sendEmail({
            ...
            html: `
              <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:20px">
                ...
                <p>A student account has been created for you as part of the bulk student import.</p>
      ...
      // Extra optional fields as medicalNotes (JSON) — only store non-empty non-mandatory values
