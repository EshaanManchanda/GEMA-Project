...
const hasConfirmedEventBooking = async (
  ...
  return !!(booking || order || registration);
};
...
export const createReview = async (
  ...
  try {
    ...
    // Event reviews: skip booking gate when submitted via a valid review link token
    if (type === ReviewType.EVENT) {
      const tokenBypass = await validateReviewToken(targetId, reviewToken);
      const hasBooked = tokenBypass || (await hasConfirmedEventBooking(userId, targetId));
    ...
    if (type === ReviewType.EVENT) {
      await User.findByIdAndUpdate(userId, { $addToSet: { hasReviewedEvents: targetId } });
...
export const getReviewLink = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { eventId } = req.params;
    const { email, accessToken } = req.query as { email?: string; accessToken?: string };

    if (!email) return next(new AppError("email query param required", 400));

    const event = await Event.findById(eventId).select("title reviewToken");
    if (!event) return next(new AppError("Event not found", 404));

    if (event.reviewToken && event.reviewToken !== accessToken) {
      return next(new AppError("Invalid or expired review link", 403));
    }
    ...
    if (existing) {
      return res.status(200).json({
        success: true,
        data: { alreadyReviewed: true, eventTitle: (event as any).title },
      });
    }

    const token = jwt.sign(
      { userId: user._id, eventId, purpose: "review" },
    ...
    res.status(200).json({
      success: true,
      data: { alreadyReviewed: false, token, userId: user._id, eventId, eventTitle: (event as any).title },
    });
  } catch (error) {
    next(error);
  }
...
export const generateReviewLink = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const reviewLink = `${baseUrl}/review/${eventId}?token=${token}`;
    await Event.findByIdAndUpdate(eventId, { reviewLink, reviewToken: token });
...
async function validateReviewToken(eventId: string, token?: string): Promise<boolean> {
  if (!token) return false;
  try {
    const payload = jwt.verify(token, process.env.JWT_SECRET as string) as any;
    return payload?.purpose === "review" && payload?.eventId === eventId;
  } catch {
    return false;
  }
}
...
async function triggerCertificateForReview(review: any, userId: any): Promise<void> {
  ...
  const certificate = await Certificate.create({
    ...
    userId,
    reviewId: review._id,
    recipient: { name: `${user.firstName} ${user.lastName}`.trim(), email: user.email },
    data: { eventName: (event as any).title, rating: review.rating, date: review.createdAt },
    status: "pending",
