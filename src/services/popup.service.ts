...
class PopupService {
  ...
  async getActivePopups(
    ...
    let filtered = popups.filter((popup) => {
      ...
      if (
        popup.targetAudience === "authenticated" &&
      ...
      if (
        userContext.isAuthenticated &&
        popup.targetRoles &&
        popup.targetRoles.length > 0
      ) {
        if (
          !userContext.role ||
          !popup.targetRoles.includes(userContext.role as any)
        ) {
          return false;
        }
      }
    ...
    return filtered as unknown as IPopupNotification[];
  }
   ...
   */
  async createPopup(
    data: Partial<IPopupNotification>,
    userId: string,
  ): Promise<IPopupNotification> {
  ...
  async updatePopup(
    id: string,
    data: Partial<IPopupNotification>,
  ): Promise<IPopupNotification | null> {
