...
const migrateEventFields = async () => {
  try {
    ...
    if (eventsWithoutFields === 0) {
      console.log(
        "✅ All events already have the required fields. No migration needed.",
      );
      return;
    }
    ...
    console.log(
      `✅ Updated ${approvedEventsUpdate.modifiedCount} approved events to active/published`,
    );
    ...
    console.log(
      `✅ Updated ${nonApprovedEventsUpdate.modifiedCount} non-approved events to inactive/pending`,
    );
    ...
    console.log(
      `✅ Updated ${legacyEventsUpdate.modifiedCount} legacy events (no isApproved field)`,
    );
