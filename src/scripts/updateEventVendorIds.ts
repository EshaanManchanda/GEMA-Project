  ...
  try {
    ...
    for (const event of events) {
      try {
        ...
        if (!user) {
          console.log(
            `⚠️  Event ${event._id} (${event.title}): User not found for vendorId ${oldVendorId}`,
          );
        ...
        if (user.role !== "vendor") {
          console.log(
            `⚠️  Event ${event._id} (${event.title}): User ${oldVendorId} is not a vendor (role: ${user.role})`,
          );
          result.skippedEvents++;
          continue;
        }
        ...
        if (!vendor) {
          console.log(
            `❌ Event ${event._id} (${event.title}): No Vendor profile found for userId ${oldVendorId}`,
          );
          result.errors.push({
        ...
        if (event.vendorId.toString() !== vendor._id.toString()) {
          ...
          console.log(
            `✅ Event ${event._id} (${event.title}): Updated vendorId from ${oldVendorId} to ${vendor._id}`,
          );
          result.updatedEvents++;
        } else {
          console.log(
            `ℹ️  Event ${event._id} (${event.title}): Already using Vendor._id, skipping`,
          );
          result.skippedEvents++;
        }
      } catch (error: any) {
