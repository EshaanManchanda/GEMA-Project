...
const CouponForm: React.FC<CouponFormProps> = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      applicationType: (coupon?.applicableEvents?.length ? 'specific_events' : coupon?.applicableCategories?.length ? 'specific_categories' : 'all') as CouponApplicationType,
      isActive: coupon?.isActive ?? true,
      validFrom: coupon?.validFrom ? new Date(coupon.validFrom).toISOString().slice(0, 16) : '',
      validUntil: coupon?.validUntil ? new Date(coupon.validUntil).toISOString().slice(0, 16) : '',
      usageLimit: coupon?.usageLimit,
      minimumOrderAmount: coupon?.minimumAmount || 0,
      maximumDiscountAmount: coupon?.maximumDiscount,
      ...
      isFirstTimeUser: coupon?.firstTimeOnly || false,
  ...
  const loadCategories = async () => {
    try {
      ...
      setCategories((response as any).data || response);
    } catch (error) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const couponData: any = {
        ...
        minimumAmount: data.minimumOrderAmount || 0,
        maximumDiscount: data.maximumDiscountAmount,
        ...
        firstTimeOnly: data.isFirstTimeUser,
      };
