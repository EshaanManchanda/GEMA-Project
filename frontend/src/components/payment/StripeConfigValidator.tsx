...
const StripeConfigValidator: React.FC<StripeConfigValidatorProps> = ({
  ...
  const [, setStripeInitialized] = useState<boolean | null>(null);
  ...
  const { config, keyValidation, environmentWarning, stripeInitialized: stripeInitResult, overallValid } = validationResults;
  const hasWarnings = keyValidation.warnings.length > 0 || Boolean(environmentWarning);

  return (
    <Card className={`${overallValid
      ...
      <CardContent className="space-y-4">
        {/* Summary */}
        <div className={`p-3 rounded-lg ${overallValid
        ...
        {showDetails && (
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              {getStatusIcon(stripeInitResult === true)}
            </div>
        ...
        {keyValidation.errors.length > 0 && (
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
            ...
            <ul className="text-sm text-red-700 list-disc list-inside">
              {keyValidation.errors.map((error: string, index: number) => (
        ...
        {(keyValidation.warnings.length > 0 || environmentWarning) && (
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            ...
            <ul className="text-sm text-yellow-700 list-disc list-inside">
              {keyValidation.warnings.map((warning: string, index: number) => (
        ...
        <div className="flex gap-2">
          ...
          {!overallValid && (
            <Button
              ...
              variant="outline"
              onClick={() => {
                logger.debug('Stripe Configuration Details', validationResults);
              }}
            >
              Debug Info
            </Button>
