...
const getAnimationVariants = (type: AnimationType, duration: number, _delay: number) => {
...
const CollectionSection: React.FC<CollectionSectionProps> = ({
  ...
  cardHoverEffect: _cardHoverEffect,
  gridCols,
  ...
  if (import.meta.env.DEV) {
    if (!Array.isArray(events)) {
      logger.error('CollectionSection: events prop must be an array, received:', typeof events);
    }
    ...
    if (enablePagination && layout !== 'grid') {
      logger.warn('CollectionSection: enablePagination only works with grid layout');
    }

    if (autoplay && layout !== 'carousel') {
      logger.warn('CollectionSection: autoplay only works with carousel layout');
    }

    if (showNavigation && layout !== 'carousel') {
      logger.warn('CollectionSection: showNavigation only applies to carousel layout');
    }

    if (showDots && layout !== 'carousel') {
      logger.warn('CollectionSection: showDots only applies to carousel layout');
    }
  }
