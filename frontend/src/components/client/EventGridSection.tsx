...
const EventGridSection: React.FC<EventGridSectionProps> = ({ events = [] }) => {
  ...
  // Debug logging (gated)
  // Debug logging
  logger.debug('📅 [EVENT GRID SECTION] Rendering:');
  logger.debug('   - Events received:', events?.length || 0);
  logger.debug('   - Safe events:', safeEvents.length);
  logger.debug('   - Transformed events:', transformedEvents.length);
  logger.debug('   - Sorted events:', sortedEvents.length);
  if (sortedEvents.length > 0) {
    logger.debug('   - First event:', {
      title: sortedEvents[0].title,
      viewsCount: sortedEvents[0].viewsCount,
      rating: (sortedEvents[0] as any).rating,
