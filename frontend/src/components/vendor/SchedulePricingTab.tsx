...
const getScheduleSectionLabel = (type?: string) => {
  if (type === 'Venue') return 'Operating Periods';
  if (type === 'Course') return 'Session Schedule & Pricing';
  if (type === 'Workshop') return 'Workshop Schedule & Pricing';
  if (type && COMPETITION_TYPES.includes(type)) return 'Event Dates & Pricing';
  return 'Event Schedules & Pricing';
};

const getScheduleItemLabel = (type?: string) => {
  if (type === 'Venue') return 'Period';
  if (type === 'Course') return 'Session';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          <label htmlFor="capacity" className="block text-sm font-medium text-gray-700 mb-1">
            ...
            {getCapacityLabel(eventType)} <span className="text-red-500">*</span>
          </label>
          <input
            type="number"
            id="capacity"
      ...
      {onTimezoneChange && (
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">All session start/end times are interpreted in this timezone.</p>
        </div>
      )}
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">
              ...
              {getScheduleSectionLabel(eventType)}
            </h3>
            <p className="text-sm text-gray-600 mt-1">
              {eventType === 'Venue'
                ? 'Add operating periods with admission pricing. Leave blank if the venue has no fixed schedule.'
                : eventType === 'Course'
                ? 'Add session dates with per-session pricing and seat availability.'
                : 'Add date ranges with individual pricing for each schedule.'}
            </p>
          </div>
                    ...
                    <h4 className="text-sm font-medium text-gray-900">
                      {schedule.isSpecialDate ? (
                        <>
                          ...
                          Special Date {getScheduleItemLabel(eventType)} #{index + 1}
                        </>
                      ) : (
                        <>{getScheduleItemLabel(eventType)} #{index + 1}</>
                      )}
                    </h4>
                    {schedule.isSpecialDate && (
