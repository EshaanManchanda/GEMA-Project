...
const RegistrationFormField: React.FC<RegistrationFormFieldProps> = ({
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      onChange(url);
      // Signal to parent that we've uploaded (no raw file needed)
  ...
  const renderField = () => {
    switch (field.type) {
      ...
      case 'website':
        return (
          <input
            type={field.type === 'website' ? 'url' : (field.type as any) === 'number' ? 'number' : field.type}
            id={field.id}
            ...
            {hasFile ? (
              <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
                ...
                <a
                  ...
                  className="text-sm text-blue-700 hover:underline truncate flex-1 font-medium"
                >
              ...
              /* Uploading state */
              <div className="flex items-center gap-3 p-4 border-2 border-indigo-200 bg-indigo-50 rounded-xl">
                <Loader2 className="w-5 h-5 text-indigo-600 animate-spin shrink-0" />
                <span className="text-sm text-indigo-700">Uploading file…</span>
              </div>
            ) : (
              /* Pick file state */
              <label
              ...
              >
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  <FileUp className={`w-10 h-10 mb-3 ${(error || uploadError) ? 'text-red-400' : 'text-gray-400'}`} />
