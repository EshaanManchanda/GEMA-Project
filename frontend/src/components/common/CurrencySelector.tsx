              ...
              <div className="py-1 max-h-60 overflow-y-auto">
                {supportedCurrencies.map((currency) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 text-sm hover:bg-gray-100 transition-colors ${currentCurrency === currency.code ? 'bg-blue-50 text-blue-600' : ''
                      }`}
                  >
          ...
          <div className="space-y-1 max-h-60 overflow-y-auto">
            {supportedCurrencies.map((currency) => (
              <button
                key={currency.code}
                onClick={() => handleCurrencySelect(currency.code)}
                className={`w-full flex items-center justify-between p-3 rounded-md hover:bg-gray-100 transition-colors ${currentCurrency === currency.code ? 'bg-blue-50 ring-2 ring-blue-500' : ''
                  }`}
              >
                <div className="flex items-center space-x-3">
