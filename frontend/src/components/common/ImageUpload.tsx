...
const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  maxSize = 5,
  maxFiles = 10,
  acceptedTypes = ['image/jpeg', 'image/png', 'image/jpg', 'image/webp', 'image/avif'],
  label = 'Event Images',
  required = false
}) => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const validFiles = filesArray.filter(file => {
        ...
        if (!isValidType) {
          alert(`File ${file.name} is not a valid image type. Only JPG, JPEG, PNG, WEBP, and AVIF files are allowed.`);
        } else if (!isValidSize) {
      ...
      if (validFiles.length > 0) {
        try {
          ...
          if (response.success && response.data) {
            ...
            logger.debug('Uploaded URLs:', uploadedUrls);
          ...
          {isUploading ? (
            ...
            <>
              ...
              <p className="text-xs text-gray-500">
                PNG, JPG, JPEG, WEBP, AVIF up to {maxSize}MB (max {maxFiles} images)
              </p>
            </>
          )}
        </div>
