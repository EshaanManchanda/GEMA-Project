...
const PopupForm: React.FC<PopupFormProps> = ({ popup, onCancel, onSubmit }) => {
  ...
  const watchedTargetPages = watch('targetPages') as any;
  const watchedTrigger = watch('trigger');
  const watchedTargetAudience = watch('targetAudience') as any;
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
          ...
          <CardContent className="space-y-4">
                ...
                render={({ field }) => (
                  <div className="flex gap-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="radio"
                        checked={(field.value as any) === 'specific'}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Exclude Pages (comma-separated, optional)
              </label>
              <input
                type="text"
                ...
                className="w-full px-4 py-2 border border-gray-300 rounded-lg"
              />
              <p className="text-xs text-gray-500 mt-1">Never show on these pages, even if matched above</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Overlay Opacity: {watch('overlayOpacity')}%
              </label>
              <Controller
                name="overlayOpacity"
                control={control}
