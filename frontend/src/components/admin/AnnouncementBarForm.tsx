...
const AnnouncementBarForm: React.FC<AnnouncementBarFormProps> = ({ announcement, onCancel, onSubmit }) => {
  ...
  const handleFormSubmit = async (data: any) => {
    try {
      console.log('[AnnouncementBarForm] Submitting data:', {
        status: data.status,
        isActive: data.isActive,
        fullData: data
      });
      console.log('[AnnouncementBarForm] Watched status value:', watchedStatus);
      await onSubmit(data);
    } catch (error: any) {
  ...
  useEffect(() => {
    console.log('[AnnouncementBarForm] Status changed to:', watchedStatus);
  }, [watchedStatus]);
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  value={watchedStatus}
                  onChange={(e) => {
                    console.log('[Status Field] Direct onChange, new value:', e.target.value);
