...
const UserManagement: React.FC<UserManagementProps> = ({
  ...
  const handleEditUser = (user: User) => {
    ...
    logger.info('Edit user requested', { userId: user._id });
  };

  const handleDeleteUser = (user: User) => {
    if (window.confirm(`Are you sure you want to delete ${user.firstName} ${user.lastName}?`)) {
      // In real app, dispatch delete action
      logger.info('Delete user requested', { userId: user._id });
    }
  };
  ...
  const executeBulkAction = () => {
    if (bulkAction && selectedUsers.size > 0) {
      logger.info('Executing bulk action', { type: bulkAction.type, usersCount: selectedUsers.size, users: Array.from(selectedUsers) });
  ...
  const handleSelectAll = () => {
    if (selectedUsers.size === filteredUsers.length) {
      ...
      setSelectedUsers(new Set(filteredUsers.map(user => user._id).filter((id): id is string => !!id)));
    }
  };
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-3 mt-2">
                  ...
                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(selectedUser.status || '')}`}>
                    {(selectedUser.status || 'unknown').charAt(0).toUpperCase() + (selectedUser.status || 'unknown').slice(1)}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="p-6">
          ...
          <div className="flex justify-end space-x-3">
            ...
            <button
              ...
              className={`px-4 py-2 text-white rounded-md hover:opacity-90 ${bulkAction?.type === 'delete' ? 'bg-red-600' : 'bg-blue-600'
                }`}
            >
              {bulkAction?.label}
            </button>
