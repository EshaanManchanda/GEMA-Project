...
const EventModeration: React.FC<EventModerationProps> = ({
  ...
  const executeModerationAction = () => {
    if (moderationAction && selectedEvents.size > 0) {
      logger.info('Executing moderation action', { type: moderationAction.type, eventsCount: selectedEvents.size, events: Array.from(selectedEvents), comment: moderationComment });
      ...
      render: (event: Event) => (
        <div className="text-sm">
          ...
          <div className="flex items-center space-x-1 text-gray-500 mt-1">
            ...
            <span>
              {format(parseISO((event.dateSchedule[0].startDateTime || event.dateSchedule[0].startDate || event.dateSchedule[0].date) ?? ''), 'MMM dd, yyyy')}
            </span>
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className={`p-2 rounded-md transition-colors ${showFilters ? 'bg-blue-100 text-blue-600' : 'text-gray-400 hover:text-gray-600'
                }`}
            >
              <FaFilter size={16} />
            </button>
              ...
              <div>
                ...
                <dl className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <dd className="text-sm font-medium text-gray-900">
                      {format(parseISO((selectedEvent.dateSchedule[0].startDateTime || selectedEvent.dateSchedule[0].startDate || selectedEvent.dateSchedule[0].date) ?? ''), 'MMM dd, yyyy HH:mm')}
                    </dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-gray-500">End Date:</dt>
                    <dd className="text-sm font-medium text-gray-900">
                      {format(parseISO((selectedEvent.dateSchedule[0].endDateTime || selectedEvent.dateSchedule[0].endDate || selectedEvent.dateSchedule[0].date) ?? ''), 'MMM dd, yyyy HH:mm')}
                    </dd>
                  </div>
                  <div className="flex justify-between">
        ...
        <div className="p-6">
          ...
          <div className="flex justify-end space-x-3">
            ...
            <button
              ...
              className={`px-4 py-2 text-white rounded-md hover:opacity-90 ${moderationAction?.type === 'delete' || moderationAction?.type === 'reject'
                  ? 'bg-red-600'
