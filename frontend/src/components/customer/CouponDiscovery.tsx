...
const CouponDiscovery: React.FC<CouponDiscoveryProps> = ({
  ...
  const { activeCoupons, loading } = useSelector((state: RootState) => state.coupons);
  ...
  const featuredCoupons = filteredCoupons.filter(coupon => (coupon as any).featured || coupon.value >= 20);
  const regularCoupons = filteredCoupons.filter(coupon => !(coupon as any).featured && coupon.value < 20);
