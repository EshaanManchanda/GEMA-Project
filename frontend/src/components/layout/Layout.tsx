...
const Layout: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
  ...
  const getUserAvatar = () => {
    if (user?.avatar) {
      ...
      if (apiOrigin && user.avatar.startsWith('/')) {
        return `${apiOrigin}${user.avatar}`;
      }
      return user.avatar;
    }
                    ...
                    <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-50">
                      ...
                      <div className="px-4 py-3 border-b border-gray-200">
                        ...
                        <span className="inline-block px-2 py-0.5 text-xs rounded-full mt-1 capitalize"
                          ...
                          {user.role === 'customer' ? 'User' : user.role}
                        </span>
                      </div>
                      ...
                      {/* My Account links — visible to all authenticated users */}
                      {user && (
                        <>
                          <div className="px-4 pt-3 pb-1">
