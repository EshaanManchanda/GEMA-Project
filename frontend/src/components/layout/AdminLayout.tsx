...
const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-30 h-full flex flex-col bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white transition-all duration-300 ${
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
          {navigationItems.map((item) => (
            <Link
              ...
              to={item.path}
              onClick={() => setIsMobileSidebarOpen(false)}
              className={`flex items-center ${isSidebarOpen ? 'px-4' : 'px-3 justify-center'} py-3 rounded-lg transition-all duration-200 group ${
                isActive(item.path)
