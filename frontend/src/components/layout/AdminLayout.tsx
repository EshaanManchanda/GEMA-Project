...
const AdminLayout: React.FC = () => {
  ...
  useEffect(() => {
    console.log('AdminLayout Mounted', new Date().toISOString());
  }, []);
  ...
  const navigationItems = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 21h18M5 21V7l7-4 7 4v14M9 9h.01M9 12h.01M9 15h.01M15 9h.01M15 12h.01M15 15h.01" />
        </svg>
      )
    },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-30 h-full flex flex-col bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-20'
          } ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 shadow-2xl`}
      >
        {/* Logo Area */}
        <div className="flex items-center justify-between h-16 px-4 border-b border-gray-700">
          {isSidebarOpen && (
            <Link to="/admin" className="flex items-center space-x-3">
              <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center font-bold text-white shadow-lg">
                G
              </div>
              <span className="text-xl font-bold bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">
                {getBrandConfig().appName} Admin
              </span>
            </Link>
          )}
          <button
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
          {navigationItems.map((item) => (
            <Link
              ...
              to={item.path}
              onClick={() => setIsMobileSidebarOpen(false)}
              className={`flex items-center ${isSidebarOpen ? 'px-4' : 'px-3 justify-center'} py-3 rounded-lg transition-all duration-200 group ${isActive(item.path)
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4 lg:ml-0">
                <h1 className="text-xl font-semibold text-gray-800">
                  {navigationItems.find(item => isActive(item.path))?.name || 'Admin Panel'}
                </h1>
              </div>
            </div>
