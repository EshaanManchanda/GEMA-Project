...
import {
  ...
  markAllAsRead,
  deleteNotification,
  bulkAction as bulkDeleteNotifications,
  selectNotifications,
  selectUnreadCount,
...
const NotificationCenter: React.FC<NotificationCenterProps> = ({
  ...
  const handleBulkDelete = async () => {
    if (selectedNotifications.size > 0) {
      await dispatch(bulkDeleteNotifications({ notificationIds: Array.from(selectedNotifications), action: 'delete' }));
  ...
  const getTypeColor = (type: NotificationType) => {
    if (type.startsWith('booking')) return 'text-blue-600 bg-blue-100';
    if (type.startsWith('payment') || type === 'payout_processed') return 'text-green-600 bg-green-100';
    if (type.startsWith('event') || type.startsWith('vendor')) return 'text-purple-600 bg-purple-100';
    if (type === 'system_maintenance') return 'text-gray-600 bg-gray-100';
    if (type === 'promotional' || type === 'review_received') return 'text-orange-600 bg-orange-100';
