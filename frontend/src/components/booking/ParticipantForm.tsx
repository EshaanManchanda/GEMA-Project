...
const ParticipantForm: React.FC<ParticipantFormProps> = ({
  ...
  const validateParticipant = (participant: BookingParticipant, index: number): boolean => {
    ...
    } else if (participant.age < event.ageRange[0] || participant.age > event.ageRange[1]) {
  ...
  const handleDynamicDataChange = useCallback((participantIndex: number, data: Record<string, any>) => {
    ...
    if (event.registrationConfig?.fields) {
      const registrationDataArray = event.registrationConfig.fields.map(field => {
        let value = data[field.id];

        // Replace File objects with serializable metadata
        if (value instanceof File) {
          value = { _isFile: true, name: value.name, size: value.size, type: value.type };
        } else if (Array.isArray(value) && value.some(v => v instanceof File)) {
          value = value.map(v => v instanceof File
