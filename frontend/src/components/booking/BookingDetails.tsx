...
import { getTimezoneAbbr, formatHHmm } from "../../utils/timezoneUtils";
import {
  Calendar,
  Clock,
  MapPin,
...
const BookingDetails: React.FC<BookingDetailsProps> = ({
  ...
  const [selectedSchedule, setSelectedSchedule] =
    useState<EventDateSchedule | null>(() => {
      // If scheduleId is provided but schedule object is not, find the schedule from event.dateSchedule
      if (
        ...
        return (
          event.dateSchedule.find(
            (s) =>
              s._id === initialData.scheduleId ||
              s.id === initialData.scheduleId,
          ) || null
        );
      }
      return initialData?.schedule || null;
    });
  ...
  // Initialize schedule from initialData if available (supports both schedule object and scheduleId)
  useEffect(() => {
    // Handle schedule object (from some sources)
    if (
      initialData?.schedule &&
      (initialData.schedule._id || initialData.schedule.id)
    ) {
    ...
    // Handle scheduleId directly (from EventDetailPage via route state)
    else if (initialData?.scheduleId) {
      dispatch(setBookingSchedule(initialData.scheduleId));
    }
  ...
  useEffect(() => {
    // Only auto-select if:
    // 1. Event has exactly one schedule
    // 2. No schedule is currently selected in bookingFlow
    // 3. No initial data schedule was provided (to avoid overriding user navigation)
    // 4. No scheduleId in initialData (EventDetailPage passes scheduleId, not schedule object)
    if (
      event.dateSchedule?.length === 1 &&
      !bookingFlow.scheduleId &&
      !initialData?.schedule &&
      !initialData?.scheduleId
    ) {
      const firstSchedule = event.dateSchedule[0];
      const scheduleId = firstSchedule._id || firstSchedule.id;

      if (scheduleId) {
        logger.debug("Auto-selecting single schedule", {
          scheduleId,
          eventId: event._id,
        ...
        // Try to parse the date for selectedDate (supports both regular and teacher event field names)
        const rawDateValue =
        ...
        if (rawDateValue) {
          try {
            ...
            logger.warn(
              "Error parsing schedule date for auto-selection",
              error,
            );
          }
        }
        ...
        dispatch(
          setBookingDateSelection({
            scheduleId,
            selectedDate: datePart || format(new Date(), "yyyy-MM-dd"),
            scheduleSnapshot: {
              _id: scheduleId,
              startDate: (firstSchedule as any).startDate,
              endDate: (firstSchedule as any).endDate,
              startTime: (firstSchedule as any).startTime,
              endTime: (firstSchedule as any).endTime,
              price: firstSchedule.price,
              unlimitedSeats: firstSchedule.unlimitedSeats,
            },
            unitPrice,
          }),
        );
        toast.success("Schedule auto-selected", { duration: 2000 });
      }
    }
  }, [
    event.dateSchedule,
  ...
  const handleDateSelect = (date: Date | null) => {
    ...
    // Find the corresponding schedule — check both legacy `date`/`startDateTime` and teacher event `startDate` fields
    const target = startOfDay(date);
    const schedule = event.dateSchedule.find((s) => {
      // Teacher events: startDate..endDate range
      if (s.startDate) {
        try {
          const start = startOfDay(new Date(s.startDate));
          const end = s.endDate ? startOfDay(new Date(s.endDate)) : start;
          return !isBefore(target, start) && !isAfter(target, end);
        } catch {
          return false;
        }
      }
      // Regular events: date or startDateTime single-day match
      const dateValue = s.date || s.startDateTime;
      if (!dateValue) return false;
      try {
        const scheduleDate = new Date(dateValue);
        if (!isValid(scheduleDate)) return false;
        return scheduleDate.toDateString() === date.toDateString();
      } catch (error) {
        logger.warn("Error parsing schedule date", error);
        return false;
      }
    });
    ...
    if (schedule) {
      ...
      dispatch(
        setBookingDateSelection({
          ...
          scheduleSnapshot: {
            _id: scheduleId,
            startDate: (schedule as any).startDate,
            endDate: (schedule as any).endDate,
            startTime: (schedule as any).startTime,
            endTime: (schedule as any).endTime,
            price: schedule.price,
            unlimitedSeats: schedule.unlimitedSeats,
          },
          unitPrice,
        }),
      );
  ...
  const handleQuantityChange = (newQuantity: number) => {
    ...
    if (selectedSchedule?.unlimitedSeats) {
      // Unlimited events: cap at a reasonable UI max (100) but no seat-count error
      if (newQuantity > 100) return;
    } else {
      const maxSeats = selectedSchedule?.availableSeats ?? 10;
      if (newQuantity > maxSeats) {
        toast.error(
          `Only ${maxSeats} seat${maxSeats !== 1 ? "s" : ""} available for this date`,
        );
        return;
      }
      // Safety cap: no more than 50 per booking for limited events
      if (newQuantity > 50) {
        toast.error("Maximum 50 tickets per booking");
        return;
      }
    }
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      logger.debug("[Coupon Validation] Full response", response);
      logger.debug("[Coupon Validation] response.success", {
        success: response?.success,
        type: typeof response?.success,
      });
      logger.debug("[Coupon Validation] response.data", response?.data);
      logger.debug("[Coupon Validation] response.data.isValid", {
        isValid: response?.data?.isValid,
        type: typeof response?.data?.isValid,
      });
      ...
      if (
        ...
        dispatch(setCouponDiscount(validationData.discountAmount)); // persist discount so confirmation page shows correct total
        setCouponError(null);
        ...
        logger.error("[Coupon Validation] Failed - response", response);
      ...
      if (error.response?.data?.message) {
        ...
        if (
          ...
          errorMessage =
            "You have already used this coupon or reached the usage limit.";
        } else if (backendMessage.includes("not applicable to this order")) {
          errorMessage =
            "This coupon is not applicable to this event or does not meet minimum requirements.";
  ...
  const handleNext = () => {
    ...
    if (!bookingFlow.scheduleId) {
      toast.error("Please select a schedule to continue");
      return;
    }

    if (!canProceed) {
      toast.error(
        "Please select a date and specify the number of participants",
      );
      return;
    }
    ...
    if (
      ...
      quantity > selectedSchedule.availableSeats
    ) {
      toast.error(
        `Only ${selectedSchedule.availableSeats} seats available for this date`,
      );
      return;
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative h-48 overflow-hidden bg-gray-100">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Badge
                variant="primary"
                className="bg-gray-900/60 backdrop-blur-sm text-white border-white/30"
              >
                {event.category}
              </Badge>
              <Badge
            ...
            <div className="flex items-start space-x-3 p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm font-medium text-gray-900 truncate">
                  {event.location?.city || "Online / Virtual"}
                </div>
                <div className="text-xs text-gray-600 truncate">
                  {event.location?.address || "See event details"}
                </div>
              </div>
            </div>
              ...
              <div className="flex-1">
                ...
                {Array.isArray(event.ageRange) && 
                 ...
                 event.ageRange[1] !== null ? (
                  ...
                  <div className="text-sm text-gray-500">Open to all ages</div>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-center space-x-6 mb-6">
              ...
              <button
                ...
                disabled={
                  quantity >= 10 ||
                  (!selectedSchedule?.unlimitedSeats &&
                    quantity >= (selectedSchedule?.availableSeats || 10))
                }
                className={`
            ...
            <div className="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 rounded-xl p-4 border border-indigo-200">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-lg font-bold text-indigo-600">
                  {selectedSchedule?.unlimitedSeats
                    ? "∞ Unlimited"
                    : `${selectedSchedule?.availableSeats || 10} seats`}
                </span>
              </div>
              ...
              <div className="mt-3 bg-white/50 rounded-full h-2 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(quantity / (selectedSchedule?.availableSeats || 10)) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
      ...
      {/* Coupon Code - Enhanced UI (hidden for free events) */}
                ...
                <div className="relative">
                  <div className="flex space-x-2">
                    <div className="relative flex-1">
                      ...
                      />
                      <Tag
                        className={`absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 ${couponError ? "text-red-400" : "text-gray-400"}`}
                      />
                    </div>
                    <Button
                      variant="primary"
          ...
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
            <div>
              ...
              <div className="text-xs text-gray-500">
                ...
                {selectedSchedule?.price || event.price}
              </div>
            </div>
            <div className="text-lg font-bold text-gray-900">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-green-600 font-medium">
                      ...
                      {validatedCoupon.coupon.type === "percentage" &&
                        ` (${validatedCoupon.coupon.value}% off)`}
                      {validatedCoupon.coupon.type === "fixed_amount" &&
                        " (Fixed amount)"}
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            >
              ...
              <div className="relative flex items-center space-x-2">
                ...
                <ChevronRight
                  className={`w-6 h-6 transition-transform ${canProceed ? "group-hover:translate-x-1" : ""}`}
                />
              </div>
            </button>
          </div>
