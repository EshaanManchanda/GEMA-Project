...
const BookingDetails: React.FC<BookingDetailsProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (scheduleId) {
        ...
        // Try to parse the date for selectedDate
        if (firstSchedule.date || firstSchedule.startDateTime) {
          try {
            const dateValue = firstSchedule.startDateTime || firstSchedule.date;
            const parsedDate = new Date(dateValue);
            if (isValid(parsedDate)) {
              setSelectedDate(parsedDate);
            }
          } catch (error) {
            logger.warn('Error parsing schedule date for auto-selection', error);
          }
        }

        // Set in Redux for payment processing
        dispatch(setBookingSchedule(scheduleId));
        toast.success('Schedule auto-selected', { duration: 2000 });
      }
    }
  }, [event.dateSchedule, bookingFlow.scheduleId, initialData?.schedule, event._id, event.title, dispatch]);
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      logger.debug('[Coupon Validation] response.success', { success: response?.success, type: typeof response?.success });
      logger.debug('[Coupon Validation] response.data', response?.data);
      logger.debug('[Coupon Validation] response.data.isValid', { isValid: response?.data?.isValid, type: typeof response?.data?.isValid });
