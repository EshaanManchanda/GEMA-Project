    ...
    try {
      ...
      dispatch(setCurrentBooking({
        ...
        payment: {
          method: 'card',
          status: 'completed',
          amount: confirmResponse?.amountPaid ?? 0,
          currency: event.currency || getDefaultCurrency(),
    ...
    try {
      if (selectedPaymentMethod === 'free') {
        ...
        if (!initiateResponse?.paymentIntentId || !initiateResponse?.orderId) {
          setPaymentError('Failed to initiate free booking. Please try again.');
          return;
        }
        ...
        toast.success('Free registration confirmed!');
      ...
      if (selectedPaymentMethod === 'stripe') {
        ...
        // Check if schedule ID exists first
        if (!bookingFlow.scheduleId) {
          setPaymentError('No schedule selected. Please go back and select a date/time for your booking.');
          return;
        }

        // Check if payment intent is ready
        if (!checkout?.clientSecret) {
          setPaymentError('Payment session is loading. Please wait a moment...');
          // Attempt to create payment intent if missing
          logger.debug('Retrying payment intent creation');
          dispatch(createPaymentIntent({
            eventId: event._id,
            participants: participants.length,
