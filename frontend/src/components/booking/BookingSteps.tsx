...
const BookingSteps: React.FC<BookingStepsProps> = ({
  ...
  // Free events skip the payment step
  const steps = isFreeEvent ? allSteps.filter(s => s.key !== 'payment').map((s, i) => ({ ...s, number: i + 1 })) : allSteps;
  ...
  return (
    ...
    <div className="flex items-center justify-between max-w-2xl mx-auto min-w-max sm:min-w-0">
      {steps.map((step, index) => {
        ...
        const isComplete =
          (isPast && stepValidated) ||
          (isCurrent && isFinalStep && stepValidated);
