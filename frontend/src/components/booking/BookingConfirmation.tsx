...
import { formatHHmm, getTimezoneAbbr } from '../../utils/timezoneUtils';
import {
  CheckCircle,
  Download,
  Mail,
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center">
                  ...
                  <span>
                    {(() => {
                      try {
                        // 1. Prefer the Redux snapshot (most reliable — set at selection time)
                        const snap = (bookingFlow as any).selectedScheduleSnapshot;
                        const selectedDateStr = (bookingFlow as any).selectedDate as string | undefined;

                        // Date label: use stored selectedDate (YYYY-MM-DD) if available, else fall back
                        let dateLabel = 'Date: TBD';
                        if (selectedDateStr) {
                          const [y, m, d] = selectedDateStr.split('-').map(Number);
                          const dateObj = new Date(y, m - 1, d);
                          if (!isNaN(dateObj.getTime())) dateLabel = format(dateObj, 'PPP');
                        } else {
                          // Fall back: find schedule in event data, extract date-only to avoid TZ offset
                          const schedule = (event.dateSchedule || []).find(
                            s => (s as any)._id === bookingFlow.scheduleId
                          );
                          const rawDate = schedule
                            ? ((schedule as any).startDate || (schedule as any).date || (schedule as any).startDateTime)
                            : null;
                          if (rawDate) {
                            const datePart = typeof rawDate === 'string' ? rawDate.split('T')[0] : '';
                        ...
                        // Time label: prefer snapshot, then live schedule
                        const st = snap?.startTime;
                        const et = snap?.endTime;
                        const tzAbbr = event.timezone ? ` ${getTimezoneAbbr(event.timezone)}` : '';
                        const timeLabel = st && et ? ` · ${formatHHmm(st)} – ${formatHHmm(et)}${tzAbbr}` : '';
