...
const CouponValidator: React.FC<CouponValidatorProps> = ({
  ...
  const { validationResult, loading, error } = useSelector((state: RootState) => state.coupons);
  const isValidating = loading.validating;
  const validationError = error.validating;
  ...
  useEffect(() => {
    if (validationResult && !appliedCoupon) {
      const discount = calculateDiscount(validationResult);
      setAppliedCoupon(validationResult);
      onCouponApplied?.(validationResult, discount);
      toast.success(`Coupon applied! You saved ${formatDiscount(validationResult, discount)}`);
    }
  }, [validationResult, appliedCoupon, onCouponApplied]);
