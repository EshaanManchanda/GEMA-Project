...
const BookingDetailPage: React.FC = () => {
  ...
  const id = event_name; // Keep internal variable name as 'id' for compatibility with existing code
  ...
  // Guard against concurrent fetches and StrictMode double-invocation
  ...
  const fetchBookingDetails = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      logger.debug('📡 Fetching booking details for ID:', id);
      const response = await bookingAPI.getBookingById(id);
      logger.debug('✅ Booking details fetched:', {
  ...
  }, [id, fetchBookingDetails, dispatch]);

  // Visibility refetch — only if the user was actually away (>30s since last fetch)
  useEffect(() => {
    const handleVisibilityChange = () => {
      const awayLongEnough = Date.now() - lastFetchTimeRef.current > 30_000;
      if (document.visibilityState === 'visible' && id && awayLongEnough) {
        logger.debug('Tab became visible, refetching booking details...');
        fetchBookingDetails();
      }
    };
  ...
  const handleDownloadTicket = (ticket: Ticket) => {
    ...
    downloadTicketAsPNG({
      ...
      qrCodeImage: ticket.qrCodeImage || undefined,
      ...
      eventLocation: eventRef?.location
        ? `${eventRef.location.address ?? ''}, ${eventRef.location.city ?? ''}`.replace(/^,\s*|,\s*$/, '')
        : undefined,
    });
  };
  ...
  const handleGenerateTickets = async () => {
    logger.debug('🎫 GENERATE TICKETS DEBUG:', {
      orderId: id,
      clickedAt: new Date().toISOString()
    });

    if (!id) {
      logger.warn('❌ No order ID, returning early');
      return;
    }

    try {
      logger.debug('🔄 Starting ticket generation...');
      setIsGeneratingTickets(true);

      logger.debug('📡 Dispatching generateMissingTickets action');
      const result = await dispatch(generateMissingTickets(id));

      logger.debug('📋 Generate tickets result:', result);

      if (generateMissingTickets.fulfilled.match(result)) {
        logger.debug('✅ Tickets generated successfully:', result.payload);
        toast.success(`Generated ${result.payload.length} tickets successfully!`);

        logger.debug('🔄 Refreshing tickets list...');
        // Refresh tickets
        dispatch(fetchTicketsByOrder(id));
      } else {
        logger.error('❌ Generate tickets action rejected:', result);
  ...
  const bookingTickets = tickets.filter(ticket => 
    ticket.orderId === booking?._id || ticket.orderId === id
  );

  // Debug logging to understand ticket data
  logger.debug('🔍 TICKET DEBUG INFO:', {
    orderId: id,
    allTickets: tickets,
    filteredTickets: bookingTickets,
    bookingTicketsLength: bookingTickets.length,
    ticketsLoading,
    bookingStatus: booking?.status,
    ticketsReduxError: ticketsError
  });
  ...
  const allParticipants = booking?.items?.flatMap(item => item.participants ?? []) ?? [];
  const isFreeBooking =
    booking?.total === 0 ||
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className={`flex items-center px-3 py-2 rounded-lg border ${getStatusColor(booking?.status || 'pending')}`}>
              ...
              <span className="ml-2 font-medium capitalize">
                ...
                {isFreeBooking && booking?.status === 'confirmed' && (
                  <span className="ml-1 text-xs font-normal text-green-600">(Free)</span>
                )}
              </span>
            </div>
          </div>
                ...
                <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
                  <div className="p-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      <img
                        src={event.coverImage || event.images?.[0] || '/placeholder-event.jpg'}
                        alt={event.title}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="space-y-2 text-sm text-gray-600">
                          <div className="flex items-center">
                            ...
                            {firstItem?.scheduleDate
                              ? format(new Date(firstItem.scheduleDate), 'PPP')
                              : event.dateSchedule?.[0]
                              ? format(new Date(event.dateSchedule[0].startDate), 'PPP')
                              : 'Date TBD'}
                          </div>
                          <div className="flex items-center">
                            <Clock className="w-4 h-4 mr-2" />
                            {(() => {
                              const sched = event.dateSchedule?.[0];
                  ...
                  {ticketsLoading ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <button
                        ...
                        className={`inline-flex items-center px-6 py-3 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white transition-colors ${isGeneratingTickets || booking?.status !== 'confirmed'
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {isFreeBooking
                          ? 'Free'
                          : firstItem && booking?.currency
                            ? formatPrice((firstItem.unitPrice || firstItem.price || 0), booking.currency)
                            : 'N/A'}
                      </span>
                    </div>
                      ...
                      <div className="flex justify-between text-lg font-semibold">
                        ...
                        <span>
                          {isFreeBooking
                            ? 'Free'
                            : booking?.currency
                              ? formatPrice(booking.total ?? 0, booking.currency)
                              : 'N/A'}
                        </span>
                      </div>
                    </div>
          ...
          {booking && (
            <QRCodeModal
              ...
              qrData={{
                ...
                type: 'order',
                eventStartDate: firstItem?.eventId?.dateSchedule?.[0]?.startDate ? new Date(firstItem.eventId.dateSchedule[0].startDate) : undefined,
                eventEndDate: firstItem?.eventId?.dateSchedule?.[0]?.endDate ? new Date(firstItem.eventId.dateSchedule[0].endDate) : undefined,
                gracePeriodHours: 2
              }}
              size={300}
          ...
          {selectedTicket && (
            <QRCodeModal
              ...
              qrData={{
                ...
                seatsAllocated: 1,
                type: 'ticket',
                eventStartDate: selectedTicket.eventId?.dateSchedule?.[0]?.startDate ? new Date(selectedTicket.eventId.dateSchedule[0].startDate) : undefined,
                eventEndDate: selectedTicket.eventId?.dateSchedule?.[0]?.endDate ? new Date(selectedTicket.eventId.dateSchedule[0].endDate) : undefined,
                gracePeriodHours: 2
              }}
              size={300}
          ...
          {booking && (
            <CancelOrderModal
              ...
              eventDate={firstItem?.scheduleDate || event?.dateSchedule?.[0]?.startDate || new Date().toISOString()}
