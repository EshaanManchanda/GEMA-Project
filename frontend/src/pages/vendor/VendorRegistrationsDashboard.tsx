...
const VendorRegistrationsDashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (eventId) {
      dispatch(fetchEventRegistrations({ eventId, queryParams: { page: 1, limit: 10 } }));
    }
  }, [eventId, dispatch]);
  ...
  const handleFilterChange = (status?: RegistrationStatus) => {
    ...
    if (eventId) {
      dispatch(fetchEventRegistrations({ eventId, queryParams: { page: 1, limit: 10, status, search: searchQuery } }));
    }
  };
  ...
  const handleSearch = () => {
    ...
    if (eventId) {
      dispatch(fetchEventRegistrations({ eventId, queryParams: { page: 1, limit: 10, search: searchQuery } }));
    }
  };
  ...
  const handlePageChange = (page: number) => {
    if (eventId) {
      dispatch(fetchEventRegistrations({ eventId, queryParams: { page, limit: 10, ...filters } }));
    }
  };
  ...
  const handleReview = async (registrationId: string, status: RegistrationReviewStatus, remarks?: string) => {
    try {
      ...
      // Refresh list
      if (eventId) {
        dispatch(fetchEventRegistrations({ eventId, queryParams: { page: pagination.currentPage, limit: 10, ...filters } }));
      }
    } catch (error: any) {
  ...
  return (
    <>
      <PrivatePageSEO title="Vendor - Listings | Kidrove" description="Manage your listings" />
                        ...
                        <tr key={registration._id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              ...
                              <div>
                                <div className="text-sm font-medium text-gray-900">
                                  {typeof registration.userId === 'object' && (registration.userId as any).name
                                    ? (registration.userId as any).name
                                    : 'User'}
                                </div>
                                <div className="text-xs text-gray-500">
                                  {typeof registration.userId === 'object' && (registration.userId as any).email
                                    ? (registration.userId as any).email
                                    : ''}
                                </div>
                              </div>
                            </div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              <div className={`w-2 h-2 rounded-full mr-2 ${registration.payment.status === 'paid' ? 'bg-green-500' : 'bg-yellow-500'
                                }`} />
