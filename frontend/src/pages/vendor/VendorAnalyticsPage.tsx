  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      ...
      try {
        ...
        // API returns { success, message, data: {...} } wrapper
        const revenueData = revenueReport?.data || revenueReport || {};
        const orderData = orderAnalytics?.data || orderAnalytics || {};
        const eventData = eventAnalytics?.data || eventAnalytics || {};
        const dashData = dashboardData?.data || dashboardData || {};

        const completedOrders = orderData.ordersByStatus?.find(
          (s: any) => s.status === 'confirmed'
        )?.count || 0;
        const cancelledOrders = orderData.ordersByStatus
          ?.filter((s: any) => s.status === 'cancelled' || s.status === 'refunded')
          .reduce((sum: number, s: any) => sum + s.count, 0) || 0;

        const transformedData: AnalyticsData = {
          revenue: {
            total: revenueData.totalRevenue || 0,
            daily: revenueData.revenueByPeriod?.map((item: any) => ({
              date: item.month || item.date || '',
              amount: item.revenue || item.amount || 0,
            })) || [],
            monthly: revenueData.revenueByPeriod?.map((item: any) => ({
          ...
          bookings: {
            ...
            byEvent: dashData.topPerformingEvents?.map((e: any) => ({
              eventName: e.title || 'Unknown',
              count: e.tickets || 0,
            })) || [],
          },
          events: {
            total: eventData.totalEvents || 0,
            published: eventData.activeEvents || 0,
            draft: eventData.pendingApproval || 0,
            byCategory: eventData.topCategories?.map((item: any) => ({
