                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {commissionData.transactions.map((tx) => (
                    <tr key={tx.id}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${tx.status === 'paid' ? 'bg-green-100 text-green-800' :
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            ...
            {/* Payment Model Badge */}
            <div className="mt-3">
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${isSubscriptionModel ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'
                }`}>
                ...
                <div>
                  ...
                  {hasFundsReady && (
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 mt-2">
                      Funds Ready: {earnings?.currency} {eligibleBalance.toFixed(2)} eligible now
                    </span>
                  )}
                </div>
                <button
                ...
                {pendingRequests.map((request: any) => (
                  <div key={request.id} className="px-6 py-4 flex items-center justify-between">
                    <div>
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mt-2 ${request.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                    ...
                    {recentPayouts.map((payout: any) => (
                      <tr key={payout.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${payout.status === 'completed' ? 'bg-green-100 text-green-800' :
