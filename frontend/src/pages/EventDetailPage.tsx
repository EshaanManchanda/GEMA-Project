  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 pb-20 lg:pb-8">
        ...
        {event.pastEventMemories && event.pastEventMemories.length > 0 && (
            ...
            <div className="p-8 md:p-10">
              <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
                <div>
                  ...
                  <p className="text-gray-600 max-w-2xl">
                    See what participants have accomplished and experienced in previous sessions of this event.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-10 bg-gray-50/50 rounded-3xl border border-dashed border-gray-200">
                <div className="flex -space-x-4 mb-6">
                  {event.pastEventMemories.slice(0, 4).map((m: any, i: number) => (
                    <div key={i} className="w-16 h-16 rounded-full border-4 border-white overflow-hidden shadow-lg transform hover:-translate-y-2 transition-transform duration-300">
                      <img src={m.image} alt="" className="w-full h-full object-cover" />
                    </div>
                  ))}
                  {event.pastEventMemories.length > 4 && (
                    <div className="w-16 h-16 rounded-full border-4 border-white bg-primary-600 flex items-center justify-center text-white text-xs font-bold shadow-lg">
                      +{event.pastEventMemories.length - 4}
                    </div>
                  )}
                </div>
                ...
                <button
                  onClick={() => setIsAllMemoriesModalOpen(true)}
                  className="px-10 py-4 bg-gray-900 text-white rounded-2xl font-bold text-lg hover:bg-black transition-all transform hover:scale-105 shadow-xl hover:shadow-2xl flex items-center gap-3 group"
                >
                  <svg className="w-6 h-6 text-primary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                  <span>View all {event.pastEventMemories.length} photos</span>
                  <ChevronRight className="w-6 h-6 transform group-hover:translate-x-1 transition-transform" />
                </button>
                <p className="mt-4 text-sm text-gray-500 font-medium">Click to open full gallery with captions and participant names</p>
              </div>
            </div>

            {/* Visual bottom accent */}
            <div className="h-2 bg-gradient-to-r from-primary-400 via-primary-600 to-indigo-600"></div>
          </div>
        )}
        ...
        {/* All Photos Scrollable Modal */}
        {isAllMemoriesModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 animate-in fade-in zoom-in-95 duration-300">
            <div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={() => setIsAllMemoriesModalOpen(false)}></div>
            <div className="bg-white w-full max-w-6xl h-full max-h-[90vh] rounded-3xl overflow-hidden relative z-[110] flex flex-col shadow-2xl border border-white/20">
              <div className="p-6 md:p-8 flex items-center justify-between border-b border-gray-100 bg-gray-50/50">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">{event.pastEventMemories.length} Photos Captured</p>
                </div>
                <button
                  onClick={() => setIsAllMemoriesModalOpen(false)}
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar">
                <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
                  {event.pastEventMemories.map((memory, index) => (
                    <div
                      key={index}
                      ...
                      onClick={() => {
                        ...
                        setIsAllMemoriesModalOpen(false);
                      }}
                    >
                      <img
                        src={memory.image}
                      ...
                      <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                        <p className="text-white text-xs italic line-clamp-2">"{memory.caption}"</p>
                        <p className="text-primary-300 text-[10px] font-bold uppercase tracking-wider mt-1">{memory.participantName}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="p-6 border-t border-gray-100 bg-gray-50 text-center">
                <button
                  onClick={() => setIsAllMemoriesModalOpen(false)}
                  className="px-8 py-3 bg-gray-900 text-white rounded-xl font-semibold hover:bg-gray-800 transition-colors shadow-lg shadow-gray-200"
                >
                  Close Gallery
                </button>
              </div>
          ...
          <div className="flex flex-col lg:flex-row gap-10">
            ...
            <div className="w-full lg:w-2/3 flex flex-col gap-5">
              {(() => {
                ...
                return all.map((review: any, idx: number) => {
