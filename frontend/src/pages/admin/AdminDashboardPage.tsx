...
const AdminDashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    if (dashboardData) {
      try {
        ...
        logger.error('Error processing dashboard data:', err);
      ...
      const errorMessage = (dashboardError as any)?.response?.data?.message || (dashboardError as any)?.message || 'Failed to load dashboard data';
      setError(errorMessage);
      setStats([]);
      setRecentActivity([{
        id: 'error-1',
  ...
  return (
    <>
      <PrivatePageSEO title="Admin Dashboard | Gema" description="Admin control panel" />
      <div className="container mx-auto px-4 py-8">
          ...
          {stats.length === 0 && !isLoading ? (
            <div className="col-span-full">
              <div className="bg-red-50 border border-red-200 rounded-2xl p-8 text-center">
                ...
                <p className="text-red-600 mb-4">{error || 'Unable to load dashboard statistics.'}</p>
                <div className="flex justify-center gap-4">
                  <button onClick={() => window.location.reload()} className="inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">Refresh Page</button>
                </div>
              </div>
            </div>
          ) : (
            stats.map((stat, index) => {
              const gradients = ['from-blue-500 to-indigo-600', 'from-green-500 to-emerald-600', 'from-purple-500 to-pink-600', 'from-orange-500 to-red-600'];
              const bgGradients = ['from-blue-50 to-indigo-50', 'from-green-50 to-emerald-50', 'from-purple-50 to-pink-50', 'from-orange-50 to-red-50'];
              return (
                <div key={index} className={`bg-gradient-to-br ${bgGradients[index]} rounded-2xl shadow-xl shadow-gray-200/50 p-6 border border-white/20 backdrop-blur-sm hover:shadow-2xl hover:shadow-gray-300/30 transition-all duration-300 transform hover:scale-105 group`}>
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className={`p-3 rounded-xl bg-gradient-to-r ${gradients[index]} text-white shadow-lg transition-all duration-300 group-hover:scale-110`}>{stat.icon}</div>
                  </div>
