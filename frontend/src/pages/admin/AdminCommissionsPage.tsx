          ...
          <div className="border-b border-gray-200">
            <nav className="flex space-x-8" aria-label="Tabs">
              <button
                ...
                className={`py-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'configs'
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {commissionConfigs.map((config) => (
                    <div
                      ...
                      className={`bg-white border rounded-xl p-6 hover:shadow-lg transition-shadow ${config.isDefault ? 'border-blue-300 bg-blue-50' : 'border-gray-200'
                        }`}
                    >
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        ...
                        <div className="space-y-2">
                          {config.rules.slice(0, 3).map((rule, index) => (
                            <div key={rule.id || rule._id || index} className="flex justify-between text-sm">
                  ...
                  <div className="bg-white rounded-lg shadow p-6">
                    ...
                    <div className="space-y-3">
                      {commissionStats?.topVendors?.slice(0, 5).map((vendor, index) => (
                        <div key={vendor.vendorId || index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <div className="flex items-center">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium ${index === 0 ? 'bg-yellow-500' : index === 1 ? 'bg-gray-400' : index === 2 ? 'bg-amber-600' : 'bg-blue-500'
                              }`}>
                              {index + 1}
                            </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {configForm.rules.map((rule, index) => (
                        <div key={rule.id || rule._id || index} className="border border-gray-200 rounded-lg p-4">
