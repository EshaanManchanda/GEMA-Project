...
import type { PayoutRequest } from '../../store/slices/adminSlice';
...
const AdminPayoutsPage: React.FC = () => {
  ...
  useSelector(selectPayoutStats);
  useSelector(selectPendingPayouts);
  ...
  const loadPayoutData = async () => {
    try {
      await Promise.all([
        dispatch(fetchPayoutRequests({} as any)).unwrap(),
        dispatch(fetchVendorEarnings({} as any)).unwrap(),
        dispatch(fetchPayoutStats({} as any)).unwrap()
      ]);
    } catch (error) {
