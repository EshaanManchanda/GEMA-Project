...
const AdminUsersPage: React.FC = () => {
  ...
  const handleViewUser = async (user: AdminUser) => {
    ...
    // Re-fetch full user data (includes related profiles)
    try {
      const res = await adminAPI.getUserById(user.id) as any;
      const full: AdminUser = res.data?.user ?? res.user ?? res.data ?? res;
      setSelectedUser(full);
    } catch {
      // silently keep the list-version data
    }
  };
  ...
  // Auto-open create modal when navigated from dashboard with openCreate state
  useEffect(() => {
                        ...
                        <div className="flex items-center">
                          <div className="relative">
                            <img
                              ...
                              src={normalizeImageUrl(user.avatar) || `https://ui-avatars.com/api/?name=${user.fullName}&background=random`}
                              alt={user.fullName}
                            />
                            <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${user.status === 'active' ? 'bg-green-400' :
                    ...
                    {Array.from({ length: Math.min(5, pagination.totalPages) }, (_, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 ${currentPage === pageNum
            ...
            <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex justify-between items-center p-6 border-b border-gray-200">
                <div className="flex items-center gap-4">
                  <img
                    ...
                    src={normalizeImageUrl(selectedUser.avatar) || `https://ui-avatars.com/api/?name=${selectedUser.firstName} ${selectedUser.lastName}&background=random`}
              ...
              <div className="flex border-b border-gray-200 px-6 bg-gray-50">
                <button
                  ...
                  className={`px-4 py-3 font-medium transition-colors ${viewModalTab === 'basic'
                ...
                <button
                  ...
                  className={`px-4 py-3 font-medium transition-colors flex items-center gap-2 ${viewModalTab === 'security'
                    ? 'border-b-2 border-blue-600 text-blue-600'
                    : 'text-gray-600 hover:text-gray-900'
                    }`}
                >
                ...
                {selectedUser.role === 'vendor' && (
                  <button
                    ...
                    className={`px-4 py-3 font-medium transition-colors flex items-center gap-2 ${viewModalTab === 'vendor'
                      ? 'border-b-2 border-blue-600 text-blue-600'
                      : 'text-gray-600 hover:text-gray-900'
                      }`}
                  >
                ...
                {selectedUser.role === 'teacher' && (
                  <button
                    onClick={() => setViewModalTab('teacher')}
                    className={`px-4 py-3 font-medium transition-colors flex items-center gap-2 ${viewModalTab === 'teacher'
                      ? 'border-b-2 border-blue-600 text-blue-600'
                      : 'text-gray-600 hover:text-gray-900'
                      }`}
                  >
                ...
                {selectedUser.role === 'employee' && (
                  <button
                    onClick={() => setViewModalTab('employee')}
                    className={`px-4 py-3 font-medium transition-colors flex items-center gap-2 ${viewModalTab === 'employee'
                      ? 'border-b-2 border-blue-600 text-blue-600'
                      : 'text-gray-600 hover:text-gray-900'
                      }`}
                  >
                ...
                <button
                  ...
                  className={`px-4 py-3 font-medium transition-colors flex items-center gap-2 ${viewModalTab === 'history'
                    ? 'border-b-2 border-blue-600 text-blue-600'
                    : 'text-gray-600 hover:text-gray-900'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-3 font-medium transition-colors flex items-center gap-2 ${viewModalTab === 'related'
                    ? 'border-b-2 border-blue-600 text-blue-600'
                    : 'text-gray-600 hover:text-gray-900'
                    }`}
                >
              ...
              <div className="flex-1 overflow-y-auto p-6">
                ...
                {viewModalTab === 'basic' && (
                  <div className="space-y-6">
                    {selectedUser.avatar && (
                      <div className="flex items-center gap-4">
                        <img src={normalizeImageUrl(selectedUser.avatar)} alt="Avatar" className="w-16 h-16 rounded-full object-cover border border-gray-200" />
                        <p className="text-xs text-gray-500 font-mono break-all">{selectedUser.avatar}</p>
                      </div>
                    )}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <p className="text-gray-900 capitalize">{selectedUser.gender?.replace(/_/g, ' ') || 'Not specified'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Date of Birth</label>
                        <p className="text-gray-900">
                          {selectedUser.dateOfBirth ? format(new Date(selectedUser.dateOfBirth), 'MMM dd, yyyy') : 'Not provided'}
                        </p>
                      </div>
                      <div>
                      ...
                      {selectedUser.firebaseUid && (
                        <div className="md:col-span-2">
                          ...
                          <p className="text-gray-600 text-sm font-mono">{selectedUser.firebaseUid}</p>
                        </div>
                      )}
                    </div>
                    {selectedUser.socialMedia && Object.values(selectedUser.socialMedia).some(Boolean) && (
                          ...
                          {Object.entries(selectedUser.socialMedia).map(([platform, url]) =>
                            url ? (
                              <div key={platform}>
                                ...
                                <span className="text-sm text-blue-600 ml-2 break-all">{url as string}</span>
                              </div>
                            ) : null
                          )}
                        </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedUser.isEmailVerified ? 'bg-green-100' : 'bg-red-100'
                            }`}>
                        ...
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedUser.isPhoneVerified ? 'bg-green-100' : 'bg-red-100'
                            }`}>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`px-3 py-1 text-xs font-semibold rounded-full ${selectedUser.twoFactorAuth?.enabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                            }`}>
                        ...
                        {selectedUser.twoFactorAuth?.enabled && (
                          <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              {selectedUser.twoFactorAuth.backupCodesCount ?? 0} backup codes generated
                            </p>
                          </div>
                        )}
                      </div>
                ...
                {viewModalTab === 'vendor' && selectedUser.role === 'vendor' && (
                  <div className="space-y-6">
                    {!selectedUser.vendorProfile ? (
                      <p className="text-gray-500 text-center py-8">Loading vendor profile…</p>
                    ) : (
                      <>
                        <div className="bg-gray-50 rounded-lg p-4">
                          <h4 className="font-semibold text-gray-900 mb-4">Business Info</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Business Name</label><p className="text-gray-900 font-medium">{selectedUser.vendorProfile.businessName}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Category</label><p className="text-gray-900">{selectedUser.vendorProfile.category || '—'}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Email</label><p className="text-gray-900">{selectedUser.vendorProfile.email}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Phone</label><p className="text-gray-900">{selectedUser.vendorProfile.phone || '—'}</p></div>
                            <div className="md:col-span-2"><label className="block text-xs font-medium text-gray-500 mb-0.5">Description</label><p className="text-gray-900 text-sm">{selectedUser.vendorProfile.description || '—'}</p></div>
                            <div>
                              <label className="block text-xs font-medium text-gray-500 mb-0.5">Verification</label>
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${selectedUser.vendorProfile.verificationStatus === 'verified' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                {selectedUser.vendorProfile.verificationStatus}
                              </span>
                            </div>
                            <div className="flex gap-4">
                              <div>
                                <label className="block text-xs font-medium text-gray-500 mb-0.5">Active</label>
                                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${selectedUser.vendorProfile.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{selectedUser.vendorProfile.isActive ? 'Yes' : 'No'}</span>
                              </div>
                              <div>
                                <label className="block text-xs font-medium text-gray-500 mb-0.5">Suspended</label>
                                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${selectedUser.vendorProfile.isSuspended ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}`}>{selectedUser.vendorProfile.isSuspended ? 'Yes' : 'No'}</span>
                              </div>
                            </div>
                            {selectedUser.vendorProfile.suspensionReason && (
                              <div className="md:col-span-2"><label className="block text-xs font-medium text-gray-500 mb-0.5">Suspension Reason</label><p className="text-red-700 text-sm">{selectedUser.vendorProfile.suspensionReason}</p></div>
                            )}
                          </div>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Payment Mode</label><p className="text-gray-900 capitalize">{selectedUser.vendorProfile.paymentMode || '—'}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Commission Rate</label><p className="text-gray-900 font-semibold">{selectedUser.vendorProfile.commissionRate ?? '—'}%</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Custom Commission</label><p className="text-gray-900">{selectedUser.vendorProfile.customCommissionRate !== undefined ? `${selectedUser.vendorProfile.customCommissionRate}%` : '—'}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Payout Schedule</label><p className="text-gray-900 capitalize">{selectedUser.vendorProfile.payoutSchedule || '—'}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Minimum Payout</label><p className="text-gray-900">AED {selectedUser.vendorProfile.minimumPayout ?? '—'}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Accepts Platform Payments</label><span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${selectedUser.vendorProfile.acceptsPlatformPayments ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{selectedUser.vendorProfile.acceptsPlatformPayments ? 'Yes' : 'No'}</span></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Subscription Status</label><p className="text-gray-900 capitalize">{selectedUser.vendorProfile.subscriptionStatus || '—'}</p></div>
                            {selectedUser.vendorProfile.subscriptionPaidUntil && (
                              <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Paid Until</label><p className="text-gray-900">{format(new Date(selectedUser.vendorProfile.subscriptionPaidUntil), 'MMM dd, yyyy')}</p></div>
                            )}
                          </div>
                        </div>
                        {selectedUser.vendorProfile.stripeConnectAccountId && (
                          <div className="bg-gray-50 rounded-lg p-4">
                            <h4 className="font-semibold text-gray-900 mb-3">Stripe Connect</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Account ID</label><p className="text-gray-900 font-mono text-sm">{selectedUser.vendorProfile.stripeConnectAccountId}</p></div>
                              <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Onboarding Complete</label><span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${selectedUser.vendorProfile.stripeConnectOnboardingComplete ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{selectedUser.vendorProfile.stripeConnectOnboardingComplete ? 'Yes' : 'No'}</span></div>
                            </div>
                          </div>
                        )}
                        {selectedUser.vendorProfile.stats && (
                          <div className="bg-gray-50 rounded-lg p-4">
                            <h4 className="font-semibold text-gray-900 mb-3">Stats</h4>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                              {Object.entries(selectedUser.vendorProfile.stats).map(([k, v]) => (
                                <div key={k} className="text-center bg-white p-3 rounded border border-gray-200">
                                  <p className="text-lg font-bold text-blue-600">{String(v)}</p>
                ...
                {/* Teacher Profile Tab */}
                {viewModalTab === 'teacher' && selectedUser.role === 'teacher' && (
                  <div className="space-y-6">
                    {!selectedUser.teacherProfile ? (
                      <p className="text-gray-500 text-center py-8">Loading teacher profile…</p>
                    ) : (
                      <>
                        <div className="bg-gray-50 rounded-lg p-4">
                          <h4 className="font-semibold text-gray-900 mb-4">Teacher Info</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Full Name</label><p className="text-gray-900 font-medium">{selectedUser.teacherProfile.fullName}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Specialization</label><p className="text-gray-900">{selectedUser.teacherProfile.specialization || '—'}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Teaching Mode</label><p className="text-gray-900 capitalize">{selectedUser.teacherProfile.teachingMode || '—'}</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Years of Experience</label><p className="text-gray-900">{selectedUser.teacherProfile.yearsOfExperience ?? '—'}</p></div>
                            <div className="md:col-span-2"><label className="block text-xs font-medium text-gray-500 mb-0.5">Bio</label><p className="text-gray-900 text-sm">{selectedUser.teacherProfile.bio || '—'}</p></div>
                            {selectedUser.teacherProfile.subjects && selectedUser.teacherProfile.subjects.length > 0 && (
                              <div className="md:col-span-2">
                                <label className="block text-xs font-medium text-gray-500 mb-1">Subjects</label>
                                <div className="flex flex-wrap gap-2">
                                  {selectedUser.teacherProfile.subjects.map((s, i) => (
                            ...
                            <div>
                              <label className="block text-xs font-medium text-gray-500 mb-0.5">Verification</label>
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${selectedUser.teacherProfile.verificationStatus === 'verified' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                {selectedUser.teacherProfile.verificationStatus || 'pending'}
                              </span>
                            </div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Commission Rate</label><p className="text-gray-900">{selectedUser.teacherProfile.commissionRate ?? '—'}%</p></div>
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Payout Schedule</label><p className="text-gray-900 capitalize">{selectedUser.teacherProfile.payoutSchedule || '—'}</p></div>
                          </div>
                        </div>
                        {selectedUser.teacherProfile.stats && (
                          <div className="bg-gray-50 rounded-lg p-4">
                            <h4 className="font-semibold text-gray-900 mb-3">Stats</h4>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                              {Object.entries(selectedUser.teacherProfile.stats).map(([k, v]) => (
                                <div key={k} className="text-center bg-white p-3 rounded border border-gray-200">
                                  <p className="text-lg font-bold text-teal-600">{String(v)}</p>
                                  <p className="text-xs text-gray-500 capitalize">{k.replace(/([A-Z])/g, ' $1').trim()}</p>
                                </div>
                              ))}
                            </div>
                ...
                {viewModalTab === 'employee' && selectedUser.role === 'employee' && (
                  <div className="space-y-6">
                    {!selectedUser.employeeProfile ? (
                      <p className="text-gray-500 text-center py-8">Loading employee profile…</p>
                    ) : (
                      <div className="bg-gray-50 rounded-lg p-4">
                        <h4 className="font-semibold text-gray-900 mb-4">Employee Details</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Employee ID</label><p className="text-gray-900 font-mono">{selectedUser.employeeProfile.employeeId}</p></div>
                          <div>
                            <label className="block text-xs font-medium text-gray-500 mb-0.5">Role</label>
                            <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-orange-100 text-orange-800 capitalize">{selectedUser.employeeProfile.role}</span>
                          </div>
                          <div>
                            <label className="block text-xs font-medium text-gray-500 mb-0.5">Status</label>
                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${selectedUser.employeeProfile.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{selectedUser.employeeProfile.status}</span>
                          </div>
                          {selectedUser.employeeProfile.vendorId && (
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Vendor ID</label><p className="text-gray-900 font-mono text-sm">{selectedUser.employeeProfile.vendorId}</p></div>
                          )}
                          {selectedUser.employeeProfile.hiredAt && (
                            <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Hired At</label><p className="text-gray-900">{format(new Date(selectedUser.employeeProfile.hiredAt), 'MMM dd, yyyy')}</p></div>
                          )}
                          {selectedUser.employeeProfile.permissions && selectedUser.employeeProfile.permissions.length > 0 && (
                            <div className="md:col-span-2">
                              <label className="block text-xs font-medium text-gray-500 mb-1">Permissions</label>
                              <div className="flex flex-wrap gap-2">
                                {selectedUser.employeeProfile.permissions.map((p, i) => (
                                  <span key={i} className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">{p.action} ({p.scope})</span>
                                ))}
                              </div>
                            </div>
                          )}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="font-semibold text-gray-900">
                        Login Attempts ({selectedUser.loginAttemptsCount ?? selectedUser.loginAttempts?.length ?? 0})
                      </h4>
                    </div>
                          ...
                          <tbody className="divide-y divide-gray-200">
                            {selectedUser.loginAttempts.map((attempt, index) => (
                              <tr key={attempt._id || index} className="hover:bg-gray-50">
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${attempt.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                    }`}>
                        ...
                        <div className="space-y-2">
                          {selectedUser.socialLogins.map((social, index) => (
                            <div key={index} className="flex items-center gap-3 bg-white p-3 rounded">
                              <span className="capitalize font-medium text-gray-900">{social.provider}</span>
                              <span className="text-sm text-gray-500 font-mono">{social.providerId}</span>
                            </div>
                          ))}
                        </div>
                      ) : (
                    ...
                    {selectedUser.preferences && (
                      <div className="bg-gray-50 rounded-lg p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Language</label><p className="text-gray-900">{selectedUser.preferences.language || '—'}</p></div>
                          <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Currency</label><p className="text-gray-900">{selectedUser.preferences.currency || '—'}</p></div>
                          <div><label className="block text-xs font-medium text-gray-500 mb-0.5">Timezone</label><p className="text-gray-900">{selectedUser.preferences.timezone || '—'}</p></div>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <form className="flex flex-col flex-1 overflow-hidden" onSubmit={async (e) => {
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  ...
                  <div className="space-y-4">
                      ...
                      {(selectedRole === 'teacher' || selectedUser?.role === 'teacher') && isCreateModalOpen && (
                        <div className="bg-teal-50 rounded-lg p-4 border border-teal-200">
                          <h4 className="font-semibold text-teal-900 mb-4 flex items-center">
                            <span className="bg-teal-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm mr-2">T</span>
                            Teacher Details
                          </h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <input
                                ...
                                placeholder="e.g., Mathematics, Music, Yoga"
                ...
                <div className="flex justify-end gap-2 p-6 border-t border-gray-200">
                  ...
                  <button
                    ...
                    disabled={actionLoading.form_submit}
                  >
                    {actionLoading.form_submit ? 'Saving...' : 'Create User'}
                  </button>
                </div>
              </form>
            </div>
