...
const BlogPage: React.FC = () => {
  ...
  const categoryId = selectedCategory === 'All' ? undefined
    : typeof selectedCategory === 'string' ? selectedCategory
      : (selectedCategory as BlogCategory)._id;

  // TanStack Query — caches data across navigations, no blank flash on re-visit
  const { data: categoriesData } = useQuery({
    queryKey: ['blog-categories'],
    queryFn: async () => {
      const res = await blogAPI.getBlogCategories();
  ...
  const { data: blogsData, isLoading: loading, error: blogsError, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { blogs: (data.blogs || []) as Blog[], totalPages: data.pagination?.totalPages ?? 1 };
    },
    staleTime: 2 * 60 * 1000,     // data stays fresh for 2 min
    placeholderData: (prev) => prev, // show previous data while refetching (no blank flash)
  });

  const categories = categoriesData || [];
  const blogs = blogsData?.blogs || [];
  const totalPages = blogsData?.totalPages ?? 1;
  const error = blogsError ? (blogsError as any)?.response?.data?.message || 'Failed to load blog posts' : null;
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-screen-xl mx-auto">
      <SEO
        title={seoData.title}
            ...
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 border-t border-b border-gray-300 text-sm font-medium ${pageNum === currentPage
