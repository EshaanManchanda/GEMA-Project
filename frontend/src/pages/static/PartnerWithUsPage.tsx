...
const PartnerWithUsPage: React.FC = () => {
  ...
  const vendorRegistrationSteps = [
    {
      step: 1,
      icon: FiUserCheck,
      title: 'Create Your Account',
      desc: 'Click the link below to enter your first name, last name, email address, phone number, and password.'
    },
    {
      step: 2,
      icon: FiMail,
      title: 'Verify Your Email',
      desc: 'Check your inbox for a 6-digit OTP code and enter it to verify your email address.'
    },
    {
      step: 3,
      icon: FiCheckCircle,
      title: 'Complete Your Profile',
      desc: 'Add your business name, category, description, logo, and cover image to make your profile stand out.'
    },
    {
      step: 4,
      icon: FiShield,
      title: 'Upload Documents',
      desc: '(Optional but recommended) Upload your business license, tax certificate, or identity document to earn a verified badge.'
    },
    {
      step: 5,
      icon: FiDollarSign,
      title: 'Set Up Payments',
      desc: 'Connect your bank account details to receive payouts directly via Stripe Connect or bank transfer.'
    },
    {
      step: 6,
      icon: FiZap,
      title: "You're Live!",
      desc: 'Your vendor account is ready. Start browsing your vendor dashboard and create your first event.'
    }
  ];

  const eventPostingSteps = [
    {
      step: 1,
      icon: FiCalendar,
      title: 'Open Your Dashboard',
      desc: 'Go to Vendor Dashboard → Events → Create New Event.'
    },
    {
      step: 2,
      icon: FiCheckCircle,
      title: 'Fill Event Details',
      desc: 'Add title, description, category, date/time, location, price, and max capacity.'
    },
    {
      step: 3,
      icon: FiGlobe,
      title: 'Add Media',
      desc: 'Upload event images and a cover photo to attract more attendees.'
    },
    {
      step: 4,
      icon: FiSend,
      title: 'Submit for Review',
      desc: 'Click "Submit for Approval" when your event details are ready.'
    },
    {
      step: 5,
      icon: FiZap,
      title: 'Go Live!',
      desc: 'Admin reviews and approves your event within 24 hours.'
    }
  ];
          ...
          >
            <div className="max-w-7xl mx-auto">
              <motion.div variants={itemVariants} className="text-center mb-16">
                ...
                <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                  Everything you need to know about setting up your vendor account and publishing events.
                </p>
              </motion.div>

              {/* Registration Steps */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
                {vendorRegistrationSteps.map((step, index) => (
                  <motion.div
                    key={index}
                    variants={itemVariants}
                    className="relative bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-blue-50"
                  >
                    <div className="absolute -top-4 -left-4 w-10 h-10 rounded-full flex items-center justify-center text-white font-bold shadow-lg text-lg"
                    ...
                    {step.step === 1 && (
                      <Link to="/vendor/register" className="mt-4 inline-flex items-center text-sm font-bold hover:underline" style={{ color: 'var(--primary-color)' }}>
                        Register as Vendor <FiArrowRight className="ml-1" />
                      </Link>
                    )}
                  </motion.div>
                ))}
              ...
              <div className="relative mb-20">
                <div className="hidden lg:block absolute top-12 left-10 right-10 h-1 bg-gradient-to-r from-purple-200 to-pink-200"></div>
                <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
                  {eventPostingSteps.map((step, index) => (
                    <motion.div key={index} variants={itemVariants} className="relative z-10 flex flex-col items-center text-center group">
                      <div className="w-24 h-24 rounded-full bg-white flex items-center justify-center shadow-xl border-4 transition-transform duration-300 group-hover:scale-110 mb-6"
                        style={{ borderColor: 'var(--accent-color)' }}
                      >
                        <step.icon className="w-10 h-10" style={{ color: 'var(--accent-color)' }} />
                      </div>
                      <div className="absolute top-0 right-1/2 transform translate-x-12 -translate-y-2 w-8 h-8 rounded-full flex items-center justify-center text-white font-bold shadow-md"
              ...
              {/* 24-Hour Approval & Admin Note */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <motion.div variants={itemVariants} className="bg-gradient-to-br from-teal-50 to-green-50 rounded-2xl p-8 border border-teal-100 shadow-md flex items-start gap-6">
                  <div className="w-14 h-14 rounded-full bg-teal-100 flex items-center justify-center flex-shrink-0 shadow-sm">
                    <FiClock className="w-7 h-7 text-teal-600" />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-teal-900 mb-2">Events Go Live Within 24 Hours</h3>
                    <p className="text-teal-800 leading-relaxed">
                      All submitted events are reviewed by our moderation team and approved within 1 business day to maintain quality standards for our community.
                    </p>
                  </div>
                </motion.div>

                <motion.div variants={itemVariants} className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-8 border border-blue-100 shadow-md flex flex-col justify-center">
                  <div className="flex items-start gap-6 mb-6">
                    <div className="w-14 h-14 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0 shadow-sm">
                      <FiUserCheck className="w-7 h-7 text-blue-600" />
                    </div>
                    <div>
                      <h3 className="text-xl font-bold text-blue-900 mb-2">Admin Onboarding Setup</h3>
                      <p className="text-blue-800 leading-relaxed">
                        Need help getting started? Our team can create your vendor account and upload your first events for you.
                      </p>
                    </div>
                  </div>
                  <Link to="/contact" className="self-end px-6 py-3 bg-white text-blue-600 font-bold rounded-xl shadow border border-blue-200 hover:bg-blue-600 hover:text-white transition-colors duration-300">
                    Contact Our Team
                  </Link>
                </motion.div>
              </div>
