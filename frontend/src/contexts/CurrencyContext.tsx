...
export const CurrencyProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [isAutoDetected] = useState<boolean>(() => {
    const storedAutoDetected = localStorage.getItem('currencyAutoDetected');
    return storedAutoDetected === 'true';
  });
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [fromCurrency] = useState<Currency>('INR'); // Default from currency
  const [toCurrency] = useState<Currency>('AED'); // Default to currency
