...
export const getStripeConfig = (): StripeConfig => {
  ...
  if (vendorKey && validateStripeKey(vendorKey).isValid) {
    ...
    logger.debug('Using vendor Stripe key - no fee applied');
  ...
  if (!validation.isValid) {
    logger.error('Stripe configuration error:', validation.errors);
    // Fallback to test key if live key is invalid
    if (keyType === 'live') {
      logger.warn('Falling back to test key due to invalid live key');
...
export const logStripeConfiguration = (): void => {
  ...
  logger.group('🔐 Stripe Configuration');
  logger.debug('Environment:', config.environment);
  logger.debug('Key Type:', config.keyType);
  logger.debug('Live Mode:', config.isLiveMode);
  logger.debug('Region:', config.region);
  logger.debug('Compliance Mode:', config.complianceMode);
  logger.debug('Public Key:', config.publicKey.substring(0, 20) + '...');

  if (validation.warnings.length > 0) {
    logger.warn('Warnings:', validation.warnings);
  }
  ...
  if (mismatchWarning) {
    logger.warn(mismatchWarning);
  }

  if (config.shouldShowWarning) {
    logger.warn('Payment warnings are enabled for this configuration');
  }
