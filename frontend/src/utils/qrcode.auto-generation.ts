...
export const autoGenerateQRForBooking = async (
  ...
  try {
    ...
    if (!bookingId) {
      logger.warn('Cannot generate QR: booking ID missing');
      return false;
    }

    dispatch(generateQRCodesForBooking(bookingId));
    logger.debug(`Auto-generated QR codes for booking ${bookingId}`);
    return true;
  } catch (error) {
    logger.error(`Error auto-generating QR codes for booking: ${booking.id}`, error);
    return false;
  }
};
...
export const batchProcessQRGeneration = async (
  ...
  for (let i = 0; i < confirmedBookings.length; i += batchSize) {
    ...
    const promises = batch.map(async (booking) => {
      try {
        ...
        logger.error(`Batch processing error for booking: ${booking.id}`, error);
        return 'error';
      }
    });
  ...
  logger.debug(`Batch QR generation completed: ${generated} generated, ${errors} errors`);
...
export const handleBookingStatusChange = (
  ...
  if (!wasConfirmed && isNowConfirmed) {
    ...
    logger.debug(`Booking ${newBooking.id} confirmed - auto-generating QR codes`);
  }
};
