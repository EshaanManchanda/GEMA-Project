...
const processPayment: (...args: any[]) => any = (data: any) => async () =>
  ({ type: 'payments/processPayment/fulfilled', payload: data });
const refundPayment: (...args: any[]) => any = (data: any) => async () =>
  ({ type: 'payments/refundPayment/fulfilled', payload: data });
...
describe('API Integration Tests', () => {
  ...
  describe('Authentication API Integration', () => {
    it('should handle user registration with MongoDB integration', async () => {
      ...
      expect((result.payload as any).user.email).toBe('admin@gema.com');
    });
    ...
    it('should handle user login with proper token management', async () => {
      ...
      expect((result.payload as any).user.role).toBe('admin');
      expect((result.payload as any).token).toBe('mock-jwt-token');
    });
  ...
  describe('Booking API Integration', () => {
    it('should create booking with seat allocation', async () => {
      ...
      const bookingData = {
        eventId: mockMongoData.event._id,
        participants: [{ id: 'p1', name: 'Test User', age: 10 }] as any[],
        paymentMethod: 'card'
      };
      ...
      expect((result.payload as any).tickets).toBe(2);
      expect((result.payload as any).status).toBe('confirmed');
    });
    ...
    it('should handle booking updates and cancellations', async () => {
      ...
      const updateData = {
        id: 'booking123',
        updates: { status: 'cancelled' as const, cancellationReason: 'User requested cancellation' }
      };

      const result = await store.dispatch(updateBooking(updateData));

      expect(result.type).toBe('bookings/updateBooking/fulfilled');
      expect((result.payload as any).status).toBe('cancelled');
    });
  });
  ...
  describe('Real-time API Integration', () => {
    it('should handle WebSocket notifications', async () => {
      ...
      expect((result.payload as any).type).toBe('booking_confirmed');
    });
  });
