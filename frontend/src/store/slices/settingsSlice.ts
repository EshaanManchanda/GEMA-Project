...
export const fetchUISettings = createAsyncThunk(
  'settings/fetchUISettings',
  async (_, { rejectWithValue }) => {
    try {
      const response = await settingsAPI.getUISettings();
      return response.data as { animationsEnabled: boolean };
    } catch (error: any) {
      const message = error.response?.data?.message || 'Failed to fetch UI settings';
      return rejectWithValue(message);
    }
  }
);
...
export const selectAnimationsEnabled = (state: RootState) => state.settings.animationsEnabled;
