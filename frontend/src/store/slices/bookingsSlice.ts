...
interface BookingsState {
  ...
  bookingFlow: {
    ...
    /** ISO date string of the user-selected date (e.g. "2026-03-16"), persisted across navigation */
    selectedDate: string | null;
    /** Snapshot of the schedule the user selected (for correct display in BookingConfirmation) */
    ...
    /** Index of the selected timeSlot within the schedule (-1 = base schedule, no slot) */
    selectedTimeSlotIndex: number;
    /** Locked-in unit price at time of selection (avoids stale price on confirmation) */
    lockedUnitPrice: number | null;
    /** Coupon discount amount confirmed by backend */
...
const generateBookingQRCodes = (booking: Booking): Booking => {
  if (booking.status === 'confirmed' || booking.status === 'completed') {
    try {
      logger.debug('🔄 Generating QR codes with event-based expiration for booking:', booking.bookingNumber);
      ...
      logger.debug('✅ QR codes generated successfully with event-based expiration');
      ...
      logger.error(`❌ Failed to generate QR codes for booking: ${booking.id}`, error);
      return booking;
    }
  }
  return booking;
...
const bookingsSlice = createSlice({
  ...
  reducers: {
    ...
    setBookingDateSelection: (state, action: PayloadAction<{
      scheduleId: string;
      selectedDate: string;
      scheduleSnapshot: BookingsState['bookingFlow']['selectedScheduleSnapshot'];
      timeSlotIndex?: number;
      unitPrice: number;
    }>) => {
      const { scheduleId, selectedDate, scheduleSnapshot, timeSlotIndex = -1, unitPrice } = action.payload;
      state.bookingFlow.scheduleId = scheduleId;
      state.bookingFlow.selectedDate = selectedDate;
      state.bookingFlow.selectedScheduleSnapshot = scheduleSnapshot;
      state.bookingFlow.selectedTimeSlotIndex = timeSlotIndex;
      state.bookingFlow.lockedUnitPrice = unitPrice;
    },

    setCouponDiscount: (state, action: PayloadAction<number>) => {
      state.bookingFlow.couponDiscountAmount = action.payload;
    },
