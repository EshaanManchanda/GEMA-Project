...
export const updateUserStatus = createAsyncThunk(
  ...
  async (params: {
    ...
    try {
      const response = await adminAPI.updateUserStatus(params.userId, params.status);
...
export const fetchAdminVendors = createAsyncThunk(
  ...
  async (params: {
    ...
    try {
      const response = await adminAPI.getAllVendorProfiles(params);
      return response;
    } catch (error: any) {
...
export const approveEvent = createAsyncThunk(
  ...
  async (params: {
    ...
    try {
      const response = await adminAPI.approveEvent(params.eventId);
...
const adminSlice = createSlice({
  ...
  reducers: {
    ...
    updateEventInList: (state, action: PayloadAction<Event>) => {
      ...
      const pendingIndex = state.pendingEvents.findIndex(event => event._id === action.payload._id);
      if (pendingIndex !== -1) {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchAdminVendors.fulfilled, (state, action) => {
        ...
        state.vendors = p.vendors;
        state.pendingVendorApplications = (p.vendors || []).filter(
      ...
      .addCase(approveVendorApplication.fulfilled, (state, action: PayloadAction<any>) => {
        adminSlice.caseReducers.updateVendorInList(state, action);
      })
      .addCase(rejectVendorApplication.fulfilled, (state, action: PayloadAction<any>) => {
        adminSlice.caseReducers.updateVendorInList(state, action);
      })
