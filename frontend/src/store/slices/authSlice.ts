  ...
  async (_, { rejectWithValue: _rejectWithValue }) => {
    try {
      ...
      try {
        ...
        logger.debug('[Auth] Silent logout - no valid session to clear');
      }
...
export const resetPassword = createAsyncThunk(
  'auth/resetPassword',
  async ({ email, token, password }: { email: string; token: string; password: string }, { rejectWithValue }) => {
    try {
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(verifyPhoneOTP.fulfilled, (state, _action) => {
        if (state.user) {
