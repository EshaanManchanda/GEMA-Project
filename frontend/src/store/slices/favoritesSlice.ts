...
export const toggleFavorite = createAsyncThunk(
  ...
  async (eventId: string, { getState, dispatch, rejectWithValue }) => {
    try {
      ...
      if (isFavorite) {
        ...
        await favoritesAPI.addToFavorites(eventId);
        toast.success('Added to favorites!');
        // Refetch to get full event data (slug, images, etc.) in Redux state
...
const favoritesSlice = createSlice({
  ...
  reducers: {
    ...
    filterFavoritesByCategory: (_state, _action: PayloadAction<string | null>) => {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(addToFavorites.fulfilled, (state, _action: PayloadAction<string>) => {
...
export const selectFavoritesByLocation = (city: string) => (state: { favorites: FavoritesState }) => {
  return state.favorites.items.filter(item =>
    (item.location as any)?.city?.toLowerCase().includes(city.toLowerCase())
  );
};
