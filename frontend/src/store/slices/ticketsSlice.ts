...
export const generateMissingTickets = createAsyncThunk(
  ...
  async (orderId: string, { rejectWithValue }) => {
    try {
      logger.debug('🔧 Redux generateMissingTickets - Order ID:', orderId);
      const response = await ticketAPI.generateMissingTickets(orderId);
      logger.debug('🔧 Redux generateMissingTickets - API Response:', response);
      return response.data.tickets;
    } catch (error: any) {
      logger.error('🔧 Redux generateMissingTickets - Error:', error);
      return rejectWithValue(error.message);
    }
  }
);
