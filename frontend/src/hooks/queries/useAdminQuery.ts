 ...
 * @param params - Filter parameters (status, category, vendor, etc.)
 * @example
 * const { data, isLoading } = useAdminEventsQuery({ status: 'pending' });
 */
...
export function useAdminPendingEventsQuery(options?: Omit<UseQueryOptions<any>, 'queryKey' | 'queryFn'>) {
  return useQuery({
    ...
    queryFn: () => adminAPI.getAllEvents({ isApproved: false }),
