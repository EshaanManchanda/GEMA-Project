...
export function useTrafficOverviewQuery(days = 30, options?: Omit<UseQueryOptions<any>, 'queryKey' | 'queryFn'>) {
  return useQuery({
    queryKey: adminKeys.traffic.overview(days),
    queryFn: () => adminAPI.getTrafficOverview(days),
    staleTime: 5 * 60 * 1000,
    gcTime: 15 * 60 * 1000,
    ...options,
  });
...
export function useTrafficReferrersQuery(days = 30, options?: Omit<UseQueryOptions<any>, 'queryKey' | 'queryFn'>) {
  return useQuery({
    queryKey: adminKeys.traffic.referrers(days),
    queryFn: () => adminAPI.getTrafficReferrers(days),
    staleTime: 5 * 60 * 1000,
    gcTime: 15 * 60 * 1000,
    ...options,
  });
