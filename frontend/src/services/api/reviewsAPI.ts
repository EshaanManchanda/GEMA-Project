...
const reviewsAPI = {
  ...
  createReview: async (reviewData: {
    type: 'event' | 'vendor' | 'venue' | 'teacher';
    targetId: string;
    rating: number;
    title?: string;
    comment?: string;
  ...
  // Check booking + review status across all events by a teacher
  checkTeacherReviewStatus: async (teacherUserId: string) => {
    try {
      const response = await ApiService.get(`/reviews/check-teacher-status/${teacherUserId}`);
      // ApiService.get already returns response.data (the full JSON body: { success, data: {...} })
      // so `response.data` is the inner data object, not response.data.data
      return response.data as {
        hasBooked: boolean;
        hasReviewed: boolean;
        reviewableTarget: string | null;  // Teacher's user ID to submit review against
        review: any | null;
      };
    } catch (error) {
      throw error;
  ...
  // Fetch all teacher-type reviews for a teacher — dedicated endpoint, no type-enum issues
  getTeacherReviews: async (teacherUserId: string, params?: { page?: number; limit?: number }) => {
    try {
      const response = await ApiService.get(`/reviews/by-teacher/${teacherUserId}`, { params });
      // ApiService.get already returns response.data (the full JSON body: { success, data: { reviews, pagination } })
      // so `response.data` is the inner data object containing reviews
      return response.data as {
        reviews: any[];
        pagination: any;
      };
