...
const eventsAPI = {
  ...
  claimEvent: async (id: string) => {
    try {
      const response = await ApiService.post(`/events/${id}/claim`);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  // Aliases and additional methods used by eventsSlice thunks
  ...
  getRelatedEvents: async (eventId: string, limit: number = 4) => {
    try {
      const response = await ApiService.get('/events', { params: { limit, exclude: eventId } });
      const events = extractEventsData(response);
      return { events };
    } catch (error) {
      throw error;
  ...
  getEventsByLocation: async (params: any) => {
    try {
      const response = await ApiService.get('/events', { params });
      return extractEventsWithPagination(response);
    } catch (error) {
  ...
  getEventsByVendor: async (params: any) => {
    try {
      const response = await ApiService.get('/events/vendor/my-events', { params });
      return extractEventsWithPagination(response);
    } catch (error) {
