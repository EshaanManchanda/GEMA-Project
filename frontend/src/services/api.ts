...
if (import.meta.env.DEV && typeof window !== 'undefined') {
  ...
  logger.debug('- Runtime API_BASE_URL:', API_BASE_URL);
  logger.debug('- Current Domain:', window.location.hostname);
  logger.debug('- Mode:', import.meta.env.MODE);
  logger.debug('- Is Dev:', import.meta.env.DEV);
  logger.debug('- Build Time:', __BUILD_TIME__);
  logger.debug('- Cache Bust ID:', __CACHE_BUST__);
  logger.debug('- Window Origin:', window.location.origin);
  logger.groupEnd();
}
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        logger.error('[API] Token refresh failed - Clearing auth state:', refreshError);
        ...
        if (!isPublicRoute && !currentPath.includes('/login')) {
          logger.debug('[API] On protected route - Redirecting to login');
          window.location.href = '/login';
        } else if (isPublicRoute) {
          logger.debug('[API] On public route - Allowing access without redirect');
        }
    ...
    if (error.response?.status === 400 || error.response?.status === 422) {
      ...
      logger.debug('[API] Validation error - letting component handle display');
      return Promise.reject(error);
    }
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ERR_NETWORK' || error.code === 'ECONNABORTED') {
      ...
      logger.error(`[API] Connection ${isTimeout ? 'timeout' : 'refused'} - Backend may be waking up:`, error.message, `Timeout: ${timeoutValue}ms`);
    ...
    if (error.response?.status === 503 && error.config?.url?.includes('/api/')) {
      logger.error('[API] Proxy error - API requests may be misconfigured');
    ...
    if (originalRequest._retryCount < RETRY_ATTEMPTS) {
      ...
      logger.debug(`API call failed, retrying in ${Math.round(totalDelay)}ms (attempt ${originalRequest._retryCount}/${RETRY_ATTEMPTS}) - Status: ${error.response?.status}, URL: ${originalRequest.url}`);
