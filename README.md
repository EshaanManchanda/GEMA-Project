...
Two GitHub Actions workflows, both under `.github/workflows/`.

### `ci.yml` — PR gate

Runs on every pull request into `main` or `dev`. Must pass before merge (once branch
protection is enabled). Jobs:

| Job | What it does |
|---|---|
| `backend-quality` | `cd backend` → `npm ci` → lint → typecheck → test |
| `frontend-quality` | `cd frontend` → `npm ci` → lint → type-check → test |
| `build` | Builds both backend and frontend (needs both quality jobs to pass first) |
| `security` | `npm audit --audit-level=high` (both packages), `dependency-review-action`, `gitleaks` secret scan |

Node 22 everywhere (matches frontend `engines`/`.nvmrc`; backend accepts `>=20 <26`).
`npm audit` currently runs non-blocking (`|| true`) for a first baseline pass — flip to
blocking once known issues are triaged. Secret-scan false positives (fake creds in
`.env.example`, test fixtures) are allowlisted in `.gitleaks.toml`.

### `deploy.yml` — deploy pipeline

Runs on `push` to `main` (→ production) or `dev` (→ staging). Flow:

```
lint (+ typecheck) → test → build → deploy-staging / deploy-production → health-check
```

- **Staging** (`dev` branch): SSH into VPS, pull, rebuild, `pm2 restart`.
- **Production** (`main` branch): same, plus:
  - Backs up current `dist/` folders and takes a `mongodump` snapshot to
    `/var/backups/kidrove/` (outside the web root, `chmod 700`, keeps last 3) before
    pulling new code. No automatic destructive DB migrations.
  - Gated by the GitHub `production` Environment — requires manual reviewer approval
    before the SSH deploy step runs.
  - `pm2 reload` (zero-downtime) instead of `restart`.
- **Health check**: probes `/health/basic` first (lightweight), waits, then
  `/api/health` (full DB + Redis + queue check). Prints a rollback hint on failure.
- A `concurrency` group prevents two deploys to the same branch overlapping.

### One-time repo setup (manual, not in the workflow files)

- Branch protection on `main`/`dev`: require the `ci.yml` jobs to pass, block direct pushes.
- GitHub Environments: `staging` (secrets `VPS_HOST_STAGING`, `VPS_USER`, `VPS_SSH_KEY`,
  `VPS_PORT`) and `production` (same + `VPS_HOST_PROD` + `PROD_MONGODB_URI`, with required
  reviewers turned on).
