...
async function migrateCloudinaryUrls() {
  try {
    ...
    console.log(`📊 Found ${cloudinaryAssets.length} Cloudinary assets to migrate\n`);

    if (cloudinaryAssets.length === 0) {
      console.log('✅ No assets need migration. All assets already use UUID URLs.');
    ...
    for (const asset of cloudinaryAssets) {
      try {
        ...
        if (!asset.uuid) {
          console.warn(`⚠️  Asset ${asset._id} (${asset.filename}) missing UUID, skipping...`);
          failed++;
          errors.push({
        ...
        console.error(`  ❌ Failed to migrate ${asset._id} (${asset.filename}):`, error.message);
        failed++;
        errors.push({
          id: asset._id.toString(),
          filename: asset.filename,
    ...
    console.log('\n📋 VALIDATION:');
    console.log(`${remainingDirectUrls} Cloudinary assets still have direct URLs`);

    if (remainingDirectUrls === 0) {
      console.log('✅ All Cloudinary assets successfully migrated to UUID URLs!');
    } else {
      console.warn(`⚠️  ${remainingDirectUrls} assets still need migration (likely missing UUIDs)`);
    }
