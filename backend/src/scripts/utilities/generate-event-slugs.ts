  ...
  try {
    ...
    for (const event of eventsWithoutSlugs) {
      try {
        ...
        successCount++;
        console.log(
          `✅ [${successCount}/${eventsWithoutSlugs.length}] Generated slug for event: "${event.title}"`,
        );
        ...
        console.error(
          `❌ [${successCount + errorCount}/${eventsWithoutSlugs.length}] Failed to generate slug for event: "${event.title}"`,
        );
