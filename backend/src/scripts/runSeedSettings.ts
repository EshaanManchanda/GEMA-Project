...
const runSeed = async () => {
  try {
    ...
    // Connect to MongoDB
    const mongoUri = config.mongodbUri || process.env.MONGODB_URI || 'mongodb://localhost:27017/gema';
    ...
    console.log(`   Minimum Payout: ${settings.minimumPayoutAmount} ${settings.payoutCurrency}`);
