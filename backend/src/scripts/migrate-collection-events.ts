...
async function migrateCollectionEvents(dryRun: boolean = true) {
  try {
    ...
    logger.info(`Mode: ${dryRun ? 'DRY RUN (no changes)' : 'LIVE (will modify data)'}\n`);

    // Find collections needing migration
    const collections = await Collection.find({
      $or: [
        { eventsData: { $exists: false } },
        { eventsData: { $size: 0 } }
      ],
    ...
    for (const collection of collections) {
      try {
        ...
        if (dryRun) {
          logger.info(`  [DRY RUN] Would sync ${collection.events.length} events`);
          stats.migrated++;
        } else {
          // Use sync service to populate eventsData
          const syncedCount = await collectionSyncService.syncCollection(collection._id.toString());
