...
async function investigateEvent() {
  try {
    ...
    console.log(`isApproved:    ${event.isApproved}    ${event.isApproved ? '✓' : '❌ FAILING'}`);
    console.log(`isActive:      ${event.isActive}      ${event.isActive ? '✓' : '❌ FAILING'}`);
    console.log(`status:        ${event.status}        ${event.status === 'published' ? '✓' : '❌ FAILING (expected: published)'}`);
    console.log(`isDeleted:     ${event.isDeleted}     ${!event.isDeleted ? '✓' : '❌ FAILING'}`);
    console.log();
    ...
    if (!event.dateSchedule || event.dateSchedule.length === 0) {
      ...
      event.dateSchedule.forEach((schedule: any, index: number) => {
        ...
        if (scheduleDate) {
          ...
          console.log(`  Status:         ${scheduleDate >= bufferTime ? '✓ VALID (within buffer)' : '❌ EXPIRED (beyond buffer)'}`);
        }
        if (startDate) {
        ...
        if (endDate) {
          ...
          console.log(`  Status:         ${endDate >= bufferTime ? '✓ VALID (within buffer)' : '❌ EXPIRED (beyond buffer)'}`);
        }
      ...
      if (latestEndDateInSchedule) {
        const expirationDate = new Date(latestEndDateInSchedule.getTime() + 24 * 60 * 60 * 1000);
        const isExpired = now > expirationDate;
        const timeDiff = expirationDate.getTime() - now.getTime();
        const daysDiff = timeDiff / (1000 * 60 * 60 * 24);

        console.log('EXPIRATION SUMMARY:');
        console.log(`  Latest end date:        ${latestEndDateInSchedule.toISOString()}`);
        console.log(`  24h buffer expiration:  ${expirationDate.toISOString()}`);
        console.log(`  Current time:           ${now.toISOString()}`);
        console.log(`  Is expired?             ${isExpired ? '❌ YES (beyond buffer)' : '✓ NO (within buffer)'}`);
        console.log(`  Days ${isExpired ? 'since' : 'until'} expiration: ${Math.abs(daysDiff).toFixed(2)} days`);
        console.log();
      }
    }
    ...
    console.log('-'.repeat(80));
    const publicFilter = buildPublicEventFilter({ _id: new mongoose.Types.ObjectId(EVENT_ID) });
    ...
    if (publicEvent) {
      ...
      console.log('  This is why the event returns "Event not found or expired"');
    }
    console.log();
    ...
    if (event.status !== 'published') {
      failures.push(`Event status is "${event.status}" (expected: "published")`);
    }
    if (event.isDeleted) {
    ...
    if (failures.length === 0) {
      ...
      console.log('  The problem might be with the server not having the latest code');
    ...
    console.log('-'.repeat(80));
    if (!event.isApproved) {
      console.log('→ To approve: db.events.updateOne({ _id: ObjectId("' + EVENT_ID + '") }, { $set: { isApproved: true } })');
    }
    if (!event.isActive) {
      console.log('→ To activate: db.events.updateOne({ _id: ObjectId("' + EVENT_ID + '") }, { $set: { isActive: true } })');
    }
    if (event.status !== 'published') {
      console.log('→ To publish: db.events.updateOne({ _id: ObjectId("' + EVENT_ID + '") }, { $set: { status: "published" } })');
    }
    if (event.isDeleted) {
      console.log('→ To restore: db.events.updateOne({ _id: ObjectId("' + EVENT_ID + '") }, { $set: { isDeleted: false } })');
    }

    const latestEndDateForFix = event.getEndDate();
    if (latestEndDateForFix) {
      const nowForFix = new Date();
      const bufferTimeForFix = new Date(nowForFix.getTime() - 24 * 60 * 60 * 1000);
      const hasValidDate = event.dateSchedule.some((schedule: any) => {
        const dateToCheck = schedule.endDate || schedule.date;
      ...
      if (!hasValidDate) {
        ...
        console.log('→ To extend dates: Update dateSchedule.endDate to a future date (e.g., ' + newDate.toISOString() + ')');
      }
    }

    if (failures.length === 0) {
      console.log('→ Restart the backend server to load the new expiration logic code');
