...
import Vendor, { PaymentMode, VendorSubscriptionStatus, VerificationStatus } from '../models/Vendor';
  ...
  try {
    ...
    const vendorData = {
      ...
      paymentSettings: {
        ...
        stripeSettings: {
          // Stripe Connect
          stripeConnectAccountId: user.vendorPaymentSettings?.stripeAccountId || undefined,
          stripeConnectOnboardingComplete: Boolean(user.vendorPaymentSettings?.stripeAccountId),
          ...
          stripePublishableKey: user.vendorPaymentSettings?.stripePublishableKey || undefined,
          stripeSecretKey: user.vendorPaymentSettings?.stripeSecretKey ||
                          user.vendorPaymentSettings?.stripeApiKey || undefined,
          stripeTestMode: !user.vendorPaymentSettings?.stripePublishableKey?.startsWith('pk_live_'),
        ...
        subscriptionAmount: user.vendorPaymentSettings?.subscriptionAmount || 150,
        subscriptionStartDate: user.vendorPaymentSettings?.subscriptionActive
          ? user.createdAt
          : undefined,
        subscriptionPaidUntil: user.vendorPaymentSettings?.subscriptionPaidUntil || undefined,
        subscriptionHistory: [],

        // Payout settings
        payoutSchedule: user.vendorPaymentSettings?.payoutSchedule || 'weekly',
        minimumPayout: user.vendorPaymentSettings?.minimumPayout || 50,
        preferredPayoutMethod: user.vendorPaymentSettings?.preferredPayoutMethod || 'bank_transfer',
        bankAccountDetails: user.vendorPaymentSettings?.bankAccountDetails ? {
          accountHolderName: user.vendorPaymentSettings.bankAccountDetails.accountHolderName,
          bankName: user.vendorPaymentSettings.bankAccountDetails.bankName,
          accountNumber: user.vendorPaymentSettings.bankAccountDetails.accountNumber,
          routingNumber: user.vendorPaymentSettings.bankAccountDetails.routingNumber,
          iban: user.vendorPaymentSettings.bankAccountDetails.iban,
          swiftCode: user.vendorPaymentSettings.bankAccountDetails.swiftCode,
          isVerified: false
        } : undefined,

        // Flags
        acceptsPlatformPayments: user.vendorPaymentSettings?.acceptsPlatformPayments !== false,
      ...
      suspensionReason: user.status === 'suspended' ? 'Migrated from suspended user account' : undefined,
...
async function runMigration(): Promise<void> {
  ...
  try {
    ...
    const vendorUsers = await User.find({ role: UserRole.VENDOR })
      .select('+vendorPaymentSettings.stripeSecretKey +vendorPaymentSettings.stripeApiKey')
      .lean();
    ...
    for (let i = 0; i < vendorUsers.length; i++) {
      ...
      console.log(`[${i + 1}/${stats.totalVendors}] Migrating ${user.email}...`);
    ...
    if (stats.failed === 0) {
      ...
      console.log('   3. Update CommissionTransaction model to reference Vendor');
      ...
      console.log('   7. Update statistics in vendor documents (run calculateStats script)');
    } else {
      console.log('⚠️  Some vendors failed to migrate. Please review errors above.');
    }
