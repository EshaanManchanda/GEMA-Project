...
async function migrateEventImages() {
  try {
    ...
    const events = await Event.find({
      ...
      $or: [{ imageAssets: { $exists: false } }, { imageAssets: { $size: 0 } }],
    });
    ...
    if (events.length === 0) {
      console.log(
        "No events to migrate. All events are already using MediaAssets or have no images.",
      );
    ...
    for (const event of events) {
      try {
        ...
        if (imageAssetIds.length > 0) {
          ...
          console.log(
            `  ✅ Migrated ${eventImagesMigrated} images successfully`,
          );
          if (eventImagesSkipped > 0) {
            console.log(
              `  ⚠  Skipped ${eventImagesSkipped} images (no matching MediaAsset)`,
            );
          }
        ...
        console.error(
          `  ❌ Error migrating event ${event._id}:`,
          error.message,
        );
        skipped++;
        errors.push({
