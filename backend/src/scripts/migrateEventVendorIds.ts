      ...
      try {
        ...
        if (user && user.role === 'vendor') {
          ...
          if (vendor) {
            ...
            await event.save();
            console.log(`✅ Updated event ${event._id} (${event.title}) - User ${user._id} → Vendor ${vendor._id}`);
            updatedCount++;
          } else {
            console.log(`⚠️  Event ${event._id} (${event.title}) - User ${user._id} found but no Vendor profile exists`);
            skippedCount++;
          }
        } else {
          ...
          if (vendor) {
            console.log(`✓  Event ${event._id} (${event.title}) - Already has correct Vendor._id`);
            skippedCount++;
          } else {
            console.log(`❌ Event ${event._id} (${event.title}) - vendorId ${event.vendorId} not found as User or Vendor`);
            errorCount++;
          }
        }
      } catch (error) {
