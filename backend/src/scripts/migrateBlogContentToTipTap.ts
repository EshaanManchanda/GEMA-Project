  ...
  try {
    ...
    for (const blog of blogs) {
      try {
        ...
        if (!content.includes('<')) {
          console.log(`  ⚠️  Blog "${blog.title}" has plain text content - wrapping in paragraphs`);
        ...
        // Check if content has minimal HTML structure (needs proper wrapping)
          ...
          console.log(`  ⚠️  Blog "${blog.title}" has minimal HTML - wrapping content`);
          content = `<p>${content}</p>`;
          needsMigration = true;
        }
        ...
        if (needsMigration) {
          ...
          skippedCount++;
          console.log(`  ⏭️  Skipped blog: "${blog.title}" (already has proper HTML)`);
        }
      } catch (error) {
