...
async function seedBanners() {
  try {
    ...
    if (!adminUser) {
      console.error(
        "❌ No admin user found. Please run main seed script first (npm run db:seed)",
      );
      process.exit(1);
    }
    ...
    for (const img of bannerImages) {
      try {
        console.log(
          `  ⬆️  Uploading: ${img.title} from ${img.url.substring(0, 50)}...`,
        );
        ...
        console.log(
          `  ✅ Created MediaAsset: ${asset.filename} (UUID: ${uuid})`,
        );
      } catch (error: any) {
    ...
    console.log(
      `✅ Successfully uploaded and created ${mediaAssets.length}/${bannerImages.length} media assets`,
    );
    ...
    const banners = [
      {
        ...
        description:
          "Find the perfect events and activities for your children across UAE",
      ...
      },
      {
        ...
        description:
          "Secure your spot at the most popular kids events in Dubai and Abu Dhabi",
    ...
    createdBanners.forEach((banner, index) => {
      console.log(
        `   ${index + 1}. "${banner.title}" (${banner.status}, order: ${banner.displayOrder})`,
      );
    });
