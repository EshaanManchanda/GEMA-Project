...
async function migrateCollectionMedia() {
  try {
    ...
    console.log(
      `Found ${collections.length} collections to potentially migrate`,
    );
    ...
    for (const collection of collections) {
      ...
      if (collection.icon && !collection.iconAsset) {
        ...
        if (asset) {
          ...
          console.log(
            `⚠ No MediaAsset found for icon URL in collection: ${collection.title}`,
          );
        }
      }
      ...
      if (collection.featuredImage && !collection.featuredImageAsset) {
        ...
        if (asset) {
          ...
          featuredImageCount++;
          console.log(
            `✓ Migrated featured image for collection: ${collection.title}`,
          );
        } else {
          console.log(
            `⚠ No MediaAsset found for featured image URL in collection: ${collection.title}`,
          );
        }
      }
