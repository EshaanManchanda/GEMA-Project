...
async function checkDependencies(): Promise<{ safe: boolean; warnings: string[] }> {
  ...
  try {
    ...
    if (vendorField) {
      ...
      if (eventsCount > 0) {
        warnings.push(`⚠️  ${eventsCount} events may reference Vendor model. Update Event model first!`);
      }
    }
  } catch (error) {
