...
async function seedSEOContent() {
  try {
    ...
    if (!adminUser) {
      throw new Error(
        "Admin user not found. Please run main seed script first (npm run db:seed)",
      );
    }
    console.log(
    ...
    console.log(
      `✅ Successfully created ${createdContent.length} SEO content items:`,
    );

    createdContent.forEach((content, index) => {
      console.log(`   ${index + 1}. ${content.page} - "${content.metaTitle}"`);
      console.log(
        `      Keywords: ${content.keywords.length} | FAQs: ${content.faqItems.length} | Features: ${content.features.length}`,
      );
    });
