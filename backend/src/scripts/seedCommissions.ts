...
import { ConfigStatus, CommissionRuleType, RecipientType, RuleStatus } from '../models/CommissionConfig';

/**
 * Seed default commission configurations
 */
export const seedCommissionConfigs = async (adminId: mongoose.Types.ObjectId) => {
  try {
    ...
    if (existingCount > 0) {
      console.log(`✓ Commission configurations already exist (${existingCount} found). Skipping seed.`);
      return;
    }
    ...
    console.log(`✅ Successfully seeded ${createdConfigs.length} commission configurations`);
 ...
 */
export const ensureDefaultCommissionConfig = async (adminId?: mongoose.Types.ObjectId) => {
  try {
    ...
    if (existingConfig) {
      ...
      if (!defaultConfig) {
        console.log('⚠️  No default commission config found. Setting first config as default...');
