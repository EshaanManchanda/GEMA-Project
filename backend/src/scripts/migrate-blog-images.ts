...
async function migrateBlogImages() {
  try {
    ...
    if (blogs.length === 0) {
      console.log('No blogs to migrate. All blogs are already using MediaAssets or have no featured images.');
    ...
    for (const blog of blogs) {
      try {
        ...
        if (!mediaAsset) {
          console.warn(`  ⚠ No MediaAsset found for URL: ${blog.featuredImage}`);
        ...
        console.log(`  ✓ Found MediaAsset: ${mediaAsset.filename} (${mediaAsset._id})`);
