...
describe('Vendor Employee Controllers - Unit Tests', () => {
  ...
  describe('getVendorEmployees', () => {
    it('should return employees list with pagination and stats', async () => {
      ...
      (Employee.find as jest.Mock).mockReturnValue(createMockQuery(mockEmployees));
  ...
  describe('createVendorEmployee', () => {
    it('should create new employee and user successfully', async () => {
      ...
      (Employee.findById as jest.Mock).mockReturnValue(createMockQuery(mockEmp));
      (User.findById as jest.Mock).mockResolvedValue({ firstName: 'Test', lastName: 'Vendor' });
  ...
  describe('updateVendorEmployee', () => {
    it('should update employee successfully', async () => {
      ...
      (Employee.findByIdAndUpdate as jest.Mock).mockReturnValue(createMockQuery(updatedEmp));
      ...
      expect(Employee.findByIdAndUpdate).toHaveBeenCalledWith(
        employeeId,
        { $set: expect.objectContaining({ firstName: 'Updated', status: 'inactive' }) },
  ...
  describe('assignEmployeeToEvent', () => {
    it('should assign employee to multiple events with transaction', async () => {
      ...
      const mockEmp = mockEmployee({ _id: employeeId, vendorId, assignedEvents: [] });
      const mockEvents = [
      ...
      (Employee.findById as jest.Mock).mockReturnValue(createMockQuery({
        ...mockEmp,
    ...
    it('should skip already assigned events', async () => {
      ...
      (mongoose.startSession as jest.Mock).mockResolvedValue(mockSession);
      (Employee.findOne as jest.Mock).mockResolvedValue(mockEmp);
      (Event.find as jest.Mock).mockResolvedValue([mockEvent({ _id: eventId1, vendorId })]);
      (Employee.findById as jest.Mock).mockReturnValue(createMockQuery(mockEmp));
    ...
    it('should return error if events not found or not owned by vendor', async () => {
      ...
      const mockSession = mockMongooseSession();
      (mongoose.startSession as jest.Mock).mockResolvedValue(mockSession);
      (Employee.findOne as jest.Mock).mockResolvedValue(mockEmployee({ vendorId }));
      (Event.find as jest.Mock).mockResolvedValue([mockEvent({ _id: eventId1, vendorId })]); // Only 1 found
    ...
    it('should rollback transaction on error', async () => {
      ...
      const mockSession = mockMongooseSession();
      (mongoose.startSession as jest.Mock).mockResolvedValue(mockSession);
      (Employee.findOne as jest.Mock).mockRejectedValue(new Error('Database error'));

      await expect(assignEmployeeToEvent(req, res, next)).rejects.toThrow('Database error');
  ...
  describe('exportVendorEmployees', () => {
    it('should export employees as CSV', async () => {
      ...
      expect(res.send).toHaveBeenCalledWith(expect.stringContaining('Employee ID'));
    });
