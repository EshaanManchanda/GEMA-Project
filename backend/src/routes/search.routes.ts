    ...
    try {
      ...
      if (type === "all" || type === "events") {
        const events = await Event.find({
          ...
          .select(
            "title description price currency location images category type",
          )
        ...
        results.events = sanitizeEventsOutput(events);
      ...
      if (type === "all" || type === "vendors") {
        const vendors = await User.find({
          ...
          .select(
            "firstName lastName email profile.businessName profile.description profile.avatar",
          )
          .limit(searchLimit)
          .sort({ createdAt: -1 });
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      // Date filter
      if (dateFrom || dateTo) {
        filters["dateSchedule.startDate"] = {};
        if (dateFrom)
          filters["dateSchedule.startDate"].$gte = new Date(dateFrom as string);
        if (dateTo)
          filters["dateSchedule.startDate"].$lte = new Date(dateTo as string);
      }

      const events = await Event.find(filters)
        .select(
          "title description price currency location images category type dateSchedule",
        )
        .populate("imageAssets", "url thumbnailUrl variations")
        .skip(skip)
        .limit(searchLimit)
      ...
      res.status(200).json({
        ...
        data: {
          events: sanitizeEventsOutput(events),
          pagination: {
            currentPage: pageNum,
