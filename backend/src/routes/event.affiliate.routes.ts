...
router.post('/vendor/events/:id/claim', authenticate, authorize(['vendor']), claimAffiliateEvent);

// Get vendor's claimed events
router.get('/vendor/claimed-events', authenticate, authorize(['vendor']), getClaimedEvents);

/**
 * Admin routes
 */
// Get global affiliate analytics
router.get('/admin/affiliate-analytics', authenticate, authorize(['admin']), getAffiliateAnalytics);
