 ...
 */
router.get('/api/seo/event/:id/structured-data', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (!event || !event.isActive || !event.isApproved || event.status !== 'published') {
 ...
 */
router.get('/api/seo/blog/:slug/structured-data', async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/api/seo/meta-data/event/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    const event = await Event.findById(id)
      .select('title description seoMeta location category tags images price currency')
      .lean();
 ...
 */
router.get('/api/seo/meta-data/blog/:slug', async (req: Request, res: Response) => {
  try {
    const { slug } = req.params;
 ...
 */
router.get('/api/seo/meta-data/category/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
