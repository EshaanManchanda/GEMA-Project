...
// Single file upload
router.post('/single', authenticate, uploadSingle('file'), handleUploadError, (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
...
router.post('/multiple', authenticate, uploadMultiple('files', 5), handleUploadError, (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
// Event images upload
router.post('/event-images', authenticate, uploadEventImages, handleUploadError, (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    Object.keys(files).forEach(fieldname => {
      uploadedFiles[fieldname] = files[fieldname].map(file => getFileInfo(file));
    });
...
// Venue images upload
router.post('/venue-images', authenticate, uploadVenueImages, handleUploadError, (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const files = req.files as { [fieldname: string]: Express.Multer.File[] };
...
// Document upload
router.post('/document', authenticate, uploadDocument, handleUploadError, (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
...
// Delete file
router.delete('/file/:filename', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const filePath = path.join(process.cwd(), config.upload.path, category as string, filename);
...
// Get file info
router.get('/info/:filename', authenticate, (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { filename } = req.params;
    const { category = 'misc' } = req.query;
...
router.get('/list/:category', authenticate, (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { category } = req.params;
    const { page = 1, limit = 20 } = req.query;
    
    // Validate category
    const allowedCategories = ['events', 'venues', 'users', 'tickets', 'documents', 'misc'];
    if (!allowedCategories.includes(category)) {
      return next(new AppError('Invalid category', 400));
    }
    
    const categoryPath = path.join(process.cwd(), config.upload.path, category);
    ...
    const paginatedFiles = files.slice(startIndex, endIndex).map(filename => {
...
router.get('/transform/:publicId', authenticate, (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { publicId } = req.params;
    const { transformation } = req.query;

    if (config.upload.provider !== 'cloudinary') {
      return next(new AppError('Image transformations are only available with Cloudinary', 400));
    }
...
router.get('/variations/:publicId', authenticate, (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { publicId } = req.params;

    if (config.upload.provider !== 'cloudinary') {
      return next(new AppError('Image variations are only available with Cloudinary', 400));
    }
...
// Batch upload
router.post('/batch', authenticate, uploadMultiple('files', 10), handleUploadError, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.files || !Array.isArray(req.files) || req.files.length === 0) {
      return next(new AppError('No files uploaded', 400));
    }
...
router.get('/enhanced-info/:identifier', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (config.upload.provider === 'cloudinary' && fileInfo.format && ['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(fileInfo.format.toLowerCase())) {
...
router.delete('/enhanced/:identifier', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { identifier } = req.params;
    ...
    if (!deleted) {
      return next(new AppError('Failed to delete file or file not found', 404));
    }
