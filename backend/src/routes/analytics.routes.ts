...
import { toCsv, analyticsToRows, buildExportFilename, safeReportFilename } from "../utils/csv.utils";
import { parseAnalyticsDateRange, lastNDaysRange, formatUtcPeriodLabel } from "../utils/dateHelpers";
...
function eventReportToCsv(report: EventReportData): string {
  ...
  rows.push({ Section: "Report Info", Field: "Event", Value: report.event.title });
  rows.push({ Section: "", Field: "Location", Value: report.event.location });
  rows.push({ Section: "", Field: "Report Period", Value: report.period.label });
  rows.push({ Section: "", Field: "Generated At", Value: report.generated.at });
  rows.push({});

  // ── Section 2: KPIs ──
  rows.push({ Section: "KPIs", Field: `Registrations ${rangeLabel}`, Value: report.kpis.registrations });
  rows.push({ Section: "", Field: `Revenue ${rangeLabel}`, Value: report.kpis.revenue });
  rows.push({ Section: "", Field: `Tickets Sold ${rangeLabel}`, Value: report.kpis.ticketsSold });
  rows.push({ Section: "", Field: `Certificates Issued ${rangeLabel}`, Value: report.kpis.certificatesIssued });
  rows.push({ Section: "", Field: `Reviews ${rangeLabel}`, Value: report.kpis.reviews });
  rows.push({ Section: "", Field: "Total Views (all-time)", Value: report.event.viewsAllTime });
  rows.push({});
  ...
  rows.push({ Section: "", Status: "TOTAL", Count: report.registrations.total });
  rows.push({});

  // ── Section 4: Reviews ──
  rows.push({ Section: "Reviews", Field: "Average Rating", Value: report.reviews.averageRating });
  rows.push({ Section: "", Field: "Total Reviews", Value: report.reviews.total });
  ...
  if (report.reviews.recent.length > 0) {
    rows.push({ Section: "Recent Reviews", Rating: "Rating", Comment: "Comment", Reviewer: "Reviewer", Date: "Date" });
    for (const r of report.reviews.recent) {
      rows.push({ Section: "", Rating: r.rating, Comment: r.comment ?? "", Reviewer: r.userName ?? "", Date: r.date });
    }
    rows.push({});
  }
  ...
  if (report.dailySales.length > 0) {
    rows.push({ Section: "Daily Sales", Date: "Date", Orders: "Orders", Revenue: "Revenue", Tickets: "Tickets" });
    let totOrders = 0, totRevenue = 0, totTickets = 0;
    for (const d of report.dailySales) {
      rows.push({ Section: "", Date: d.date, Orders: d.orders, Revenue: d.revenue, Tickets: d.tickets });
      totOrders += d.orders;
      totRevenue += d.revenue;
      totTickets += d.tickets;
    }
    rows.push({ Section: "", Date: "TOTAL", Orders: totOrders, Revenue: Math.round(totRevenue * 100) / 100, Tickets: totTickets });
  } else {
    rows.push({ Section: "Daily Sales", Date: "No data in this period", Orders: "", Revenue: "", Tickets: "" });
  }
...
function buildEventReportHtml(report: EventReportData): string {
  ...
  const statusTableRows = (byStatus: Record<string, number>) => {
    ...
    if (!entries.length) return `<tr><td colspan="2" class="empty">No data in this period</td></tr>`;
    return entries.map(([s, n]) => `<tr><td>${s}</td><td>${n}</td></tr>`).join("");
  };
  ...
  const dailySalesRows = () => {
    ...
    let totO = 0, totR = 0, totT = 0;
    const rows = report.dailySales.map((d) => {
      totO += d.orders; totR += d.revenue; totT += d.tickets;
      return `<tr><td>${d.date}</td><td>${d.orders}</td><td>${d.revenue.toFixed(2)}</td><td>${d.tickets}</td></tr>`;
    });
    rows.push(`<tr class="totals-row"><td>TOTAL</td><td>${totO}</td><td>${totR.toFixed(2)}</td><td>${totT}</td></tr>`);
    return rows.join("");
  };
  ...
  const recentReviewsSection = () => {
    ...
    return `<table><thead><tr><th>Rating</th><th>Reviewer</th><th>Comment</th><th>Date</th></tr></thead><tbody>
      ${report.reviews.recent.map((r) => `<tr>
...
<div class="header">
  ...
  <div class="meta">
    ${report.event.location} &nbsp;|&nbsp; Event Report &nbsp;|&nbsp; Generated ${report.generated.at.slice(0, 10)}
  </div>
</div>
    ...
    try {
      ...
      if (format === "pdf") {
        ...
        await new Promise<void>((resolve, reject) => {
          pdfReportLimiter(req, res, (err?: any) => (err ? reject(err) : resolve()));
        });

        const cacheKey = `report:event:${eventId}:${range}:pdf`;
        const cached = await cacheService.get<Buffer>(cacheKey);
        if (cached) {
          const buf = Buffer.isBuffer(cached) ? cached : Buffer.from(cached as any);
          const ev = await Event.findById(eventId).select("title").lean<{ title: string }>();
          const title = ev?.title ?? "event";
          const filename = safeReportFilename("event-report", title, range) + ".pdf";
        ...
        const filename = safeReportFilename("event-report", report.event.title, range) + ".pdf";
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `attachment; filename="${filename}"`);
      ...
      const filename = safeReportFilename("event-report", report.event.title, range) + ".csv";
