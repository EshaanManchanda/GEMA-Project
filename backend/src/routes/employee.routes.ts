...
import { createEmployee, getEmployeeDetails, updateEmployee, deleteEmployee, assignEmployeeToEvent, removeEmployeeFromEvent } from '../controllers/employee.controller';
...
router.get('/:employeeId', authorize(['vendor', 'admin', 'employee']), getEmployeeDetails);

// Update Employee (Vendor/Admin, or employee themselves)
router.put('/:employeeId', authorize(['vendor', 'admin', 'employee']), updateEmployee);
...
router.post('/assign-event', authorize(['vendor', 'admin']), assignEmployeeToEvent);

// Remove Employee from Event (Vendor/Admin only)
router.post('/remove-event', authorize(['vendor', 'admin']), removeEmployeeFromEvent);
