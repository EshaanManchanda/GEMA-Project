    ...
    .custom((value) => {
      ...
      if (value.language !== undefined) {
        if (typeof value.language !== "string" || value.language.length > 10) {
          throw new Error(
            "Language code must be a string with max 10 characters",
          );
        }
      }
      ...
      if (value.currency !== undefined) {
        if (typeof value.currency !== "string" || value.currency.length > 10) {
          throw new Error(
            "Currency code must be a string with max 10 characters",
          );
        }
      }
      ...
      if (value.notifications !== undefined) {
        ...
        for (const key of validNotificationKeys) {
          if (
            value.notifications[key] !== undefined &&
            typeof value.notifications[key] !== "boolean"
          ) {
...
export const validateAvatarUpdate = [
  body("avatar")
    ...
    .custom((value) => {
      ...
      const uuidPattern =
        /^\/api\/media\/file\/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i;
      ...
      try {
        ...
        throw new Error(
          "Avatar must be either a UUID-based media URL (/api/media/file/{uuid}) or a full URL with http/https protocol",
        );
      }
    })
    .isLength({ max: 500 })
      ...
      for (const day in value) {
        ...
        if (dayData.isOpen) {
          if (!dayData.openTime || !dayData.closeTime) {
            throw new Error(
              `${day} requires openTime and closeTime when isOpen is true`,
            );
          }
          ...
          if (
            !timeRegex.test(dayData.openTime) ||
            !timeRegex.test(dayData.closeTime)
          ) {
