...
export const validateCouponImport: ValidationChain[] = [
  ...
  body('data.*.validFrom')
    ...
    .isISO8601().withMessage('Valid from must be a valid ISO 8601 date')
    .toDate(),

  // Valid until (required)
  body('data.*.validUntil')
    .notEmpty().withMessage('Valid until date is required')
    .isISO8601().withMessage('Valid until must be a valid ISO 8601 date')
    .toDate(),
  ...
  body('data.*.usageCount')
    .optional()
    .isInt({ min: 0 }).withMessage('Usage count must be non-negative'),
  ...
  body('data.*.firstTimeOnly')
    .optional()
    .isBoolean().withMessage('firstTimeOnly must be a boolean'),

  // ========== APPLICABLE EVENTS ==========

  body('data.*.eventTitles')
    .optional()
    .isArray({ max: 100 }).withMessage('Event titles must be an array with max 100 items'),
  ...
  body('data.*.eventIds')
    .optional()
    .isArray({ max: 100 }).withMessage('Event IDs must be an array with max 100 items'),
  ...
  body('data.*.excludedEventTitles')
    .optional()
    .isArray({ max: 100 }).withMessage('Excluded event titles must be an array'),

  body('data.*.excludedEventTitles.*')
    .optional()
    .isString().withMessage('Each excluded event title must be a string')
    .trim(),

  body('data.*.excludedEventIds')
    .optional()
    .isArray({ max: 100 }).withMessage('Excluded event IDs must be an array'),

  body('data.*.excludedEventIds.*')
    .optional()
    .custom((value) => {
      if (value && !mongoose.Types.ObjectId.isValid(value)) {
        throw new Error('Each excludedEventId must be a valid MongoDB ObjectId');
      }
      return true;
    }),
  ...
  body('data.*.categorySlugs')
    .optional()
    .isArray({ max: 50 }).withMessage('Category slugs must be an array'),

  body('data.*.categorySlugs.*')
    .optional()
    .isString().withMessage('Each category slug must be a string')
    .trim()
    .toLowerCase(),

  body('data.*.categoryIds')
    .optional()
    .isArray({ max: 50 }).withMessage('Category IDs must be an array'),
  ...
  body('data.*.excludedCategorySlugs')
    .optional()
    .isArray({ max: 50 }).withMessage('Excluded category slugs must be an array'),

  body('data.*.excludedCategorySlugs.*')
    .optional()
    .isString().withMessage('Each excluded category slug must be a string')
    .trim()
    .toLowerCase(),

  body('data.*.excludedCategoryIds')
    .optional()
    .isArray({ max: 50 }).withMessage('Excluded category IDs must be an array'),

  body('data.*.excludedCategoryIds.*')
    .optional()
    .custom((value) => {
      if (value && !mongoose.Types.ObjectId.isValid(value)) {
        throw new Error('Each excludedCategoryId must be a valid MongoDB ObjectId');
      }
      return true;
    }),
  ...
  body('data.*.vendorEmails')
    .optional()
    .isArray({ max: 100 }).withMessage('Vendor emails must be an array'),

  body('data.*.vendorEmails.*')
    .optional()
    .isEmail().withMessage('Each vendor email must be valid')
    .normalizeEmail()
    .toLowerCase(),

  body('data.*.vendorIds')
    .optional()
    .isArray({ max: 100 }).withMessage('Vendor IDs must be an array'),
  ...
  body('data.*.excludedVendorEmails')
    .optional()
    .isArray({ max: 100 }).withMessage('Excluded vendor emails must be an array'),

  body('data.*.excludedVendorEmails.*')
    .optional()
    .isEmail().withMessage('Each excluded vendor email must be valid')
    .normalizeEmail()
    .toLowerCase(),

  body('data.*.excludedVendorIds')
    .optional()
    .isArray({ max: 100 }).withMessage('Excluded vendor IDs must be an array'),

  body('data.*.excludedVendorIds.*')
    .optional()
    .custom((value) => {
      if (value && !mongoose.Types.ObjectId.isValid(value)) {
        throw new Error('Each excludedVendorId must be a valid MongoDB ObjectId');
      }
      return true;
    }),
  ...
  body('data.*.applicableEventTypes')
    .optional()
    .isArray({ max: 10 }).withMessage('Applicable event types must be an array'),
  ...
  body('data.*.priceRange')
    .optional()
    .isObject().withMessage('Price range must be an object'),
  ...
  body('data.*.createdByEmail')
    .optional()
    .isEmail().withMessage('Created by email must be valid')
    .normalizeEmail()
    .toLowerCase(),
  ...
  body('data.*.usage')
    .custom((value) => {
      if (value !== undefined) {
        throw new Error('usage array cannot be imported (security). Use usageCount instead.');
      }
      return true;
    }),
];
...
export const validateCouponExport: ValidationChain[] = [
  ...
  body('filters.validFromAfter')
    .optional()
    .isISO8601().withMessage('validFromAfter must be a valid ISO 8601 date')
    .toDate(),

  body('filters.validFromBefore')
    .optional()
    .isISO8601().withMessage('validFromBefore must be a valid ISO 8601 date')
    .toDate(),

  body('filters.validUntilAfter')
    .optional()
    .isISO8601().withMessage('validUntilAfter must be a valid ISO 8601 date')
    .toDate(),

  body('filters.validUntilBefore')
    .optional()
    .isISO8601().withMessage('validUntilBefore must be a valid ISO 8601 date')
    .toDate(),

  // Filter by usage
  body('filters.hasUsageLimit')
    .optional()
    .isBoolean().withMessage('hasUsageLimit must be a boolean'),

  body('filters.minUsageCount')
    .optional()
    .isInt({ min: 0 }).withMessage('minUsageCount must be non-negative'),

  body('filters.maxUsageCount')
    .optional()
    .isInt({ min: 0 }).withMessage('maxUsageCount must be non-negative'),
  ...
  body('includeUsageHistory')
    .optional()
    .isBoolean().withMessage('includeUsageHistory must be a boolean'),
  ...
  body('sortBy')
    .optional()
    .isIn(['code', 'name', 'createdAt', 'updatedAt', 'validFrom', 'validUntil', 'usageCount'])
    .withMessage('sortBy must be one of: code, name, createdAt, updatedAt, validFrom, validUntil, usageCount'),
