...
export const validateEventImport = [
  ...
  body('data.*.type')
    ...
    .isIn(['Olympiad', 'Championship', 'Competition', 'Event', 'Course', 'Venue', 'Workshop'])
    .withMessage('type must be: Olympiad, Championship, Competition, Event, Course, Venue, or Workshop'),
  ...
  body('data.*.ageRange')
    ...
    .isArray({ min: 2, max: 2 })
    .withMessage('ageRange must be an array with exactly 2 elements [min, max]'),
  ...
  body('data.*.dateSchedule.*.startDate')
    .optional()
    .isISO8601()
    .toDate(),

  body('data.*.dateSchedule.*.endDate')
    .optional()
    .isISO8601()
    .toDate(),
  ...
  body('data.*.dateSchedule.*.timeSlots')
    .optional()
    .isArray(),
  ...
  body('data.*.status')
    ...
    .withMessage('status must be: draft, published, archived, pending, or rejected'),

  body('data.*.isFeatured')
    .optional()
    .isBoolean(),

  // Registration config (optional complex object)
  body('data.*.registrationConfig.enabled')
    .optional()
    .isBoolean(),

  body('data.*.registrationConfig.fields')
    .optional()
    .isArray(),

  body('data.*.registrationConfig.requiresApproval')
    .optional()
    .isBoolean(),

  // Affiliate event fields
  body('data.*.isAffiliateEvent')
    .optional()
    .isBoolean(),
