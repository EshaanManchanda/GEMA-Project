...
export const validateUserImport = [
  ...
  body('data.*.addresses.*.state')
    .optional()
    .isString()
    .trim(),

  body('data.*.addresses.*.zipCode')
    .optional()
    .isString()
    .trim(),
  ...
  body('data.*.passwordHash')
    .custom((value) => {
      if (value !== undefined) {
        throw new Error('passwordHash cannot be set directly. Use plaintext password field instead.');
      }
      return true;
    }),

  body('data.*.twoFactorAuth')
    .custom((value) => {
      if (value !== undefined) {
