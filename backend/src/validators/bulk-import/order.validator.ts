...
export const validateOrderImport: ValidationChain[] = [
  ...
  body('data.*.userEmail')
    .optional()
    .isEmail().withMessage('User email must be valid')
    .normalizeEmail()
    .toLowerCase(),
  ...
  body('data.*.orderNumber')
    ...
    .isString().withMessage('Order number must be a string')
    .trim()
    .matches(/^ORD-[A-Z0-9-]+$/)
  ...
  body('data.*.items.*.scheduleDate')
    .notEmpty().withMessage('Schedule date is required for each item')
    .isISO8601().withMessage('Schedule date must be a valid ISO 8601 date')
    .toDate(),
  ...
  body('data.*.items.*.totalPrice')
    .notEmpty().withMessage('Total price is required for each item')
    .isFloat({ min: 0 })
    .withMessage('Total price must be non-negative'),

  body('data.*.items.*.currency')
    .notEmpty().withMessage('Currency is required for each item')
  ...
  body('data.*.items.*')
    .custom((item) => {
      if (item.unitPrice !== undefined && item.quantity !== undefined && item.totalPrice !== undefined) {
        ...
        if (diff > 0.01) {
          throw new Error(`Item totalPrice mismatch: expected ${calculated}, got ${item.totalPrice}`);
        }
      }
      return true;
    }),
  ...
  body('data.*.items.*.participants.*.name')
    .optional()
    .isString().withMessage('Participant name must be a string')
    .trim()
    .notEmpty().withMessage('Participant name cannot be empty'),
  ...
  body('data.*.items.*.participants.*.registrationData.*.fieldType')
    .optional()
    .isString().withMessage('Field type must be a string'),
  ...
  body('data.*')
    .custom((order) => {
      ...
      if (diff > 0.01) {
        throw new Error(`Total amount mismatch: subtotal(${subtotal}) + tax(${tax}) + serviceFee(${serviceFee}) - discount(${discount}) - couponDiscount(${couponDiscount}) = ${calculated}, but total is ${total}`);
      }
      return true;
    }),
  ...
  body('data.*.status')
    ...
    .withMessage('Status must be: pending, confirmed, or draft (not cancelled/refunded)'),

  body('data.*.paymentStatus')
    .optional()
    .isIn(['pending', 'failed'])
    .withMessage('Payment status can only be: pending or failed (not paid to prevent fraud)'),
  ...
  body('data.*.paymentRouting.vendorStripeAccountId')
    .optional()
    .isString().withMessage('Vendor Stripe account ID must be a string'),
  ...
  body('data.*')
    .custom((order) => {
      if (order.paymentRouting) {
        ...
        if (sum > total + 0.01) {
          throw new Error(`Payment routing sum (${sum}) cannot exceed order total (${total})`);
        }
      }
      return true;
    }),
  ...
  body('data.*.accessibilityNeeds.*')
    .optional()
    .isString().withMessage('Each accessibility need must be a string'),
  ...
  body('data.*.dietaryRestrictions.*')
    .optional()
    .isString().withMessage('Each dietary restriction must be a string'),
  ...
  body('data.*.revenueMetadata.commissionSource')
    ...
    .withMessage('Commission source must be: platform_fee, service_fee, or addon_fee'),
  ...
  body('data.*.communications')
    .custom((value) => {
      if (value !== undefined) {
        throw new Error('communications array is auto-generated and cannot be imported');
      }
      return true;
    }),
...
export const validateOrderExport: ValidationChain[] = [
  ...
  body('filters.createdAfter')
    .optional()
    .isISO8601().withMessage('createdAfter must be a valid ISO 8601 date')
    .toDate(),

  body('filters.createdBefore')
    .optional()
    .isISO8601().withMessage('createdBefore must be a valid ISO 8601 date')
    .toDate(),

  // Filter by amount range
  body('filters.minTotal')
    .optional()
    .isFloat({ min: 0 }).withMessage('minTotal must be non-negative'),

  body('filters.maxTotal')
    .optional()
    .isFloat({ min: 0 }).withMessage('maxTotal must be non-negative'),
  ...
  body('includeParticipants')
    .optional()
    .isBoolean().withMessage('includeParticipants must be a boolean'),

  body('includeCommunications')
    .optional()
    .isBoolean().withMessage('includeCommunications must be a boolean'),
  ...
  body('sortBy')
    ...
    .withMessage('sortBy must be one of: orderNumber, createdAt, updatedAt, total, status'),
