...
const SORT_FIELDS = ['createdAt', 'firstName', 'lastName', 'role', 'email', 'hiredAt', 'status'];
...
export const validateEmployeeListQuery = [
  ...
  query('status')
    ...
    .withMessage(`status must be one of: ${EMPLOYEE_STATUSES.join(', ')}, or 'all'`),
  query('assignedEvent')
    .optional()
    .custom((value) => {
    ...
    .custom((permissions) => {
      ...
      for (const perm of permissions) {
        if (typeof perm !== 'object' || !perm.action || !perm.scope) {
          throw new Error('Each permission must have action and scope properties');
        }

        if (!PERMISSION_ACTIONS.includes(perm.action)) {
          throw new Error(`Invalid permission action: ${perm.action}. Must be one of: ${PERMISSION_ACTIONS.join(', ')}`);
        }

        if (!PERMISSION_SCOPES.includes(perm.scope)) {
          throw new Error(`Invalid permission scope: ${perm.scope}. Must be one of: ${PERMISSION_SCOPES.join(', ')}`);
        }
      }
  ...
  body('emergencyContact')
    ...
    .custom((contact) => {
      ...
      if (!contact.name || typeof contact.name !== 'string') {
        throw new Error('Emergency contact name is required and must be a string');
      }

      if (contact.name.length < 2 || contact.name.length > 100) {
        throw new Error('Emergency contact name must be between 2 and 100 characters');
      }

      if (!contact.phone || typeof contact.phone !== 'string') {
        throw new Error('Emergency contact phone is required and must be a string');
      }
...
export const validateUpdateEmployee = [
  ...
  body('emergencyContact')
    ...
    .custom((contact) => {
      ...
      if (contact.name && (typeof contact.name !== 'string' || contact.name.length < 2 || contact.name.length > 100)) {
        throw new Error('Emergency contact name must be between 2 and 100 characters');
      }
