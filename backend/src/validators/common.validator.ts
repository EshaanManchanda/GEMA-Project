...
// Admin pagination validation (higher limits for admin endpoints)
export const validateAdminPagination = [
  query("page")
    .optional()
    .isInt({ min: 1 })
    .withMessage("Page must be a positive integer")
...
export const validatePriceRange = [
  ...
  query("maxPrice")
    ...
    .custom((maxPrice, { req }) => {
      if (
        req.query?.minPrice &&
        maxPrice < parseFloat(req.query.minPrice as string)
      ) {
...
export const validateEmail = (
  ...
  const validator = body(field).trim().toLowerCase();
...
export const validatePhone = (
  ...
  return validator.custom(async (value) => {
    ...
    } = await import("../utils/phoneValidation");
...
export const validateHtmlLength = (
  ...
  return body(field).custom((value) => {
    ...
    if (length < min) {
      throw new Error(
        `${field} must be at least ${min} characters (currently ${length})`,
      );
    }

    if (length > max) {
      throw new Error(
        `${field} must not exceed ${max} characters (currently ${length})`,
      );
    }
