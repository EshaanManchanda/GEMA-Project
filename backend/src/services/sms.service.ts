...
class FirebaseSMSProvider implements SMSProvider {
  ...
  constructor() {
    ...
    try {
      ...
      logger.warn("Firebase Admin not initialized, SMS logs won't be stored");
    }
  }
  ...
  async sendSMS(phone: string, message: string): Promise<SMSSendResult> {
    ...
    if (this.firestore && result.success) {
      try {
        ...
        logger.error("Failed to log SMS to Firestore:", error);
      }
    }
...
class SMSService {
  ...
  private initializeProvider(): SMSProvider {
    ...
    logger.info(`📱 Initializing SMS provider: ${smsProvider}`);
  ...
  async sendVerificationOTP(
    ...
    logger.info(`Sending OTP to ${phone} via ${this.provider.name}`);
