...
class SMSService {
  ...
  private initializeProvider(): SMSProvider {
    ...
    try {
      ...
      console.error(`Failed to initialize ${smsProvider} provider:`, error.message);
   ...
   */
  async sendVerificationOTP(phone: string, otp: string): Promise<SMSSendResult> {
    ...
    try {
      ...
      if (result.success) {
        console.log(`✓ OTP sent successfully to ${phone} (Message ID: ${result.messageId})`);
      } else {
