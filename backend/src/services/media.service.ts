...
export class MediaService {
  ...
  async listMedia(filters: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ assets: IMediaAsset[]; total: number; pages: number; page: number; limit: number }> {
  ...
  async getMediaById(id: string): Promise<IMediaAsset | null> {
    const asset = await MediaAsset.findById(id).populate('uploadedBy', 'firstName lastName email avatar');
  ...
  async deleteMedia(id: string, force: boolean = false): Promise<boolean> {
    ...
    try {
      ...
      if (identifier) {
        ...
        if (!deleted) {
          console.warn(`Failed to delete file from storage: ${identifier}`);
        }
      }
   ...
   */
  async bulkDeleteMedia(ids: string[], force: boolean = false): Promise<{ success: number; failed: number; errors: string[] }> {
    const results = {
      success: 0,
      failed: 0,
      errors: [] as string[]
   ...
   * Update media tags
   */
  async updateMediaTags(id: string, tags: string[]): Promise<IMediaAsset | null> {
   ...
   */
  async findUnusedMedia(category?: string, limit: number = 50): Promise<IMediaAsset[]> {
