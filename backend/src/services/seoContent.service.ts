...
class SEOContentService {
  ...
  async getPublicSEOContent(page: string): Promise<ISEOContent | null> {
    return await SEOContent.findOne({ page, isActive: true }).lean();
  }
  ...
  async getSEOContentByPage(page: string): Promise<ISEOContent | null> {
    return await SEOContent.findOne({ page })
      .populate('updatedBy', 'firstName lastName email');
  }
   ...
   */
  async createSEOContent(data: Partial<ISEOContent>, userId: string): Promise<ISEOContent> {
    // Check if content for this page already exists
    const existing = await SEOContent.findOne({ page: data.page });
    if (existing) {
      throw new AppError(`SEO content for ${data.page} already exists. Use update instead.`, 400);
    }
   ...
   */
  async updateSEOContent(page: string, data: Partial<ISEOContent>, userId: string): Promise<ISEOContent | null> {
