...
class AnnouncementBarService {
  ...
  async getActiveAnnouncements(
    currentRoute?: string,
  ): Promise<IAnnouncementBar[]> {
    const now = new Date();

    logger.info("[getActiveAnnouncements] Querying announcements:", {
      currentRoute,
      now: now.toISOString(),
    });
    ...
    logger.info("[getActiveAnnouncements] ALL announcements in DB:", {
    ...
    logger.info("[getActiveAnnouncements] Found announcements:", {
    ...
    if (currentRoute) {
      return announcements.filter((announcement) => {
        ...
        if (announcement.targetPages === "all") {
          // Check excludePages
          if (
            announcement.excludePages &&
            announcement.excludePages.length > 0
          ) {
        ...
        if (
          announcement.specificPages &&
          announcement.specificPages.length > 0
        ) {
          const matchesSpecific = this.matchesAnyPattern(
            currentRoute,
            announcement.specificPages,
          );
        ...
        return false;
      }) as unknown as IAnnouncementBar[];
    }

    return announcements as unknown as IAnnouncementBar[];
  }
  ...
  async getAnnouncementById(id: string): Promise<IAnnouncementBar | null> {
    return await AnnouncementBar.findById(id).populate(
      "createdBy",
  ...
  async createAnnouncement(
    data: Partial<IAnnouncementBar>,
    userId: string,
  ): Promise<IAnnouncementBar> {
    logger.info("[announcementBarService.createAnnouncement] Input data:", {
      status: data.status,
    ...
    if (!announcementData.dismissalDuration)
      delete announcementData.dismissalDuration;
    ...
    logger.info("[announcementBarService.createAnnouncement] Prepared data:", {
    ...
    logger.info(
      "[announcementBarService.createAnnouncement] Created document:",
      {
  ...
  async updateAnnouncement(
    id: string,
    data: Partial<IAnnouncementBar>,
  ): Promise<IAnnouncementBar | null> {
    logger.info("[announcementBarService.updateAnnouncement] Input data:", {
      status: data.status,
      isActive: data.isActive,
      fullData: data,
    });
    ...
    logger.info(
      "[announcementBarService.updateAnnouncement] Existing announcement:",
      {
    ...
    logger.info(
      "[announcementBarService.updateAnnouncement] Update data after cleanup:",
      {
    ...
    logger.info(
      "[announcementBarService.updateAnnouncement] Updated document:",
      {
        status: existingAnnouncement.status,
        isActive: existingAnnouncement.isActive,
      },
