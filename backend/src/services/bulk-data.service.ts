...
class BulkDataService {
  ...
  async validateImport(
    ...
    const summary: ValidationSummary = {
      ...
      validRecords: data.length - errors.filter(e => e.severity === 'error').length,
    ...
    console.log(`✓ Validation complete: ${validationId} (${Date.now() - startTime}ms)`);
  ...
  async executeImport(
    ...
    const contextData = await cacheService.get(`bulk-import:validation:${validationId}`);
  ...
  async exportData(
    ...
    const sanitized = data.map(doc => this.sanitizeDocument(options.model, doc));
    ...
    switch (modelName) {
      case 'Event':
        ...
        if (record.vendorEmail || record.vendorId) {
          const vendorResult = await this.relationshipResolver.resolveVendor(record);
          if (vendorResult.error) {
            errors.push({ row, field: 'vendorId', message: vendorResult.error, severity: 'error' });
            return null;
          }
        ...
        if (record.categorySlug || record.categoryId) {
          const categoryResult = await this.relationshipResolver.resolveCategory(record);
          if (categoryResult.error) {
            errors.push({ row, field: 'category', message: categoryResult.error, severity: 'error' });
            return null;
          }
      ...
      case 'Category':
        ...
        if (record.parentSlug || record.parentId) {
          const parentResult = await this.relationshipResolver.resolveParentCategory(record);
          if (parentResult.error) {
            errors.push({ row, field: 'parentId', message: parentResult.error, severity: 'error' });
            return null;
          }
      ...
      case 'Blog':
        ...
        if (record.categoryName || record.categoryId) {
          const categoryResult = await this.relationshipResolver.resolveBlogCategory(record);
          if (categoryResult.error) {
            errors.push({ row, field: 'category', message: categoryResult.error, severity: 'error' });
            return null;
          }
      ...
      case 'Collection':
        ...
        if (record.eventTitles && Array.isArray(record.eventTitles)) {
          ...
          for (const title of record.eventTitles) {
            const eventResult = await this.relationshipResolver.resolveEvent({ eventTitle: title });
            if (eventResult.error) {
              errors.push({ row, field: 'events', message: `Event "${title}": ${eventResult.error}`, severity: 'error' });
              return null;
            }
      ...
      case 'Coupon':
        ...
        if (record.createdByEmail || record.createdById) {
          const createdByResult = await this.relationshipResolver.resolveUser(record, 'createdBy');
          if (createdByResult.error) {
            errors.push({ row, field: 'createdBy', message: createdByResult.error, severity: 'error' });
            return null;
          }
        ...
        if (record.eventTitles && Array.isArray(record.eventTitles)) {
          ...
          for (const title of record.eventTitles) {
            const eventResult = await this.relationshipResolver.resolveEvent({ eventTitle: title });
            if (eventResult.error) {
              errors.push({ row, field: 'applicableEvents', message: `Event "${title}": ${eventResult.error}`, severity: 'error' });
              return null;
            }
            eventIds.push(eventResult.resolvedId!);
          }
        ...
        if (record.excludedEventTitles && Array.isArray(record.excludedEventTitles)) {
          const eventIds: string[] = [];
          for (const title of record.excludedEventTitles) {
            const eventResult = await this.relationshipResolver.resolveEvent({ eventTitle: title });
            if (eventResult.error) {
              errors.push({ row, field: 'excludedEvents', message: `Event "${title}": ${eventResult.error}`, severity: 'error' });
              return null;
            }
            eventIds.push(eventResult.resolvedId!);
          }
        ...
        } else if (record.excludedEventIds && Array.isArray(record.excludedEventIds)) {
        ...
        if (record.categorySlugs && Array.isArray(record.categorySlugs)) {
          ...
          for (const slug of record.categorySlugs) {
            const categoryResult = await this.relationshipResolver.resolveCategory({ categorySlug: slug });
            if (categoryResult.error) {
              errors.push({ row, field: 'applicableCategories', message: `Category "${slug}": ${categoryResult.error}`, severity: 'error' });
              return null;
            }
        ...
        if (record.excludedCategorySlugs && Array.isArray(record.excludedCategorySlugs)) {
          const categoryIds: string[] = [];
          for (const slug of record.excludedCategorySlugs) {
            const categoryResult = await this.relationshipResolver.resolveCategory({ categorySlug: slug });
            if (categoryResult.error) {
              errors.push({ row, field: 'excludedCategories', message: `Category "${slug}": ${categoryResult.error}`, severity: 'error' });
              return null;
            }
            categoryIds.push(categoryResult.resolvedId!);
          }
        ...
        } else if (record.excludedCategoryIds && Array.isArray(record.excludedCategoryIds)) {
        ...
        if (record.vendorEmails && Array.isArray(record.vendorEmails)) {
          ...
          for (const email of record.vendorEmails) {
            const vendorResult = await this.relationshipResolver.resolveVendor({ vendorEmail: email });
            if (vendorResult.error) {
              errors.push({ row, field: 'applicableVendors', message: `Vendor "${email}": ${vendorResult.error}`, severity: 'error' });
              return null;
            }
        ...
        if (record.excludedVendorEmails && Array.isArray(record.excludedVendorEmails)) {
          const vendorIds: string[] = [];
          for (const email of record.excludedVendorEmails) {
            const vendorResult = await this.relationshipResolver.resolveVendor({ vendorEmail: email });
            if (vendorResult.error) {
              errors.push({ row, field: 'excludedVendors', message: `Vendor "${email}": ${vendorResult.error}`, severity: 'error' });
              return null;
            }
            vendorIds.push(vendorResult.resolvedId!);
          }
        ...
        } else if (record.excludedVendorIds && Array.isArray(record.excludedVendorIds)) {
      ...
      case 'Order':
        ...
        if (record.userEmail || record.userId) {
          const userResult = await this.relationshipResolver.resolveUser(record, 'user');
          if (userResult.error) {
            errors.push({ row, field: 'userId', message: userResult.error, severity: 'error' });
            return null;
          }
        ...
        if (record.items && Array.isArray(record.items)) {
          for (let i = 0; i < record.items.length; i++) {
            ...
            if (item.eventTitle || item.eventId) {
              const eventResult = await this.relationshipResolver.resolveEvent(item, 'event');
              if (eventResult.error) {
                errors.push({ row, field: `items[${i}].eventId`, message: eventResult.error, severity: 'error' });
                return null;
              }
    ...
    switch (modelName) {
      case 'Event':
        ...
        if (record.dateSchedule && Array.isArray(record.dateSchedule)) {
          for (let i = 0; i < record.dateSchedule.length; i++) {
            ...
            if (schedule.availableSeats !== undefined && schedule.totalSeats !== undefined) {
            ...
            if (schedule.timeSlots && Array.isArray(schedule.timeSlots)) {
              for (let j = 0; j < schedule.timeSlots.length; j++) {
                ...
                if (slot.availableSeats !== undefined && slot.totalSeats !== undefined) {
        ...
        if (record.imageAssets && Array.isArray(record.imageAssets) && record.imageAssets.length > 0) {
      ...
      case 'Blog':
        ...
        if (record.featuredImageAsset) {
          const MediaAsset = mongoose.model('MediaAsset');
          const exists = await MediaAsset.exists({ _id: record.featuredImageAsset });
          if (!exists) {
            errors.push({
              row,
      ...
      case 'Collection':
        ...
        if (record.eventIds && Array.isArray(record.eventIds) && record.eventIds.length > 0) {
      ...
      case 'Coupon':
        ...
        if (record.usageLimit !== undefined && record.usageCount !== undefined) {
      ...
      case 'Order':
        ...
        if (record.subtotal !== undefined && record.total !== undefined) {
          const calculated = (record.subtotal || 0) +
            (record.tax || 0) +
            (record.serviceFee || 0) -
            (record.discount || 0) -
        ...
        if (record.paymentRouting) {
          const platformCommission = record.paymentRouting.platformCommission || 0;
  ...
  private async upsertRecord(
    ...
    if (options.mode === 'upsert') {
      ...
      const existing = await Model.findOne({ [matchField]: matchValue }).session(session);
  ...
  private async fetchRelationships(
    ...
    switch (modelName) {
      case 'Event':
        // Fetch vendors
        const vendorIds = [...new Set(data.map(d => d.vendorId?.toString()).filter(Boolean))];
        ...
        // Fetch categories
        const categoryIds = [...new Set(data.map(d => d.category?.toString()).filter(Boolean))];
        if (categoryIds.length > 0) {
          relationships.categories = await Category.find({ _id: { $in: categoryIds } })
  ...
  private async createAuditLog(audit: BulkOperationAudit): Promise<void> {
    ...
    console.log(`Audit log created: ${audit.action} by ${audit.userId} - ${audit.success ? 'SUCCESS' : 'FAILED'}`);
  }
}
