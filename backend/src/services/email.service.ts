...
class EmailService {
  ...
  async sendPartnershipNotification(
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="info-box">
              ...
              ${
                options.campaignType === "summer_2026"
                  ? `
              <div class="info-row" style="background:#fff7ed;">
                <span class="info-label">🌞 Campaign:</span>
                <span class="info-value" style="color:#ea580c;font-weight:bold;">KIDROVE SUMMER 2026</span>
              </div>
              <div class="info-row">
                <span class="info-label">Package:</span>
                <span class="info-value">${options.selectedPackage ? options.selectedPackage.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()) : 'Not specified'}</span>
              </div>
              ${options.emirate ? `<div class="info-row"><span class="info-label">Emirate:</span><span class="info-value">${options.emirate}</span></div>` : ""}
              `
                  : ""
              }
              <div class="info-row">
    ...
    const text = `
      NEW PARTNERSHIP INQUIRY${options.campaignType === "summer_2026" ? " — KIDROVE SUMMER 2026" : ""}

      Partnership Type: ${partnershipTypeLabels[options.partnershipType] || options.partnershipType}
      ${options.campaignType === "summer_2026" ? `Campaign: KIDROVE SUMMER 2026\n      Package: ${options.selectedPackage || "Not specified"}\n      ${options.emirate ? `Emirate: ${options.emirate}` : ""}` : ""}
    ...
    await this.sendEmail({
      to: supportEmail,
      subject: `${options.campaignType === "summer_2026" ? "[SUMMER 2026] " : ""}New Partnership Inquiry: ${partnershipTypeLabels[options.partnershipType]} - ${options.name}`,
      html,
      text,
    });
  }
