...
export class CollectionSyncService {
  ...
  private async getEventDataForEmbed(eventId: mongoose.Types.ObjectId) {
    try {
      ...
      logger.error(
        `Error fetching event data for embed ${eventId}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
    ...
    try {
      ...
      for (const collection of collections) {
        try {
          ...
          logger.error(
            `Failed to sync event ${objId} to collection ${collection._id}: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
      ...
      logger.error(
        `Error syncing event ${eventId}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async removeEventFromCollections(eventId: string | mongoose.Types.ObjectId) {
    try {
      ...
      logger.error(
        `Error removing event ${eventId}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async syncCollection(collectionId: string | mongoose.Types.ObjectId) {
    try {
      ...
      // Update collection directly to avoid validation noise from legacy malformed docs.
      await Collection.updateOne(
        { _id: collection._id },
        {
          $set: {
            ...
            count: `${eventsData.length}+ activities`,
          },
          $inc: { dataVersion: 1 },
        },
      );
      ...
      logger.error(
        `Error syncing collection ${collectionId}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
