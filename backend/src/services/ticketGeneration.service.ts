...
export class TicketGenerationService {
  ...
  static async generateTicketsForOrder(
    ...
    try {
      ...
      logger.info('🔍 TICKET GENERATION: Finding order', { orderId: orderId.toString() });
      const order = await Order.findById(orderId).populate('items.eventId');

      if (!order) {
        logger.error('❌ TICKET GENERATION: Order not found', { orderId: orderId.toString() });
      ...
      if (order.status !== 'confirmed') {
        ...
        throw new Error(`Order must be confirmed to generate tickets. Current status: ${order.status}`);
      }

      // Check if tickets already exist
      if (skipExisting) {
        logger.info('🔍 TICKET GENERATION: Checking for existing tickets', { orderId: order._id.toString() });
        const existingTickets = await Ticket.find({ orderId: order._id });

        if (existingTickets.length > 0) {
          logger.info('✅ TICKET GENERATION: Found existing tickets, skipping generation', {
            orderId: order._id.toString(),
        ...
        logger.info('📋 TICKET GENERATION: No existing tickets found, proceeding with generation', {
          orderId: order._id.toString()
        });
      }

      // Get user information
      logger.info('🔍 TICKET GENERATION: Finding user', { userId: order.userId.toString() });
      ...
      for (const orderItem of order.items) {
        try {
          ...
          for (let i = 0; i < orderItem.quantity; i++) {
            try {
              logger.info(`🎫 TICKET GENERATION: Creating ticket ${i + 1}/${orderItem.quantity}`, {
                orderId: order._id.toString(),
                eventId: event._id.toString()
              });

              const ticketNumber = await this.generateUniqueTicketNumber();
              logger.info('✅ TICKET GENERATION: Generated unique ticket number', {
                ticketNumber,
                ticketIndex: i + 1
              });
              ...
              const qrDataPayload = {
                ...
                vendorId: event.vendorId.toString(),
                orderNumber: order.orderNumber,
                validUntil: new Date(orderItem.scheduleDate.getTime() + 24 * 60 * 60 * 1000),
              ...
              logger.info('✅ TICKET GENERATION: Ticket saved to database successfully', {
              ...
              if (sendEmail) {
                // Send email asynchronously without blocking ticket generation
                sendTicketByEmail({
                  to: user.email,
                  ...
                  // Email failure doesn't affect ticket generation success
                });
              }
   ...
   */
  static async orderHasTickets(orderId: string | Types.ObjectId): Promise<boolean> {
   ...
   */
  static async getTicketsForOrder(orderId: string | Types.ObjectId): Promise<any[]> {
