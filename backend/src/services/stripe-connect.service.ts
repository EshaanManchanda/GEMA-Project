...
interface IStripeConnectService {
  createConnectAccount(vendorId: Types.ObjectId, email: string, businessName: string): Promise<string>;
  generateOnboardingLink(vendorId: Types.ObjectId, refreshUrl: string, returnUrl: string): Promise<string>;
  getAccountStatus(vendorId: Types.ObjectId): Promise<StripeAccountStatus>;
  refreshAccountLink(vendorId: Types.ObjectId, refreshUrl: string, returnUrl: string): Promise<string>;
  verifyAccountCapabilities(stripeAccountId: string): Promise<AccountCapabilities>;
  disconnectAccount(vendorId: Types.ObjectId): Promise<void>;
  updateAccountInfo(vendorId: Types.ObjectId, accountData: AccountUpdateData): Promise<void>;
}
...
class StripeConnectService implements IStripeConnectService {
  ...
  async createConnectAccount(
    ...
    try {
      ...
      console.log(`✓ Created Stripe Connect account ${account.id} for vendor ${vendorId}`);
  ...
  async generateOnboardingLink(
    ...
    try {
      ...
      let accountId = vendor.paymentSettings.stripeSettings.stripeConnectAccountId;
      ...
      console.log(`✓ Generated onboarding link for vendor ${vendorId}`);
   ...
   */
  async getAccountStatus(vendorId: Types.ObjectId): Promise<StripeAccountStatus> {
    try {
      const vendor = await Vendor.findById(vendorId);
      if (!vendor) {
        throw new Error('Vendor not found');
      ...
      const accountId = vendor.paymentSettings.stripeSettings.stripeConnectAccountId;
      ...
      const status: StripeAccountStatus = {
        ...
        capabilities: {
          card_payments: (account.capabilities?.card_payments || 'inactive') as 'active' | 'inactive' | 'pending',
      ...
      if (account.details_submitted && !vendor.paymentSettings.stripeSettings.stripeConnectOnboardingComplete) {
        vendor.paymentSettings.stripeSettings.stripeConnectOnboardingComplete = true;
        vendor.paymentSettings.stripeSettings.stripeConnectCapabilities = status.capabilities;
        await vendor.save();
        console.log(`✓ Vendor ${vendorId} completed Stripe onboarding`);
      }
   ...
   */
  async verifyAccountCapabilities(stripeAccountId: string): Promise<AccountCapabilities> {
    try {
    ...
    try {
      ...
      if (accountId) {
        ...
        try {
          ...
          console.log(`✓ Deleted Stripe Connect account ${accountId}`);
      ...
      vendor.paymentSettings.stripeSettings.stripeConnectOnboardingComplete = false;
      vendor.paymentSettings.stripeSettings.stripeConnectCapabilities = undefined;
      ...
      if (vendor.paymentSettings.subscriptionStatus === VendorSubscriptionStatus.ACTIVE) {
        vendor.paymentSettings.subscriptionStatus = VendorSubscriptionStatus.INACTIVE;
      }

      await vendor.save();

      console.log(`✓ Disconnected Stripe account for vendor ${vendorId}`);
    } catch (error: any) {
  ...
  async updateAccountInfo(
    ...
    try {
      ...
      console.log(`✓ Updated Stripe Connect account info for vendor ${vendorId}`);
    } catch (error: any) {
  ...
  private async handleAccountUpdated(account: Stripe.Account): Promise<void> {
    ...
    if (!vendorId) {
      console.warn('Account updated but no vendorId in metadata');
      return;
    }

    const vendor = await Vendor.findById(vendorId);
    if (!vendor) {
      console.warn(`Vendor ${vendorId} not found for account ${account.id}`);
      return;
    }
    ...
    console.log(`✓ Updated vendor ${vendorId} from webhook`);
  }

  /**
   * Handle account.application.deauthorized webhook
   */
  private async handleAccountDeauthorizedById(accountId: string): Promise<void> {
    ...
    if (!vendor) {
      console.warn(`Vendor not found for deauthorized account ${accountId}`);
      return;
    }

    await this.disconnectAccount(vendor._id);
    console.log(`✓ Deauthorized account ${accountId} for vendor ${vendor._id}`);
  }
  ...
  private async handleCapabilityUpdated(capability: Stripe.Capability): Promise<void> {
    ...
    if (!vendor) {
      console.warn(`Vendor not found for account ${accountId}`);
      return;
    }
    ...
    console.log(`✓ Updated capabilities for vendor ${vendor._id}`);
  }
}
