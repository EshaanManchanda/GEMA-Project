import NewsletterSubscriber, { INewsletterSubscriber } from '../models/NewsletterSubscriber';
...
export class NewsletterService {
   ...
   */
  async updateSubscription(email: string, data: UpdateSubscriptionData): Promise<INewsletterSubscriber> {
    try {
   ...
   */
  async unsubscribeByEmail(email: string, reason?: string): Promise<INewsletterSubscriber> {
    try {
      const subscriber = await NewsletterSubscriber.findOne({
        email: email.toLowerCase(),
        isActive: true
   ...
   */
  async unsubscribeByToken(token: string, reason?: string): Promise<INewsletterSubscriber> {
    try {
      const subscriber = await NewsletterSubscriber.findOne({
   ...
   */
  async getSubscriptionByEmail(email: string): Promise<INewsletterSubscriber | null> {
    try {
      const subscriber = await NewsletterSubscriber.findOne({
        email: email.toLowerCase()
      });
   ...
   */
  async getActiveSubscribers(filters: SubscriptionFilters): Promise<INewsletterSubscriber[]> {
    try {
      ...
      console.error('=== NEWSLETTER SERVICE ERROR (GET ACTIVE SUBSCRIBERS) ===');
      console.error('Error:', error.message);
