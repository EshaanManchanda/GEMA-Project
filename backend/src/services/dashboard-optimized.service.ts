import {
  ...
  Category, Venue, Blog, Affiliate, Coupon, RevenueTransaction
} from '../models/index';
    ...
    const userStats = await User.aggregate([
      {
        $facet: {
          ...
          previousPeriod: [{ $match: filters.previousPeriodFilter }, { $count: 'count' }],
    ...
    const orderStats = await Order.aggregate([
      {
        $facet: {
          ...
          previousRevenue: [
            { $match: { paymentStatus: 'paid', ...filters.previousPeriodFilter } },
            { $group: { _id: null, total: { $sum: '$total' } } }
          ],
