...
export class CacheGroupService {
  ...
  async invalidateGroup(group: string): Promise<number> {
    ...
    try {
      ...
      logger.info(`Invalidated cache group '${group}': ${keys.length} keys deleted`);
      return keys.length;
    } catch (error) {
   ...
   */
  async isKeyInGroup(key: string, group: string, options?: CacheGroupOptions): Promise<boolean> {
    if (!this.isRedisAvailable()) {
      return false;
    }
   ...
   */
  async removeFromGroup(key: string, group: string, options?: CacheGroupOptions): Promise<boolean> {
    if (!this.isRedisAvailable()) {
      return false;
    }
