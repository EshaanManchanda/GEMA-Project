...
class SubscriptionService implements ISubscriptionService {
  ...
  async createSubscription(
    ...
    try {
      ...
      logger.info(`✓ Created subscription for vendor ${vendorId}`);
  ...
  async processSubscriptionPayment(
    ...
    try {
      ...
      const periodStart =
        currentPaidUntil && currentPaidUntil > now ? currentPaidUntil : now;
      ...
      logger.info(`✓ Processed subscription payment for vendor ${vendorId}`);
  ...
  async cancelSubscription(
    ...
    try {
      ...
      logger.info(
        `✓ Cancelled subscription for vendor ${vendorId}. Reason: ${reason || "Not specified"}`,
      );
    } catch (error: any) {
  ...
  async suspendSubscription(
    ...
    try {
      ...
      logger.info(
        `✓ Suspended subscription for vendor ${vendorId}. Reason: ${reason}`,
      );

      emailService
        .sendEmail({
          to: vendor.email,
          subject: "Your Gema subscription has been suspended",
  ...
  async reactivateSubscription(vendorId: Types.ObjectId): Promise<void> {
    try {
      ...
      logger.info(
        `✓ Reactivated subscription for vendor ${vendorId}. Events restored.`,
      );

      emailService
        .sendEmail({
          to: vendor.email,
          subject: "Your Gema subscription is active again",
          html: `<p>Hi ${vendor.businessName}, your subscription has been reactivated. Your events are now live.</p>`,
        })
        .catch(() => {});
    } catch (error: any) {
  ...
  async sendPaymentReminder(
    ...
    try {
      ...
      logger.info(
        `📧 Sending payment reminder to vendor ${vendorId} (${daysUntilDue} days until due)`,
      );

      emailService
        .sendEmail({
          to: vendor.email,
          subject: `Subscription payment due in ${daysUntilDue} day(s)`,
          html: `<p>Hi ${vendor.businessName}, your subscription renewal is due in <strong>${daysUntilDue} day(s)</strong>. Please ensure your payment method is up to date.</p>`,
        })
        .catch(() => {});
...
<p>Please renew at <a href="${process.env.FRONTEND_URL}/vendor/subscription">your subscription page</a>.</p>`,
            }).catch((err: Error) => logger.error("Vendor deactivation email failed:", err));
          }
        }
      }
