...
export class ReelService {
  ...
  async updateReel(id: string, data: Partial<IReel>): Promise<IReel> {
    ...
    try {
      ...
      if (newVideoSourceType === 'uploaded' && newVideoId && newVideoId !== oldVideoId) {
      ...
      } else if (oldVideoSourceType === 'uploaded' && newVideoSourceType !== 'uploaded' && oldVideoId) {
   ...
   */
  async getPublicReels(options: PaginationOptions = {}): Promise<PaginatedResult<IReel>> {
   ...
   */
  async getAllReels(filters: ReelFilters = {}): Promise<PaginatedResult<IReel>> {
   ...
   */
  async getReelById(id: string, populateAssets: boolean = true): Promise<IReel | null> {
   ...
   */
  async toggleLike(id: string, increment: boolean = true): Promise<{ liked: boolean; likes: number }> {
   ...
   * Update visibility
   */
  async updateVisibility(id: string, visibility: 'public' | 'draft' | 'archived'): Promise<IReel> {
    const reel = await Reel.findById(id);
    if (!reel) {
      throw new Error('Reel not found');
    }
   ...
   */
  async updateDisplayOrders(updates: Array<{ id: string; displayOrder: number }>): Promise<void> {
    const session = await mongoose.startSession();
    session.startTransaction();
