...
import CommissionConfig, { CommissionRuleType, RecipientType } from '../models/CommissionConfig';
import CommissionTransaction, { CommissionTransactionStatus, CommissionRecipientType } from '../models/CommissionTransaction';
...
class CommissionService {
  ...
  async getActiveCommissionConfig(): Promise<any> {
    try {
      ...
      if (!config) {
        console.warn('⚠️  No active commission configuration found. Using fallback.');
   ...
   */
  async calculateCommissionForOrder(orderId: string | mongoose.Types.ObjectId): Promise<any> {
    try {
      ...
      const vendor = await User.findById(event.vendorId).select('firstName lastName email').lean();
      if (!vendor) {
        throw new Error(`Vendor not found: ${event.vendorId}`);
      }

      // Check if commission already exists for this order
      const existingCommission = await CommissionTransaction.findOne({ orderId });
      ...
      const commissionResult = this.applyCommissionRules(totalAmount, config, orderData);
      ...
      const commissionTransaction = new CommissionTransaction({
        ...
        customerName: orderData.userId ? `${(orderData.userId as any).firstName} ${(orderData.userId as any).lastName}` : 'Guest',
      ...
      console.log(`✅ Commission calculated: ${commissionResult.totalCommission} AED (${commissionResult.rate}%)`);
   ...
   */
  private applyCommissionRules(totalAmount: number, config: any, order?: any): any {
    ...
    if (!config.rules || config.rules.length === 0) {
      ...
      platformCommission = (totalAmount * rate) / 100;
    ...
    for (const rule of config.rules.filter((r: any) => r.status === 'active')) {
      ...
      if (rule.conditions && !this.checkRuleConditions(rule.conditions, totalAmount, order)) {
        continue;
      }
      ...
      switch (rule.type) {
        case CommissionRuleType.PERCENTAGE:
          commissionAmount = (totalAmount * (rule.percentage || 0)) / 100;
          break;
        ...
        case CommissionRuleType.TIERED:
          commissionAmount = this.calculateTieredCommission(totalAmount, rule.tiers || []);
          break;
      ...
      commissions.push({
        recipientId: rule.recipient === RecipientType.PLATFORM ? 'platform' : order?.affiliateId || 'unknown',
   ...
   */
  private checkRuleConditions(conditions: any, totalAmount: number, order?: any): boolean {
   ...
   */
  private mapRecipientType(recipientType: RecipientType): CommissionRecipientType {
    switch (recipientType) {
      case RecipientType.PLATFORM:
  ...
  async recalculateCommission(transactionId: string): Promise<any> {
    try {
      ...
      // Recalculate
      const newCommission = await this.calculateCommissionForOrder(transaction.orderId);

      console.log(`✅ Commission recalculated for order: ${transaction.orderId}`);
