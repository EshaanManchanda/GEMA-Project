...
// Search Console Sync Queue — monthly durable pull of Google Search Console
// data (plus on-demand runs from the admin "Sync Now" button)
export const searchConsoleSyncQueue = areQueuesDisabled
  ? null
  : new Queue(QUEUE_NAMES.SEARCH_CONSOLE_SYNC, queueConfig);
...
function getQueueByName(name: string): Queue | null {
  ...
  switch (name) {
    ...
    case QUEUE_NAMES.SEARCH_CONSOLE_SYNC:
      return searchConsoleSyncQueue;
    default:
      return null;
  }
}
