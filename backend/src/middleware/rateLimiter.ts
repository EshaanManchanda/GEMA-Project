...
export const passwordResetLimiter: RateLimitRequestHandler = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    console.warn(`Password reset rate limit exceeded from IP: ${req.ip}, email: ${req.body?.email}`);
    res.status(429).json({
...
export const emailVerificationLimiter: RateLimitRequestHandler = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    res.status(429).json({
      ...createErrorMessage(60 * 60),
      message: 'Too many verification requests. Please check your email or try again later.',
    });
  },
});
...
export const paymentLimiter: RateLimitRequestHandler = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    console.warn(`Payment rate limit exceeded for user: ${(req as any).user?._id}, IP: ${req.ip}`);
    res.status(429).json({
      ...createErrorMessage(60 * 60),
