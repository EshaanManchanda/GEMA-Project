...
export const sanitizeRequest = mongoSanitize({
  ...
  onSanitize: ({ req, key }) => {
    logger.warn(
      `Sanitized potentially malicious input: ${key} from ${req.ip}`,
    );
  },
});
...
export const requestSizeLimiter = (
  ...
  if (contentLength > maxSize) {
    logger.warn(
      `Large request blocked from IP: ${req.ip}, size: ${contentLength} bytes`,
    );
...
export const detectSuspiciousActivity = (
  ...
  if (checkString(req.url)) {
    logger.error(`Suspicious URL detected from IP ${req.ip}: ${req.url}`);
  ...
  if (checkString(queryString)) {
    logger.error(
      `Suspicious query parameters from IP ${req.ip}: ${queryString}`,
    );
  ...
  if (req.body) {
    ...
    if (checkString(bodyString)) {
      logger.error(`Suspicious request body from IP ${req.ip}`);
