...
export const conditionalPhoneVerification = async (
  ...
  try {
    ...
    if (!eventId && req.body.orderId) {
      try {
        const order = await Order.findById(req.body.orderId).select('items').lean();
        if (order && order.items && order.items.length > 0) {
          // Get eventId from first order item
          eventId = order.items[0].eventId;
        }
      } catch (error) {
        console.error('conditionalPhoneVerification: Error fetching order:', error);
      }
    }
    ...
    if (!eventId) {
      console.warn('conditionalPhoneVerification: No eventId found in request, skipping verification');
      return next();
    }

    // Fetch the event to check its requirePhoneVerification setting
    const event = await Event.findById(eventId).select('requirePhoneVerification').lean();

    if (!event) {
      // Event not found - let the actual route handler deal with this error
      // For now, skip phone verification
      console.warn(`conditionalPhoneVerification: Event ${eventId} not found, skipping verification`);
      return next();
    }
