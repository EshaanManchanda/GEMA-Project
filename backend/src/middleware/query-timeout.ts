 ...
 */
export const setQueryTimeout = (req: Request, res: Response, next: NextFunction): void => {
  const isUploadRoute = req.path.startsWith('/api/uploads') || req.path.startsWith('/api/media');
  const isDashboardRoute = req.path.includes('/dashboard') || req.path.includes('/analytics');
  const isReportRoute = req.path.includes('/report') || req.path.includes('/export');
