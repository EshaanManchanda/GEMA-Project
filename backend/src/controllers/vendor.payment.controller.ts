...
export const getPaymentOverview = async (req: Request, res: Response) => {
  try {
    ...
    if (vendor.paymentSettings.paymentMode === PaymentMode.CUSTOM_STRIPE) {
      subscriptionInfo = await subscriptionService.checkSubscriptionStatus(vendor._id);
    }
    ...
    if (vendor.paymentSettings.stripeSettings.stripeConnectAccountId) {
      stripeConnectStatus = await stripeConnectService.getAccountStatus(vendor._id);
    }
    ...
    const overview = {
      ...
      // Stripe Connect info
      stripeConnect: {
        connected: !!vendor.paymentSettings.stripeSettings.stripeConnectAccountId,
        accountId: vendor.paymentSettings.stripeSettings.stripeConnectAccountId,
        onboardingComplete: vendor.paymentSettings.stripeSettings.stripeConnectOnboardingComplete,
      ...
      manualStripe: {
        ...
        publishableKey: vendor.paymentSettings.stripeSettings.stripePublishableKey,
        testMode: vendor.paymentSettings.stripeSettings.stripeTestMode,
        lastValidated: vendor.paymentSettings.stripeSettings.stripeKeysLastValidated,
        validationError: vendor.paymentSettings.stripeSettings.stripeKeysValidationError
      },
      ...
      bankAccount: {
        ...
        isVerified: vendor.paymentSettings.bankAccountDetails?.isVerified || false,
        accountHolderName: vendor.paymentSettings.bankAccountDetails?.accountHolderName,
    ...
    res.status(500).json({ message: 'Failed to get payment overview', error: error.message });
  }
};
...
export const initiateStripeConnect = async (req: Request, res: Response) => {
  try {
    ...
    if (!returnUrl || !refreshUrl) {
      return res.status(400).json({ message: 'returnUrl and refreshUrl are required' });
    }
    ...
    res.status(500).json({ message: 'Failed to initiate Stripe Connect', error: error.message });
  }
};
...
export const getStripeConnectStatus = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to get Stripe Connect status', error: error.message });
  }
};
...
export const refreshStripeConnectLink = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to refresh Stripe Connect link', error: error.message });
  }
};
...
export const disconnectStripeConnect = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to disconnect Stripe Connect', error: error.message });
  }
};
...
export const updateStripeKeys = async (req: Request, res: Response) => {
  try {
    ...
    if (!publishableKey || !secretKey) {
      return res.status(400).json({ message: 'Both publishableKey and secretKey are required' });
    }
    ...
    try {
      ...
      // Keys are valid
      vendor.paymentSettings.stripeSettings.stripePublishableKey = publishableKey;
      ...
      vendor.paymentSettings.stripeSettings.stripeKeysLastValidated = new Date();
      vendor.paymentSettings.stripeSettings.stripeKeysValidationError = undefined;
      ...
      vendor.paymentSettings.stripeSettings.stripeKeysValidationError = validationError.message;
      await vendor.save();
    ...
    res.status(500).json({ message: 'Failed to update Stripe keys', error: error.message });
  }
};
  ...
  try {
    ...
    if (paymentMode === PaymentMode.CUSTOM_STRIPE) {
      if (!vendor.canSwitchToCustomStripe()) {
        return res.status(400).json({
          message: 'Cannot switch to custom Stripe. Please complete Stripe setup and verification first.'
        });
      }
    ...
    res.status(500).json({ message: 'Failed to switch payment mode', error: error.message });
  }
};
...
export const getSubscriptionInfo = async (req: Request, res: Response) => {
  try {
    ...
    const subscriptionInfo = await subscriptionService.checkSubscriptionStatus(vendor._id);

    res.json(subscriptionInfo);
  } catch (error: any) {
    console.error('Error getting subscription info:', error);
    res.status(500).json({ message: 'Failed to get subscription info', error: error.message });
  }
};
...
export const paySubscription = async (req: Request, res: Response) => {
  try {
    ...
    const result = await subscriptionService.processSubscriptionPayment(vendor._id);
    ...
    res.status(500).json({ message: 'Failed to process subscription payment', error: error.message });
  }
};
...
export const getSubscriptionHistory = async (req: Request, res: Response) => {
  try {
    ...
    const history = await subscriptionService.getSubscriptionHistory(vendor._id);
...
export const updateBankAccount = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    const { accountHolderName, bankName, accountNumber, routingNumber, iban, swiftCode } = req.body;
...
export const updatePayoutPreferences = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    const { payoutSchedule, minimumPayout, preferredPayoutMethod, autoPayoutEnabled } = req.body;
    ...
    if (minimumPayout !== undefined) {
      if (minimumPayout < 50) {
        return res.status(400).json({ message: 'Minimum payout must be at least 50 AED' });
      }
      vendor.paymentSettings.minimumPayout = minimumPayout;
    }

    if (preferredPayoutMethod) {
      if (!['bank_transfer', 'stripe', 'paypal'].includes(preferredPayoutMethod)) {
 ...
 */
export const calculateCommissionComparison = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    const { monthlyRevenue } = req.body;

    if (!monthlyRevenue || monthlyRevenue < 0) {
      return res.status(400).json({ message: 'Valid monthly revenue is required' });
    }
    ...
    const comparison = {
      ...
      savingsPercentage: ((Math.abs(savings) / Math.max(commissionCost, subscriptionCost)) * 100).toFixed(2),

      recommendation: recommendCustomStripe ? 'custom_stripe' : 'platform_stripe',
    ...
    res.status(500).json({ message: 'Failed to calculate comparison', error: error.message });
  }
};
