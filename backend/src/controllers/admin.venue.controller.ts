...
/**
 * Format venue for admin response
 * Maps Event model fields back to Venue-like structure for admin compatibility
 */
const formatAdminVenueResponse = (venue: any) => {
  // Determine legacy status from Event fields
  let status = "inactive";
  ...
  return {
    ...
    name: venue.title || venue.name, // Fallback
    description: venue.description,
    status: status,
    // Map location back to address
    ...
    images: venue.images,
    virtualTourUrl: venue.virtualTourUrl, // Might be missing in Event?
    baseRentalPrice: venue.price, // Mapped from price
    currency: venue.currency,
    safetyFeatures: venue.safetyFeatures,
    certifications: venue.certifications,
    insuranceInfo: venue.insuranceInfo,
    totalEvents: venue.totalEvents || 0, // Event model doesn't have totalEvents logic yet?
    // totalEvents in Venue was explicitly stored. In Event it's not.
    // We might need to aggregate or just return 0 for now.
    ...
    vendor:
      venue.vendorId && typeof venue.vendorId === "object"
        ...
        : venue.vendorId, // Return ID if not populated
    createdAt:
