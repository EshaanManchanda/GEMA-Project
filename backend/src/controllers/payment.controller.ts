...
export const confirmPayment = async (
  ...
  try {
    ...
    if (paymentIntent.status === "succeeded") {
      ...
      try {
        await CommissionService.calculateCommissionForOrder(
          order._id.toString(),
        );
        logger.info(`✅ Commission calculated for order ${order._id}`);
      } catch (commissionError) {
        logger.error(
          `⚠️  Failed to calculate commission for order ${order._id}:`,
          commissionError,
        );
