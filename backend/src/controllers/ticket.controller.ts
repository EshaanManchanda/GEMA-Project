...
import { Ticket, User, Event, Employee, Order } from "../models/index";
...
export const verifyTicketQR = async (
  ...
  try {
    ...
    logger.error("Error verifying ticket QR:", error);
...
export const getEventTickets = async (
  ...
  try {
    ...
    logger.error("Error retrieving event tickets:", error);
...
export const getTicketsByOrder = async (
  ...
  try {
    ...
    if (!isMongoId && !orderId.startsWith("GM-")) {
      // Try slug lookup
      const event = await Event.findOne({ slug: orderId });
      if (event) {
        ...
        if (order) {
          ...
          logger.debug(`Resolved slug ${orderId} to order ${resolvedOrderId}`);
        } else {
          logger.warn(`Could not find order for slug ${orderId} and user ${userId}`);
        }
      }
    } else if (orderId.startsWith("GM-")) {
      const order = await Order.findOne({
        orderNumber: orderId,
        userId,
      });
      if (order) {
        resolvedOrderId = order._id.toString();
        logger.debug(`Resolved order number ${orderId} to ID ${resolvedOrderId}`);
      }
    }

    // Find tickets by order ID and populate event data
    const tickets = await Ticket.find({ orderId: resolvedOrderId })
      .populate("eventId", "title dateSchedule location images description")
      .populate("userId", "firstName lastName email")
      .populate("vendorId", "firstName lastName email businessName")
      .sort({ createdAt: -1 });
...
export const generateMissingTickets = async (
  ...
  try {
    ...
    if (isMongoId || orderId.startsWith("GM-")) {
      query.$or = [{ _id: isMongoId ? orderId : undefined }, { orderNumber: orderId }].filter(Boolean);
    } else {
      // Slug lookup
      const event = await Event.findOne({ slug: orderId });
      if (event) {
    ...
    const result =
      await TicketGenerationService.generateMissingTicketsForOrder(order._id.toString());

    if (!result.success) {
      logger.error("Ticket generation failed:", result.errors);
      return next(
        new AppError(
    ...
    logger.error("Error generating missing tickets:", error);
