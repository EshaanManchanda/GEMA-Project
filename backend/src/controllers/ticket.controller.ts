...
export const generateTickets = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!orderId || !attendees || !Array.isArray(attendees) || attendees.length === 0 || !eventId) {
    return next(new AppError('Missing required fields: orderId, attendees, or eventId', 400));
  }
  ...
  try {
    ...
    for (const attendee of attendees) {
      const { name, email, phone, ticketType, price, currency, seatNumber } = attendee;
...
export const getTicketDetails = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!req.user || (req.user._id?.toString() !== ticket.userId.toString() && req.user.role !== 'admin' && req.user.role !== 'vendor')) {
...
export const transferTicket = async (req: AuthRequest, res: Response, next: NextFunction) => {
  const { ticketId } = req.params;
  ...
  try {
    ...
    const newQrCodeData = JSON.stringify({ ticketNumber: ticket.ticketNumber, eventId: ticket.eventId, userId: toUser._id });
...
export const resendTicket = async (req: AuthRequest, res: Response, next: NextFunction) => {
  const { ticketId } = req.params;
  const { method } = req.body;

  if (!method || !['email', 'sms'].includes(method)) {
    return next(new AppError('Invalid resend method. Must be \'email\' or \'sms\'.', 400));
  }
...
export const verifyTicketQR = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const lastEventDate = event.dateSchedule[event.dateSchedule.length - 1]?.date;
    if (lastEventDate && now > new Date(lastEventDate.getTime() + 24 * 60 * 60 * 1000)) { // 24 hours after last event
      return res.status(400).json({
        success: false,
        status: 'expired',
...
export const getEventTickets = async (req: AuthRequest, res: Response, next: NextFunction) => {
  const { eventId } = req.params;
  ...
  try {
    ...
    if (upcoming === 'true') {
      ...
      tickets = tickets.filter(ticket => {
        const event = ticket.eventId as any;
        return event.dateSchedule.some((schedule: any) => new Date(schedule.date) > now);
      });
    }
...
export const downloadTicketPDF = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!req.user || req.user._id?.toString() !== ticket.userId._id.toString()) {
