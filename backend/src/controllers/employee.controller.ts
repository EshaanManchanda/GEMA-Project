...
export const createEmployee = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (user) {
      ...
      if (existingEmployee) {
        return next(new AppError('User with this email is already an employee.', 409));
      }
...
export const getEmployeeDetails = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!employee) {
      return next(new AppError('Employee not found or not associated with your vendor account', 404));
    }
...
export const deleteEmployee = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const employee = await Employee.findOneAndDelete({ _id: employeeId, vendorId });
...
export const removeEmployeeFromEvent = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    employee.assignedEvents = employee.assignedEvents.filter(id => id.toString() !== eventId);
    await employee.save();
