...
// @access  Private
export const submitRegistration = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!event.registrationConfig?.enabled) {
      return next(new AppError('Registration is not enabled for this event', 400));
    }
    ...
    if (event.registrationConfig.maxRegistrations) {
      const currentCount = await Registration.countByEvent(eventId, RegistrationStatus.APPROVED);
      if (currentCount >= event.registrationConfig.maxRegistrations) {
        return next(new AppError('Maximum number of registrations reached', 400));
      }
    }
    ...
    const existingRegistration = await Registration.findOne({
      eventId,
      userId,
      status: { $nin: [RegistrationStatus.WITHDRAWN, RegistrationStatus.REJECTED] }
    });
    ...
    const registration = new Registration({
      ...
      files,
      payment: {
        status: amount > 0 ? RegistrationPaymentStatus.PENDING : RegistrationPaymentStatus.PAID,
        amount,
        currency: event.currency || 'AED',
      },
      status: saveAsDraft ? RegistrationStatus.DRAFT : RegistrationStatus.SUBMITTED,
      metadata: {
        ipAddress: req.ip,
    ...
    if (amount > 0 && !saveAsDraft) {
      ...
      registration.payment.stripePaymentIntentId = paymentSession.paymentIntentId;
      await registration.save();
    }
    ...
    res.status(201).json({
      success: true,
      message: saveAsDraft ? 'Registration saved as draft' : 'Registration submitted successfully',
      data: {
        registration: {
          id: registration._id,
...
// @access  Private
export const confirmRegistrationPayment = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const getRegistrationById = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const registration = await Registration.findById(id)
      ...
      .populate('eventId', 'title description price currency registrationConfig vendorId');
    ...
    if (!isOwner && !isVendor && !isAdmin) {
      return next(new AppError('You do not have permission to view this registration', 403));
    }
...
export const getUserRegistrations = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const registrations = await Registration.find(filter)
      .populate('eventId', 'title description price currency images dateSchedule')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limitNum);
...
export const getEventRegistrations = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!isVendor && !isAdmin) {
      return next(new AppError('You do not have permission to view these registrations', 403));
    }
...
export const updateRegistration = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!registration.canBeModified()) {
      return next(new AppError('Only draft registrations can be modified', 400));
    }
...
export const reviewRegistration = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!isVendor && !isAdmin) {
      return next(new AppError('You do not have permission to review this registration', 403));
    }

    // Update registration
    registration.status = status === 'approved' ? RegistrationStatus.APPROVED : RegistrationStatus.REJECTED;
