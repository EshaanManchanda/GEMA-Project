...
export const getAllBlogs = catchAsync(async (req: Request, res: Response) => {
  ...
  // Build sort query
  const sortQuery: any = {};
  if (sortBy === 'publishedAt' || sortBy === 'createdAt' || sortBy === 'viewCount' || sortBy === 'likeCount') {
...
export const getFeaturedBlogs = catchAsync(async (req: Request, res: Response) => {
...
export const getPopularBlogs = catchAsync(async (req: Request, res: Response) => {
  const { limit = 6 } = req.query;
  const limitNumber = Math.min(20, Math.max(1, Number(limit)));
...
export const getRecentBlogs = catchAsync(async (req: Request, res: Response) => {
  const { limit = 6 } = req.query;
  const limitNumber = Math.min(20, Math.max(1, Number(limit)));
...
export const getRelatedBlogs = catchAsync(async (req: Request, res: Response) => {
  const { slug } = req.params;
...
export const updateBlog = catchAsync(async (req: Request, res: Response) => {
  ...
  if (updateData.category && oldCategoryId !== updateData.category) {
    await Promise.all([
      BlogCategory.findByIdAndUpdate(oldCategoryId, { $inc: { postsCount: -1 } }),
      BlogCategory.findByIdAndUpdate(updateData.category, { $inc: { postsCount: 1 } })
    ]);
  }
...
export const getAllBlogsAdmin = catchAsync(async (req: Request, res: Response) => {
  ...
  // Build sort query
  const sortQuery: any = {};
  if (['createdAt', 'updatedAt', 'publishedAt', 'viewCount', 'likeCount'].includes(sortBy as string)) {
