...
export const getMyAffiliate = async (
  ...
  try {
    ...
    const stats = {
      ...
      averageOrderValue: affiliate.totalConversions > 0 
...
export const updateAffiliateProfile = async (
  ...
  try {
    ...
    const restrictedFields = ['affiliateCode', 'status', 'totalClicks', 'totalConversions', 'totalRevenue'];
...
export const generateTrackingUrl = async (
  ...
  try {
    ...
    const affiliate = await Affiliate.findOne({ userId, status: AffiliateStatus.ACTIVE });
    if (!affiliate) {
...
export const getDashboardStats = async (
  ...
  try {
    ...
    const periodClicks = affiliate.clicks.filter(click => 
      click.clickedAt >= startDate
    );
    
    const periodConversions = affiliate.clicks.filter(click => 
      click.converted && click.conversionDate && click.conversionDate >= startDate
    );
    
    const periodCommissions = affiliate.commissions.filter(commission => 
      commission.createdAt >= startDate
    );
    
    const periodRevenue = periodCommissions.reduce((sum, c) => sum + c.orderAmount, 0);
    const periodCommissionEarned = periodCommissions.reduce((sum, c) => sum + c.commissionAmount, 0);
    ...
    for (let i = periodDays - 1; i >= 0; i--) {
      ...
      const dayConversions = periodConversions.filter(c => 
        c.conversionDate >= dayStart && c.conversionDate <= dayEnd
      ).length;
      ...
      const dayRevenue = dayCommissions.reduce((sum, c) => sum + c.orderAmount, 0);
      const dayEarnings = dayCommissions.reduce((sum, c) => sum + c.commissionAmount, 0);
...
export const getCommissions = async (
  ...
  try {
    ...
    const paginatedCommissions = commissions.slice(skip, skip + parseInt(limit as string));
    ...
    const commissionsWithOrders = paginatedCommissions.map(commission => {
      const order = orders.find(o => o._id.toString() === commission.orderId.toString());
      return {
        ...commission,
        order
      };
...
export const getTopPerformers = async (
  ...
  try {
    ...
    const affiliates = await Affiliate.findTopPerformers(parseInt(limit as string));
    ...
    const performersWithPeriodStats = await Promise.all(
      affiliates.map(async (affiliate) => {
        const periodCommissions = affiliate.commissions.filter(c => 
          c.createdAt >= startDate
        );
        
        const periodRevenue = periodCommissions.reduce((sum, c) => sum + c.orderAmount, 0);
        const periodEarnings = periodCommissions.reduce((sum, c) => sum + c.commissionAmount, 0);
    ...
    const analytics = await Affiliate.aggregate([
      { $match: matchQuery },
      {
        $group: {
          ...
          activeAffiliates: {
            $sum: { $cond: [{ $eq: ['$status', AffiliateStatus.ACTIVE] }, 1, 0] }
          },
          pendingAffiliates: {
            $sum: { $cond: [{ $eq: ['$status', AffiliateStatus.PENDING] }, 1, 0] }
          },
      ...
      },
      {
        $project: {
          ...
          averageCommissionRate: {
            $cond: [
              { $gt: ['$totalRevenue', 0] },
              { $multiply: [{ $divide: ['$totalCommissionEarned', '$totalRevenue'] }, 100] },
              0
            ]
          }
        }
