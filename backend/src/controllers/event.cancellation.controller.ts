...
const queueEmail = async (type: string, data: any) => {
  if (!emailQueue) {
...
export const cancelEvent = async (
  ...
  try {
    ...
    if (!event.isCancellable()) {
      res.status(400).json({
        success: false,
        message: 'Event cannot be cancelled. It may have already started or been cancelled.',
      });
      return;
    }
    ...
    const refundResults = await RefundService.batchProcessRefunds(id, reason, userId);
    ...
    const notificationPromises = affectedOrders.map(async (order) => {
      try {
        const customer = await User.findById(order.userId).select('email firstName phone');
        if (customer?.email) {
          // Queue cancellation email
          const tax = order.tax || 0;
          const serviceFee = order.serviceFee || 0;
          await queueEmail('eventCancellation', {
            to: customer.email,
            firstName: customer.firstName || 'Customer',
            eventTitle: event.title,
            eventDate: event.dateSchedule[0]?.startDate || event.dateSchedule[0]?.date,
        ...
        logger.error(`Failed to queue notification for order ${order._id}:`, error);
...
export const getCancellationStatus = async (
  ...
  try {
    ...
    const event = await Event.findById(id)
      .select('title cancellationStatus cancellationReason cancelledAt cancelledBy cancellationNotification')
...
export const getAffectedOrders = async (
  ...
  try {
    ...
    const orders = await Order.find({
      ...
      .select('orderNumber status paymentStatus total currency refundAmount refundStatus createdAt')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(Number(limit));
...
export const retryNotifications = async (
  ...
  try {
    ...
    for (const log of failedLogs) {
      try {
        const customer = await User.findById(log.customerId).select('email firstName phone');
        if (customer?.email) {
          const tax = log.tax || 0;
...
export const cancelOrder = async (
  ...
  try {
    ...
    if (customer?.email) {
      await queueEmail('cancellationConfirmation', {
        to: customer.email,
        firstName: customer.firstName || 'Customer',
        orderNumber: order.orderNumber,
    ...
    res.status(200).json({
      ...
      data: {
        ...
        message: `Your refund of ${order.currency} ${refundResult.refundAmount.toFixed(2)} will be processed within 5-10 business days. Fees of ${order.currency} ${refundResult.nonRefundableAmount.toFixed(2)} are non-refundable.`,
      },
    });
...
export const getRefundStatus = async (
  ...
  try {
    ...
    const order = await Order.findById(id)
      .select('orderNumber userId status paymentStatus refundStatus refundAmount refundTransactionId refundedAt serviceFee currency cancellationType');
    ...
    const cancellationLog = await CancellationLog.findOne({ orderId: id })
      .select('refundStatus refundProcessedAt refundFailureReason notificationStatus');
