...
import AdvertisingCampaign, { CampaignStatus } from '../models/AdvertisingCampaign';
  ...
  try {
    ...
    const [
      ...
      RevenueTransaction.aggregate([
        { $match: query },
        { $group: { _id: null, total: { $sum: '$adminCommission' }, count: { $sum: 1 } } }
      ]),

      // Revenue by stream
      RevenueTransaction.aggregate([
        { $match: query },
        { $group: { _id: '$revenueStream', revenue: { $sum: '$adminCommission' }, count: { $sum: 1 } } },
        { $sort: { revenue: -1 } }
      ]),
      ...
      RevenueTransaction.aggregate([
        { $match: query },
        { $group: { _id: '$status', revenue: { $sum: '$adminCommission' }, count: { $sum: 1 } } }
      ]),

      // Top vendors by revenue
      RevenueTransaction.aggregate([
        { $match: query },
        { $group: { _id: '$vendorId', revenue: { $sum: '$adminCommission' }, transactions: { $sum: 1 } } },
        { $sort: { revenue: -1 } },
        { $limit: 10 },
      ...
      RevenueTransaction.aggregate([
        {
          $match: {
            transactionDate: { $gte: new Date(new Date().setMonth(new Date().getMonth() - 12)) }
          }
        },
        {
          $group: {
      ...
      RevenueTransaction.aggregate([
        ...
        { $group: { _id: null, totalPayout: { $sum: '$vendorPayout' }, count: { $sum: 1 } } }
      ])
    ]);
 ...
 */
export const getRevenueTransactions = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const {
      page = '1',
      limit = '20',
 ...
 */
export const createRevenueTransaction = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const {
      vendorId,
      customerId,
 ...
 */
export const updateRevenueTransaction = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
 ...
 */
export const processVendorPayouts = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (transactions.length === 0) {
      return next(new AppError('No pending transactions found for specified vendors', 404));
    }
    ...
    const successfulPayouts = payoutResults.filter(p => p.status === 'success');
 ...
 */
export const getSubscriptionAnalytics = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const [
      subscriptionStats,
      revenueByPlan,
    ...
    const response: ApiResponse = {
      ...
      data: {
        ...
        newSubscriptionsThisMonth: newSubscriptions.reduce((total, sub) => total + sub.count, 0)
      }
    };
 ...
 */
export const getAdvertisingAnalytics = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const [
      campaignStats,
      revenueStats,
 ...
 */
export const getRevenueSettings = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
 ...
 */
export const updateRevenueSettings = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const updateData = req.body;
 ...
 */
export const generateRevenueReport = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const {
      startDate,
      endDate,
    ...
    const [
      ...
      includeVendorBreakdown ? RevenueTransaction.aggregate([
        { $match: dateFilter },
        {
          $group: {
            _id: '$vendorId',
