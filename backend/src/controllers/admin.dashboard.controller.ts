...
import {
  ...
  Payment,
  Category,
  Blog, // Notification, // Commented out - notification system disabled
  Affiliate,
  Coupon,
  RevenueTransaction,
  ...
  try {
    ...
    const customerRetentionRate =
      totalUsers > 0
        ...
        : 0;
    const eventApprovalRate =
      totalEvents > 0 ? (approvedEvents / totalEvents) * 100 : 0;
    ...
    // ✅ Cache with tiered TTL strategy (1 minute for high-churn dashboard data)
    // This dramatically reduces load - 40+ queries cached!
    await cacheService.set(cacheKey, dashboardStats, {
 ...
 *
 * OPTIMIZATION: Caching (2 minutes TTL) + optimized parallel queries
 *
 * ARCHITECTURE NOTE: This uses 9 parallel queries which is efficient with Promise.all.
 * For future scaling (100K+ activities), consider:
 * 1. Dedicated Activity collection with change streams
 * 2. MongoDB $unionWith (requires 4.4+) to merge collections in single query
 * 3. Event sourcing pattern with activity log
 *
 * Current approach is optimal for medium-scale (< 50K activities) with aggressive caching.
 */
export const getRecentActivity = async (
  ...
  try {
    ...
    // ✅ OPTIMIZATION: Reduced limit per collection (10 → 5) for faster queries
    // Total: 45 activities fetched, then sorted and paginated
    const PER_COLLECTION_LIMIT = 5;

    // Get recent activities from multiple sources
    // ✅ OPTIMIZED: Parallel queries with Promise.all (efficient for < 50K records)
    // Each query is cached independently and uses minimal field projection
    const [
      recentUsers,
      recentEvents,
      recentOrders,
      ...
      // Recent users - only essential fields
      User.find()
        .sort({ createdAt: -1 })
        .limit(PER_COLLECTION_LIMIT)
        .select("_id firstName lastName email role status createdAt")
        .lean(),

      // Recent events - with vendor info
      Event.find({ isDeleted: false })
        .sort({ createdAt: -1 })
        .limit(PER_COLLECTION_LIMIT)
        .populate("vendorId", "firstName lastName email")
        .select("_id title status isApproved createdAt vendorId type category")
        .lean(),
      ...
      Order.find()
        ...
        .select("_id orderNumber total paymentStatus status createdAt userId")
        .lean(),

      // Recent reviews - with user and event info
      Review.find()
        .sort({ createdAt: -1 })
        .limit(PER_COLLECTION_LIMIT)
        .populate("userId", "firstName lastName")
        .populate("eventId", "title")
        .select("_id rating comment createdAt userId eventId")
        .lean(),

      // Recent bookings - with user and event info
      Booking.find()
        .sort({ createdAt: -1 })
        .limit(PER_COLLECTION_LIMIT)
        .populate("userId", "firstName lastName")
        .populate("eventId", "title")
        .select("_id status createdAt userId eventId")
        .lean(),

      // Recent tickets - with user and event info
      Ticket.find()
        .sort({ createdAt: -1 })
        .limit(PER_COLLECTION_LIMIT)
        .populate("userId", "firstName lastName")
      ...
      Payment.find()
        ...
        .limit(PER_COLLECTION_LIMIT)
        .populate("userId", "firstName lastName")
        .select("_id amount status method createdAt userId")
        .lean(),
      ...
      // Recent blogs - with author info
      Blog.find()
        .sort({ createdAt: -1 })
        .limit(PER_COLLECTION_LIMIT)
        .populate("authorId", "firstName lastName")
        .select("_id title status createdAt authorId")
        .lean(),

      // Recent affiliates - with user info
      Affiliate.find()
        .sort({ createdAt: -1 })
        .limit(PER_COLLECTION_LIMIT)
        .populate("userId", "firstName lastName")
        .select("_id code status commission createdAt userId")
        .lean(),
    ]);
    ...
    // ✅ Cache with tiered TTL strategy (30 seconds for high-churn activity data)
    // Short TTL since activity data changes frequently
    await cacheService.set(cacheKey, responseData, {
      ttl: CacheTTL.ADMIN_ACTIVITY,
    });
  ...
  try {
    ...
    const [
      ...
      User.aggregate([
        { $match: { createdAt: { $gte: startDate } } },
        {
          $group: {
            ...
            count: { $sum: 1 },
            // ✅ Removed unbounded $push operation
          },
        },
        { $sort: { "_id.year": 1, "_id.month": 1, "_id.day": 1 } },
        { $limit: 366 }, // ✅ Max 1 year of daily data to prevent OOM
      ]),
      ...
      Event.aggregate([
        ...
        { $sort: { totalRevenue: -1 } },
        { $limit: 50 }, // ✅ Limit to top 50 categories to prevent unbounded results
      ]),
      ...
      Order.aggregate([
        ...
        { $sort: { "_id.year": 1, "_id.month": 1, "_id.day": 1 } },
        { $limit: 366 }, // ✅ Max 1 year of daily revenue data to prevent unbounded results
      ]),
      ...
      Order.aggregate([
        ...
        { $limit: 24 }, // ✅ Max 2 years of monthly refund data to prevent unbounded results
      ]),
      ...
      Booking.aggregate([
        ...
        { $sort: { "_id.year": 1, "_id.month": 1, "_id.day": 1 } },
        { $limit: 366 }, // ✅ Max 1 year of daily booking data to prevent unbounded results
      ]),
      ...
      Order.aggregate([
        ...
        { $sort: { _id: 1 } },
        { $limit: 52 }, // ✅ Max 1 year of weekly data to prevent unbounded results
      ]),
      ...
      Order.aggregate([
        ...
        { $sort: { "_id.year": 1, "_id.month": 1 } },
        { $limit: 24 }, // ✅ Max 2 years of monthly data to prevent unbounded results
      ]),
