...
export const claimAffiliateEvent = async (req: Request, res: Response) => {
  try {
    ...
    logger.error("Error claiming affiliate event:", error);
    res
      .status(500)
      .json({ message: "Error claiming event", error: error.message });
  }
};
...
export const getClaimedEvents = async (req: Request, res: Response) => {
  try {
    ...
    const claimedEvents = await Event.find({
      ...
      .select(
        "title description images category type dateSchedule location affiliateClickTracking claimedAt externalBookingLink",
      )
      .sort({ claimedAt: -1 });
    ...
    logger.error("Error fetching claimed events:", error);
    res
      .status(500)
      .json({ message: "Error fetching claimed events", error: error.message });
  }
};
...
export const getAffiliateAnalytics = async (req: Request, res: Response) => {
  try {
    ...
    const matchFilter: any = { isAffiliateEvent: true };
