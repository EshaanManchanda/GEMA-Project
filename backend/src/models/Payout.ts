...
payoutSchema.methods.markAsCompleted = async function (
  ...
  if (
    ![PayoutRequestStatus.APPROVED, PayoutRequestStatus.PROCESSING].includes(
      this.status,
    )
  ) {
    throw new Error(
      "Only approved or processing payouts can be marked as completed",
    );
  }
...
payoutSchema.methods.markAsFailed = async function (
  reason: string,
): Promise<IPayout> {
  if (
    ![PayoutRequestStatus.APPROVED, PayoutRequestStatus.PROCESSING].includes(
    ...
    throw new Error(
      "Only approved or processing payouts can be marked as failed",
    );
  }
...
payoutSchema.methods.cancel = async function (
  ...
  if (
    ![PayoutRequestStatus.PENDING, PayoutRequestStatus.APPROVED].includes(
      this.status,
    )
  ) {
...
interface IPayoutModel extends mongoose.Model<IPayout> {
  findByVendor(
    vendorId: string,
    status?: PayoutRequestStatus,
  ): mongoose.Query<IPayout[], IPayout>;
