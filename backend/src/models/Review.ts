...
export interface IReview extends Document {
  ...
  teacher?: mongoose.Types.ObjectId;  // Teacher's User ID — used for type=TEACHER reviews
  user: mongoose.Types.ObjectId;
...
const reviewSchema = new Schema<IReview>(
  {
    ...
    teacher: {
      ...
      required: function () {
        return this.type === ReviewType.TEACHER;
      },
    },
    user: {
      type: Schema.Types.ObjectId,
...
reviewSchema.post("save", async function (doc) {
  try {
    ...
    if (doc.type === ReviewType.EVENT && doc.event) {
      ...
      if (
        statusChanged ||
        (ratingChanged && isApproved) ||
        (doc.isNew && isApproved)
      ) {
    ...
    logger.error("Error in post-save middleware for review stats:", error);
...
reviewSchema.post("deleteOne", async function (doc: any) {
  try {
    ...
    logger.error(
      "Error in post-deleteOne middleware for review stats:",
      error,
    );
...
reviewSchema.post("findOneAndDelete", async function (doc) {
  try {
    ...
    logger.error(
      "Error in post-findOneAndDelete middleware for review stats:",
      error,
    );
  }
});
...
reviewSchema.methods.addHelpfulVote = function (
  ...
  // Update counters
  this.helpful = this.helpfulVotes.filter(
    (vote: IHelpfulVote) => vote.helpful,
  ).length;
  this.notHelpful = this.helpfulVotes.filter(
    (vote: IHelpfulVote) => !vote.helpful,
  ).length;
...
reviewSchema.methods.addFlag = function (
  ...
  const existingFlag = this.flags.find(
    (flag: IFlag) => flag.user.equals(userId) && !flag.resolved,
  );
  if (existingFlag) {
...
reviewSchema.statics.findByVendor = function (
...
reviewSchema.statics.updateEventStats = async function (
  this: IReviewModel,
  ...
  try {
    ...
    logger.info(
      `Updated review stats for event ${eventId}: ${stats.totalReviews} reviews, ${stats.averageRating} avg rating`,
    );
    ...
    try {
      ...
      logger.error(
        `Error updating combined rating for event ${eventId}:`,
        error,
      );
      // Don't throw - combined rating calculation failures shouldn't break review stats update
    }
  } catch (error) {
    logger.error(`Error updating event stats for ${eventId}:`, error);
    throw error;
  }
};
