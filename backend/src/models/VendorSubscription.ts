...
export interface IVendorSubscription extends Document {
  ...
  addOns: Array<{
    type: 'extra_events' | 'extra_storage' | 'extra_bandwidth' | 'priority_listing';
    name: string;
    price: number;
    quantity: number;
    billingCycle: BillingCycle;
...
const VendorSubscriptionSchema = new Schema<IVendorSubscription>(
  {
    ...
    addOns: [{
      type: {
        type: String,
        enum: ['extra_events', 'extra_storage', 'extra_bandwidth', 'priority_listing'],
        required: true
      },
      name: {
        type: String,
...
VendorSubscriptionSchema.virtual('mrr').get(function(this: IVendorSubscription) {
...
VendorSubscriptionSchema.methods.getRemainingEvents = function(): number {
  return Math.max(0, this.planFeatures.maxEvents - this.usageTracking.eventsCreated);
};
...
VendorSubscriptionSchema.methods.pauseSubscription = async function(reason?: string): Promise<IVendorSubscription> {
...
VendorSubscriptionSchema.methods.cancelSubscription = async function(reason?: string): Promise<IVendorSubscription> {
...
VendorSubscriptionSchema.methods.addUsageWarning = function(type: string, threshold: number): void {
...
const VendorSubscription = mongoose.model<IVendorSubscription>('VendorSubscription', VendorSubscriptionSchema);
