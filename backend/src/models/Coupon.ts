...
const couponSchema = new Schema<ICoupon>(
  {
    code: {
      ...
      match: [/^[A-Z0-9-_]+$/, 'Coupon code can only contain uppercase letters, numbers, hyphens, and underscores'],
    },
    name: {
      type: String,
...
couponSchema.methods.isValidForUser = async function (userId: mongoose.Types.ObjectId): Promise<boolean> {
  ...
  if (this.userUsageLimit) {
    const userUsageCount = this.usage.filter(u => u.userId.toString() === userId.toString()).length;
