...
export interface IAffiliate extends Document {
  ...
  // Methods
  generateTrackingUrl(eventId?: mongoose.Types.ObjectId, customParams?: Record<string, string>): string;
  recordClick(clickData: Partial<IAffiliateClick>): Promise<IAffiliate>;
  recordConversion(orderId: mongoose.Types.ObjectId, orderAmount: number): Promise<IAffiliate>;
  calculateCommission(orderAmount: number, eventId?: mongoose.Types.ObjectId, category?: string): number;
...
export interface IAffiliateModel extends mongoose.Model<IAffiliate> {
  findByCode(affiliateCode: string): mongoose.Query<IAffiliate | null, IAffiliate>;
...
const affiliateSchema = new Schema<IAffiliate>(
  {
    ...
    affiliateCode: {
      ...
      match: [/^[A-Z0-9]+$/, 'Affiliate code can only contain uppercase letters and numbers'],
    },
    status: {
      type: String,
...
affiliateSchema.methods.recordClick = async function (clickData: Partial<IAffiliateClick>): Promise<IAffiliate> {
...
affiliateSchema.methods.recordConversion = async function (
  ...
  const click = this.clicks.find(c => 
    !c.converted && 
    c.clickedAt > expiryDate
  );
...
affiliateSchema.methods.calculateCommission = function (
  ...
  if (eventId) {
    const eventCommission = this.eventCommissions.find(ec => 
  ...
  if (category && !eventId) {
    const categoryCommission = this.categoryCommissions.find(cc => 
  ...
  if (type === CommissionType.TIERED && this.commissionTiers?.length) {
    const applicableTier = this.commissionTiers.find(tier => 
...
affiliateSchema.methods.getMonthlyStats = function (year: number, month: number) {
  return this.monthlyStats.find(stats => stats.year === year && stats.month === month);
};

// Method to update monthly stats
affiliateSchema.methods.updateMonthlyStats = async function (): Promise<IAffiliate> {
  const now = new Date();
  const year = now.getFullYear();
  const month = now.getMonth() + 1;
  
  let monthStats = this.monthlyStats.find(stats => stats.year === year && stats.month === month);
  ...
  monthStats.clicks = this.clicks.filter(c => 
    c.clickedAt >= monthStart && c.clickedAt <= monthEnd
  ).length;
  
  monthStats.conversions = this.clicks.filter(c => 
    c.converted && c.conversionDate && 
    c.conversionDate >= monthStart && c.conversionDate <= monthEnd
  ).length;
  ...
  monthStats.revenue = monthCommissions.reduce((sum, c) => sum + c.orderAmount, 0);
  monthStats.commissionEarned = monthCommissions.reduce((sum, c) => sum + c.commissionAmount, 0);
...
affiliateSchema.methods.canReceivePayout = function (): boolean {
  ...
  return pendingAmount >= this.minimumPayoutAmount && this.status === AffiliateStatus.ACTIVE;
};

// Method to get pending commissions
affiliateSchema.methods.getPendingCommissions = function (): IAffiliateCommission[] {
...
affiliateSchema.methods.getTotalPendingAmount = function (): number {
  return this.getPendingCommissions().reduce((sum, c) => sum + c.commissionAmount, 0);
};
...
const Affiliate = mongoose.model<IAffiliate, IAffiliateModel>('Affiliate', affiliateSchema);
