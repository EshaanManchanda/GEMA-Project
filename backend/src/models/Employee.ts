...
const EmployeeSchema = new Schema<IEmployee>({
  ...
  phone: { type: String },
  role: { type: String, enum: ['manager', 'scanner', 'coordinator', 'security'], required: true },
  permissions: [{
    action: { type: String, required: true },
    scope: { type: String, enum: ['all', 'assigned'], required: true },
  }],
  assignedEvents: [{ type: Schema.Types.ObjectId, ref: 'Event' }],
  assignedVenues: [{ type: Schema.Types.ObjectId, ref: 'Venue' }],
  ...
  scanHistory: [{
    ...
    result: { type: String, enum: ['success', 'invalid', 'duplicate', 'expired'], required: true },
  }],
  emergencyContact: {
    name: { type: String },
    phone: { type: String },
