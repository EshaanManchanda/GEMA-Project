...
reelSchema.post("save", function (doc) {
  ...
  logger.info(`[Reel] Cache invalidation needed for reel: ${doc._id}`);
});

// Post-delete middleware: Cache invalidation
reelSchema.post("deleteOne", function () {
  logger.info(
    `[Reel] Cache invalidation needed after deleteOne: ${this.getQuery()._id}`,
  );
});

reelSchema.post("deleteMany", function () {
  logger.info(`[Reel] Cache invalidation needed after deleteMany`);
});
...
reelSchema.virtual("thumbnailUrlComputed").get(function () {
  ...
  if (
    this.populated("thumbnailAsset") &&
    typeof this.thumbnailAsset === "object"
  ) {
...
reelSchema.methods.extractInstagramId = function (): string | null {
  if (this.videoSourceType !== "instagram" || !this.externalVideoUrl)
    return null;
