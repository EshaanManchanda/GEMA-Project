...
VendorSchema.methods.canSwitchToCustomStripe = function (): boolean {
  ...
  const hasStripeConnect =
    this.paymentSettings.stripeSettings.stripeConnectOnboardingComplete;
  const hasManualKeys =
    this.paymentSettings.stripeSettings.stripePublishableKey &&
...
VendorSchema.methods.isSubscriptionActive = function (): boolean {
  ...
  if (
    ...
    status === VendorSubscriptionStatus.GRACE_PERIOD
  ) {
    return true;
  }
...
VendorSchema.methods.getEffectiveCommissionRate = function (): number {
  ...
  if (
    this.paymentSettings.paymentMode === PaymentMode.CUSTOM_STRIPE &&
  ...
  if (
    this.paymentSettings.commissionAgreements &&
    this.paymentSettings.commissionAgreements.length > 0
  ) {
    const now = new Date();
    const activeAgreement = this.paymentSettings.commissionAgreements.find(
      (agreement) => {
