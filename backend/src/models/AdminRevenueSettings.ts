  ...
  if (this.revenueSharingRules && this.revenueSharingRules.length > 0) {
    for (const rule of this.revenueSharingRules) {
      ...
      if (Math.abs(total - 100) > 0.01) {
        ...
        return next(
          new Error(
            `Revenue sharing rule for ${rule.revenueStream} must total 100%. Current total: ${total}%`,
          ),
        );
      }
    }
...
AdminRevenueSettingsSchema.methods.calculateCommission = function (
  ...
  if (
    this.commissionStructure === CommissionStructure.TIERED &&
    this.tieredCommissions
  ) {
...
AdminRevenueSettingsSchema.methods.getEffectiveRefundPolicy = function (
  category?: string,
) {
...
AdminRevenueSettingsSchema.methods.isPaymentGatewayEnabled = function (
  gateway: string,
): boolean {
...
AdminRevenueSettingsSchema.methods.getMinimumPayoutForVendor = function (
  vendorId: string,
): number {
...
const AdminRevenueSettings = mongoose.model<
  IAdminRevenueSettings,
  IAdminRevenueSettingsModel
>("AdminRevenueSettings", AdminRevenueSettingsSchema);
