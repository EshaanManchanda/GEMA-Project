 ...
 */
import { parsePhoneNumber, isValidPhoneNumber, CountryCode, PhoneNumber } from 'libphonenumber-js';
...
export const validatePhoneNumber = (
  ...
  try {
    ...
    if (!phoneNumber) {
      return {
        isValid: false,
        error: 'Unable to parse phone number. Please use international format (e.g., +1234567890)',
        errorCode: PhoneValidationError.INVALID_FORMAT,
      };
    }
 ...
 */
export const isMobileNumber = (phone: string, defaultCountry?: CountryCode): boolean => {
...
export const validatePhoneForAPI = async (
  ...
  const { requireMobile = true, checkDuplicate = true, excludeUserId, defaultCountry } = options;
  ...
  if (requireMobile && !validation.isMobile) {
    return {
      isValid: false,
      error: 'Only mobile phone numbers are allowed. Landline numbers cannot receive SMS verification codes.',
  ...
  if (checkDuplicate && validation.e164Format) {
    const isDuplicate = await isPhoneNumberDuplicate(validation.e164Format, excludeUserId);
 ...
 */
export const sanitizeToE164 = (phone: string, defaultCountry?: CountryCode): string | null => {
  const validation = validatePhoneNumber(phone, defaultCountry);
...
export const getPhoneValidationErrorMessage = (
  ...
  const exampleNumber = country ? getExamplePhoneNumber(country) : '+1 234 567 8900';
