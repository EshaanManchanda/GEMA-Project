...
export const generateQRCode = async (
  ...
  try {
    logger.info("🖼️ QR CODE UTILS: Starting QR code generation", {
    ...
    logger.info("🔧 QR CODE UTILS: Using QR options", qrOptions);

    // Generate QR code as data URL (base64)
    const qrCodeDataURL = await QRCode.toDataURL(data, qrOptions);

    logger.info("✅ QR CODE UTILS: QR code generated successfully", {
    ...
    logger.error("❌ QR CODE UTILS: Error generating QR code", {
      error: error.message,
      stack: error.stack,
...
export const generateSecureQRData = (data: {
  ...
  logger.info("🔒 QR DATA UTILS: Generating secure QR data", {
...
export const generateSecureQRDataLegacy = (data: {
  ...
  const secureData = {
    ...
    checksum: Buffer.from(`${data.ticketNumber}-${data.eventId}-${timestamp}`)
      .toString("base64")
      .slice(0, 8),
  };
...
export const validateQRData = (
  ...
  try {
    ...
    const expectedChecksum = Buffer.from(
      `${data.ticketNumber}-${data.eventId}-${data.generatedAt}`,
    )
      .toString("base64")
      .slice(0, 8);
