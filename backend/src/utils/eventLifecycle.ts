...
export const archiveExpiredEvents = async () => {
  try {
    logger.info("Running event expiration job...");
    ...
    if (eventIdsToArchive.length > 0) {
      ...
      logger.info("Invalidating cache for archived events...");
      ...
      logger.info(`Invalidated ${deletedListKeys} event list cache entries`);
    }
    ...
    logger.error("Error in event expiration job:", error);
    return {
      success: false,
...
export const reactivateValidEvents = async () => {
  try {
    logger.info("Running event reactivation check...");
    ...
    if (eventIdsToReactivate.length > 0) {
      ...
      logger.info("Invalidating cache for reactivated events...");
    ...
    logger.error("Error in event reactivation job:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : "Unknown error",
    };
...
export const getEventLifecycleStats = async () => {
  try {
    ...
    logger.error("Error getting event lifecycle stats:", error);
    throw error;
  }
};
...
export const runEventLifecycleMaintenance = async () => {
  try {
    logger.info("Starting event lifecycle maintenance...");
    ...
    logger.info("Event lifecycle maintenance completed.");
    ...
    logger.error("Error in event lifecycle maintenance:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : "Unknown error",
    };
...
export const scheduleEventLifecycleJobs = () => {
  try {
    ...
    logger.info("Event lifecycle management jobs scheduled successfully");
    logger.info("- Full maintenance: Daily at 00:00 UTC");
    logger.info("- Quick expiration check: Every 6 hours");

    return true;
  } catch (error) {
    logger.error("Error scheduling event lifecycle jobs:", error);
    return false;
  }
};
...
export const stopEventLifecycleJobs = () => {
  if (lifecycleMaintenanceTask) {
    ...
    logger.info("Event lifecycle maintenance job stopped");
  }

  if (quickExpirationTask) {
    quickExpirationTask.stop();
    logger.info("Quick expiration check job stopped");
  }
};
