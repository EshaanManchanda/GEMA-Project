...
export class RelationshipResolver {
  ...
  async resolveVendor(record: any): Promise<RelationshipResolution> {
    ...
    if (record.vendorId) {
      ...
      const vendor = await User.findById(record.vendorId).select('_id email role');
  ...
  async resolveCategory(record: any): Promise<RelationshipResolution> {
    ...
    if (record.categoryId) {
      ...
      const category = await Category.findById(record.categoryId).select('_id slug name isActive');
   ...
   */
  async resolveUser(record: any, fieldPrefix: string = 'user'): Promise<RelationshipResolution> {
    ...
    if (record[idField]) {
      ...
      const user = await User.findById(record[idField]).select('_id email role');
   ...
   */
  async resolveEvent(record: any, fieldPrefix: string = 'event'): Promise<RelationshipResolution> {
  ...
  async resolveParentCategory(record: any): Promise<RelationshipResolution> {
    ...
    if (record.parentId) {
      ...
      const parent = await Category.findById(record.parentId).select('_id slug name level');
  ...
  async resolveBlogCategory(record: any): Promise<RelationshipResolution> {
    ...
    if (record.categoryId) {
      ...
      const category = await BlogCategory.findById(record.categoryId).select('_id name slug isActive');
   ...
   */
  async validateCategoryParent(categoryId: string | undefined, parentId: string): Promise<void> {
    if (!parentId) return;
    ...
    while (current) {
      ...
      if (visited.has(current)) {
        throw new AppError('Circular parent reference detected in existing data', 400);
      }
