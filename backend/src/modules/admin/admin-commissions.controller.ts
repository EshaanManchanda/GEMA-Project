...
import { CommissionConfig, ConfigStatus, CommissionTransaction, CommissionTransactionStatus } from "../../models/index";
...
export const updateCommissionConfig = async (
  ...
  try {
    ...
    const config = await CommissionConfig.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
...
export const deleteCommissionConfig = async (
  ...
  try {
    ...
    if (config.isDefault) {
      return next(
        new AppError("Cannot delete default commission configuration", 400),
      );
    }
...
export const getCommissionTransactions = async (
  ...
  try {
    ...
    if (startDate || endDate) {
      ...
      if (startDate)
        matchStage.calculatedAt.$gte = new Date(startDate as string);
    ...
    const mostRecentDate =
      aggregationResult[0]?.mostRecent[0]?.calculatedAt || new Date();
  ...
  try {
    ...
    for (const id of transactionIds) {
      try {
        ...
        errors.push(
          `Failed to approve transaction ${id}: ${(error as Error).message}`,
        );
      }
    }
...
export const getCommissionStats = async (
  ...
  try {
    ...
    const stats = {
      ...
      totalAmount,
      pendingCommissions:
        statusMap[CommissionTransactionStatus.CALCULATED]?.count || 0,
      pendingAmount:
        statusMap[CommissionTransactionStatus.CALCULATED]?.amount || 0,
      approvedCommissions:
        statusMap[CommissionTransactionStatus.APPROVED]?.count || 0,
      approvedAmount:
        statusMap[CommissionTransactionStatus.APPROVED]?.amount || 0,
      ...
      averageCommissionRate:
        totalAmount > 0 ? totalAmount / totalCommissions : 5,
...
export const getCommissionAnalytics = async (
  ...
  try {
    ...
    if (startDate || endDate) {
      ...
      if (startDate)
        dateFilter.calculatedAt.$gte = new Date(startDate as string);
  ...
  try {
    ...
    for (const id of transactionIds) {
      try {
        ...
        errors.push(
          `Failed to reject transaction ${id}: ${(error as Error).message}`,
        );
      }
    }
