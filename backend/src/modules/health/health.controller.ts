...
async function checkRedisHealthStatus(): Promise<
  ServiceStatus & {
    poolEnabled?: boolean;
    poolSize?: number;
    poolActive?: number;
...
async function checkFirebaseHealth(): Promise<ServiceStatus> {
  ...
  try {
    ...
    return {
      ...
      lastChecked: new Date().toISOString(),
      error:
        error instanceof Error ? error.message : "Firebase health check failed",
    };
  }
}
...
async function getSystemMetrics() {
  try {
    ...
    // Get database counts
    const [totalUsers, totalEvents, totalOrders, activeBookings] =
      await Promise.all([
        User.countDocuments(),
    ...
    const systemUsedPercentage = Math.round(
      (systemUsedMB / systemTotalMB) * 100,
    );
