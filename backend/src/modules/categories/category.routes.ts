...
const sortOrderValidation = [
  ...
  body("categories.*.id").isMongoId().withMessage("Category ID must be valid"),
...
router.get(
  ...
  param("parentId")
    .custom((value) => {
      return (
        value === "null" || require("mongoose").Types.ObjectId.isValid(value)
      );
    })
...
router.post("/update-counts", authorize(["admin"]), updateEventCounts);
