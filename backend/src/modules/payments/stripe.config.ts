...
const getStripeKeys = () => {
  ...
  if (paymentEnv === "production" || useLiveKeys) {
    ...
    logger.info("🔴 Using LIVE Stripe keys");
  } else {
...
const validateStripeKey = (key: string, keyType: string): void => {
  ...
  const expectedPrefixes = keyType.includes("secret")
    ? keyPrefixes.secret
    : keyPrefixes.publishable;
  ...
  if (!hasValidPrefix) {
    throw new Error(
      `Invalid ${keyType} format. Expected format: ${expectedPrefixes.join(" or ")}...`,
    );
  }
  ...
  if (key.length < 20) {
    throw new Error(
      `${keyType} appears to be too short. Please check your Stripe configuration.`,
    );
  }
};
...
export const convertFromStripeAmount = (
  amount: number,
  currency: string,
): number => {
  const stripeCurrency = "aed"; // Always AED
