...
export const getVendorPayoutsDashboard = async (
  ...
  try {
    ...
    // Eligible balance: pending transactions past the 24hr refund window
    const now = new Date();
    ...
    res.status(200).json({
      ...
      data: {
        ...
        paymentSettings: {
          hasStripeAccount: vendorDoc.paymentSettings?.stripeSettings
            ?.stripeConnectAccountId
            ? true
            : false,
          subscriptionActive:
            vendorDoc.paymentSettings?.subscriptionStatus === "active",
          preferredPayoutMethod:
            vendorDoc.paymentSettings?.preferredPayoutMethod || "bank_transfer",
          minimumPayout: vendorDoc.paymentSettings?.minimumPayout || 50,
          paymentMode:
            vendorDoc.paymentSettings?.paymentMode || "platform_stripe",
  ...
  try {
    ...
    const settings = {
      ...
      stripeConnectOnboarded:
        vendorDoc.paymentSettings?.stripeSettings
...
export const getSubscriptionStatus = async (
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        paymentMode:
          paymentSettings?.paymentMode || PaymentMode.PLATFORM_STRIPE,
        isSubscriptionModel,
...
export const getCommissionHistory = async (
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        paymentMode:
          vendorDoc.paymentSettings?.paymentMode || PaymentMode.PLATFORM_STRIPE,
        commissionRate: vendorDoc.getEffectiveCommissionRate(),
