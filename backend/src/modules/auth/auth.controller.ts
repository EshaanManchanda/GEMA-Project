...
export const registerStudent = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { firstName, lastName, email, password, phone, schoolId, studentId, grade, section, parentEmail, dateOfBirth } = req.body;

    if (!email || !password || !schoolId || !studentId || !grade) {
      throw new AppError("Email, password, schoolId, studentId, and grade are required", 400);
    }
    ...
    if (existingStudent) throw new AppError("Student ID already exists for this school", 400);

    const verificationOTP = generateOTP();
    const otpExpiry = getOTPExpiry();

    const user = await User.create({
      firstName, lastName, email, passwordHash: password, phone,
      role: UserRole.STUDENT, status: UserStatus.PENDING,
      emailVerification: { otp: verificationOTP, expiresAt: otpExpiry },
    });

    const StudentModel = mongoose.model("Student");
    const student = await StudentModel.create({
      userId: user._id, schoolId, firstName, lastName,
      dateOfBirth: dateOfBirth ? new Date(dateOfBirth) : new Date(),
      studentId, grade, section, enrollmentDate: new Date(),
    });
      ...
      if (parentUser) {
        ...
        if (parent) {
          ...
          parent.relationshipToStudents.push({
            studentId: student._id, relationship: "guardian",
            isPrimary: parent.studentIds.length === 0,
            hasBookingPermission: true, hasViewAccess: true,
          });
    ...
    res.status(201).json({
      success: true, message: "Student registered successfully. Auth cookies have been set.",
      data: { user: formatUserResponse(user), tokens },
    });
  } catch (error) {
    next(error);
  }
...
export const registerParent = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { firstName, lastName, email, password, phone, studentIds } = req.body;
    ...
    const user = await User.create({
      firstName, lastName, email, passwordHash: password, phone,
      role: UserRole.PARENT, status: UserStatus.PENDING,
      emailVerification: { otp: verificationOTP, expiresAt: otpExpiry },
    });

    const Parent = mongoose.model("Parent");
    const parent = await Parent.create({
      userId: user._id, firstName, lastName, phone: phone || "", studentIds: studentIds || [],
    });

    user.parentProfileId = parent._id;
    await user.save();

    if (studentIds?.length) {
      const Student = mongoose.model("Student");
      await Student.updateMany({ _id: { $in: studentIds } }, { $addToSet: { parentIds: user._id } });
    }
    ...
    res.status(201).json({
      success: true, message: "Parent registered successfully. Auth cookies have been set.",
      data: { user: formatUserResponse(user), tokens },
    });
  } catch (error) {
    next(error);
...
export const switchRole = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { role } = req.body;
    const userId = req.user?._id || req.user?.id;
    if (!userId) throw new AppError("User not authenticated", 401);

    const validRoles = [UserRole.STUDENT, UserRole.PARENT, UserRole.CUSTOMER, UserRole.VENDOR, UserRole.TEACHER, UserRole.SCHOOL, UserRole.EMPLOYEE];
    if (!validRoles.includes(role)) throw new AppError("Invalid role for switching", 400);
    ...
    res.json({
      success: true, message: `Switched to ${role} role`,
      data: { user: formatUserResponse(user), activeRole: role, dashboardUrl: dashboardUrls[role] || "/dashboard" },
    });
  } catch (error) {
    next(error);
  }
...
export const getAvailableRoles = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const userId = req.user?._id || req.user?.id;
    if (!userId) throw new AppError("User not authenticated", 401);
    ...
    if (user.studentProfileId) roles.push(UserRole.STUDENT);
    if (user.parentProfileId) roles.push(UserRole.PARENT);
    if (user.vendorId) roles.push(UserRole.VENDOR);
    if (user.teacherProfileId) roles.push(UserRole.TEACHER);
    if (user.schoolId) roles.push(UserRole.SCHOOL);
    if (user.employeeProfileId) roles.push(UserRole.EMPLOYEE);
    ...
    res.json({
      ...
      data: {
        ...
        availableRoles: roles.map((r) => ({ role: r, dashboardUrl: dashboardUrls[r] || "/dashboard" })),
      },
    });
  } catch (error) {
    next(error);
