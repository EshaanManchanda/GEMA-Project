...
const cancellationLogSchema = new Schema<ICancellationLog>(
  {
    ...
    serviceFee: { type: Number, default: 0, min: [0, "Service fee cannot be negative"] },
...
cancellationLogSchema.index({ eventId: 1, type: 1 });
cancellationLogSchema.index({ refundStatus: 1, createdAt: -1 });
...
cancellationLogSchema.statics.findPendingRefunds = function () {
  return this.find({ refundStatus: { $in: ["pending", "processing"] } }).sort({
    createdAt: 1,
  });
};
...
const CancellationLog = mongoose.model<ICancellationLog>(
  "CancellationLog",
