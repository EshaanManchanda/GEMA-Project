...
const EventAddonSchema = new Schema<IEventAddon>(
  {
    ...
    promotionalOffer: {
      description: {
        ...
        maxlength: [
          200,
          "Promotional description cannot exceed 200 characters",
        ],
      },
      discountPercentage: {
        type: Number,
...
EventAddonSchema.methods.isAvailable = function (
  date: Date,
  quantity: number = 1,
): boolean {
  ...
  if (
    this.requirements.maximumCapacity &&
    quantity > this.requirements.maximumCapacity
  ) {
    return false;
  }
  ...
  switch (this.availabilitySchedule.type) {
    ...
    case AvailabilityType.SEASONAL:
      ...
      return this.availabilitySchedule.seasonalMonths.includes(
        date.getMonth() + 1,
      );
...
EventAddonSchema.methods.calculatePrice = function (
  quantity: number,
  eventSize?: number,
): number {
  let price = 0;
  ...
  switch (this.pricingModel) {
    ...
    case PricingModel.PER_PERSON:
      if (!eventSize)
        throw new Error("Event size required for per-person pricing");
  ...
  if (this.promotionalOffer?.isActive) {
    const now = new Date();
    if (
      this.promotionalOffer.validFrom <= now &&
      this.promotionalOffer.validTo >= now
    ) {
...
EventAddonSchema.methods.calculateAdminRevenue = function (
  totalPrice: number,
): number {
...
EventAddonSchema.methods.updateBookingStats = async function (
...
EventAddonSchema.methods.addReview = async function (
  rating: number,
): Promise<IEventAddon> {
...
EventAddonSchema.statics.findAvailableForEvent = function (
  ...
  if (eventSize) {
    query.$and = [
      {
        $or: [
          { minimumEventSize: { $exists: false } },
          { minimumEventSize: { $lte: eventSize } },
        ],
      },
      {
        $or: [
          { maximumEventSize: { $exists: false } },
          { maximumEventSize: { $gte: eventSize } },
        ],
      },
    ];
  }
