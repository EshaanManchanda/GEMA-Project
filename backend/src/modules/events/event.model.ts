...
eventSchema.pre("validate", function (next) {
  ...
  // 2. Offline Events must have location details (address, city, country)
  if (this.venueType === "Offline") {
    // Note: location object is nested. Validation logic here complements schema-level validation.
    ...
    if (!this.location || !this.location.city) {
      this.invalidate("location.city", "City is required for offline events");
    }
  }
...
eventSchema.post("save", async function (doc: IEvent) {
  try {
    ...
    const { cacheService } = await import("../../shared/services/cache.service");
    const { getEventCacheKey, getEventListCachePattern } =
      await import("./events.utils");
...
eventSchema.post("updateMany", async function () {
  try {
    const { cacheService } = await import("../../shared/services/cache.service");
    const { getEventListCachePattern } = await import("./events.utils");
