...
import { validateEventSEO } from "./events.validator";
...
router.get("/vendor/my-events", authorize([UserRole.VENDOR]), getVendorEvents);
router.get("/vendor/analytics", authorize([UserRole.VENDOR]), getVendorEventAnalytics);
...
router.post(
  "/:id/claim",
  authorize([UserRole.VENDOR, UserRole.TEACHER]),
  [param("id").isMongoId().withMessage("Invalid event ID")],
  claimEvent,
);
...
// Admin routes
router.get("/admin/all", authorize([UserRole.ADMIN]), getAdminEvents);
