...
export class EventService {
  ...
  async updateEvent(id: string, data: Partial<IEvent>): Promise<IEvent> {
    ...
    try {
      ...
      const oldImageIds = (oldEvent.imageAssets || []).map((id) =>
        id.toString(),
      );
   ...
   */
  async updateCombinedRating(
    eventId: string | mongoose.Types.ObjectId,
  ): Promise<void> {
    try {
      const Review = (await import("../../models/index")).Review;
      ...
      const platformStats = await Review.getAverageRating(
        event._id as mongoose.Types.ObjectId,
        (await import("../../models/index")).ReviewType.EVENT,
      );
      ...
      if (event.googlePlaceId) {
        try {
          ...
          console.error(
            `Failed to fetch Google reviews for event ${eventId}:`,
            error,
          );
      ...
      if (totalReviews > 0) {
        ...
        combinedRating =
          platformStats.averageRating * platformWeight +
          googleRating * googleWeight;
      ...
      console.log(
        `Updated combined rating for event ${eventId}: ${combinedRating} (${platformStats.totalReviews} platform + ${googleReviewCount} Google)`,
      );
    } catch (error) {
      console.error(
