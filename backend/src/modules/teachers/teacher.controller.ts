...
import { TeacherBooking, User, UserRole, Teacher, Event } from "../../models/index";
import { AppError, catchAsync } from "../../middleware/index";
import mongoose from "mongoose";
import { AuthRequest } from "../../types/index";
import { NextFunction, Response } from "express";
import { getOrCreateTeacherProfile } from "./teacher.helpers";
...
export const getTeacherDashboardStats = catchAsync(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    const totalTeachingEvents = await Event.countDocuments({
      teacherId,
    ...
    // Get all event IDs
    const teacherEvents = await Event.find({
      teacherId,
      type: { $in: ["Class", "Course", "Workshop", "Bootcamp", "Masterclass"] },
    }).select("_id");
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    for (let i = 0; i < csvData.length; i++) {
      ...
      try {
        ...
        // Find teaching event
        const teachingEvent = await Event.findOne({
          teacherId,
          type: {
