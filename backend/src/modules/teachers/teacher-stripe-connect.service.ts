...
import { stripe } from "../payments/stripe.config";
...
class TeacherStripeConnectService implements IteacherStripeConnectService {
  ...
  async generateOnboardingLink(
    ...
    try {
      ...
      console.log(`✓ Generated onboarding link for teacher ${teacherId}`);
  ...
  async getAccountStatus(
    ...
    try {
      ...
      if (
        ...
        console.log(`✓ Teacher ${teacherId} completed Stripe onboarding`);
      }
  ...
  async disconnectAccount(teacherId: Types.ObjectId): Promise<void> {
    try {
      ...
      console.log(`✓ Disconnected Stripe account for teacher ${teacherId}`);
    } catch (error: any) {
  ...
  private async handleAccountUpdated(account: Stripe.Account): Promise<void> {
    ...
    if (!teacherId) {
      console.warn("Account updated but no teacherId in metadata");
      return;
    }

    const teacher = await Teacher.findById(teacherId);
    if (!teacher) {
      console.warn(`Teacher ${teacherId} not found for account ${account.id}`);
      return;
    }
    ...
    console.log(`✓ Updated teacher ${teacherId} from webhook`);
  }
  ...
  private async handleAccountDeauthorizedById(
    ...
    if (!teacher) {
      console.warn(`Teacher not found for deauthorized account ${accountId}`);
      return;
    }
  ...
  private async handleCapabilityUpdated(
    ...
    if (!teacher) {
      console.warn(`Teacher not found for account ${accountId}`);
      return;
    }
    ...
    console.log(`✓ Updated capabilities for teacher ${teacher._id}`);
  }
}
